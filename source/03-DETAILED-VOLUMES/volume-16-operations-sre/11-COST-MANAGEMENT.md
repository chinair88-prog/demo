...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Cost Management**.
...
|---|---|
| `CostManagement` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CostManagementVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CostManagementDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CostManagementEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CostManagementException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ValidateCostManagement`
- `ApproveCostManagement`
- `ActivateCostManagement`
- `SuspendCostManagement`
- `CorrectCostManagement`
- `RetireCostManagement`

Reference Events:

- `CostManagementCreated`
- `CostManagementValidated`
- `CostManagementApproved`
- `CostManagementActivated`
- `CostManagementSuspended`
- `CostManagementCorrected`
- `CostManagementRetired`

Reference queries:

- `GetCostManagement`
- `ListCostManagementRecords`
- `GetCostManagementTimeline`
- `GetCostManagementEvidence`
- `GetCostManagementDecisionPackage`
- `GetCostManagementExceptions`
- `GetCostManagementAuditTrail`

Reference API surface:

```text
POST /api/v1/costmanagement/commands
GET  /api/v1/costmanagement/{id}
GET  /api/v1/costmanagement/{id}/timeline
GET  /api/v1/costmanagement/{id}/evidence
GET  /api/v1/costmanagement/{id}/decisions
POST /api/v1/costmanagement/{id}/exceptions
POST /api/v1/costmanagement/{id}/corrections
```
...
|---|---|
| `costmanagement_aggregate` | authoritative subject |
| `costmanagement_version` | immutable submitted, approved, issued, or signed versions |
| `costmanagement_relationship` | governed relationships |
| `costmanagement_decision` | binding Decisions and rationale |
| `costmanagement_evidence_link` | provenance and evidence relationships |
| `costmanagement_event_outbox` | transactional Event publication |
| `costmanagement_idempotency` | duplicate-write protection |
| `costmanagement_exception` | exception, claim, dispute, or hold |
| `costmanagement_correction` | correction and supersession lineage |
| `costmanagement_audit` | privileged action evidence |
...
- `Cost Management` is part of the target GTOS handbook scope.
