...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for On-Call**.
...
|---|---|
| `On-Call` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `On-CallVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `On-CallDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `On-CallEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `On-CallException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `GetOn-CallTimeline`
- `GetOn-CallEvidence`
- `GetOn-CallDecisionPackage`
- `GetOn-CallExceptions`
- `GetOn-CallAuditTrail`

Reference API surface:

```text
POST /api/v1/on_call/commands
GET  /api/v1/on_call/{id}
GET  /api/v1/on_call/{id}/timeline
GET  /api/v1/on_call/{id}/evidence
GET  /api/v1/on_call/{id}/decisions
POST /api/v1/on_call/{id}/exceptions
POST /api/v1/on_call/{id}/corrections
```
...
|---|---|
| `on_call_aggregate` | authoritative subject |
| `on_call_version` | immutable submitted, approved, issued, or signed versions |
| `on_call_relationship` | governed relationships |
| `on_call_decision` | binding Decisions and rationale |
| `on_call_evidence_link` | provenance and evidence relationships |
| `on_call_event_outbox` | transactional Event publication |
| `on_call_idempotency` | duplicate-write protection |
| `on_call_exception` | exception, claim, dispute, or hold |
| `on_call_correction` | correction and supersession lineage |
| `on_call_audit` | privileged action evidence |
...
- `On-Call` is part of the target GTOS handbook scope.
