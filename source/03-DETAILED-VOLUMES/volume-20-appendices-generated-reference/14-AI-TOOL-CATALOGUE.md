...
This chapter defines how GTOS shall **define the generation source, schema, owner, evidence level, update cadence, validation, publication, retention, and use of the AI Tool Catalogue**.
...
|---|---|
| `AIToolCatalogue` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIToolCatalogueVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIToolCatalogueDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIToolCatalogueEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIToolCatalogueException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreateAIToolCatalogue`
- `ValidateAIToolCatalogue`
- `ApproveAIToolCatalogue`
- `ActivateAIToolCatalogue`
- `SuspendAIToolCatalogue`
- `CorrectAIToolCatalogue`
- `RetireAIToolCatalogue`

Reference Events:

- `AIToolCatalogueCreated`
- `AIToolCatalogueValidated`
- `AIToolCatalogueApproved`
- `AIToolCatalogueActivated`
- `AIToolCatalogueSuspended`
- `AIToolCatalogueCorrected`
- `AIToolCatalogueRetired`

Reference queries:

- `GetAIToolCatalogue`
- `ListAIToolCatalogueRecords`
- `GetAIToolCatalogueTimeline`
- `GetAIToolCatalogueEvidence`
- `GetAIToolCatalogueDecisionPackage`
- `GetAIToolCatalogueExceptions`
- `GetAIToolCatalogueAuditTrail`

Reference API surface:

```text
POST /api/v1/aitoolcatalogue/commands
GET  /api/v1/aitoolcatalogue/{id}
GET  /api/v1/aitoolcatalogue/{id}/timeline
GET  /api/v1/aitoolcatalogue/{id}/evidence
GET  /api/v1/aitoolcatalogue/{id}/decisions
POST /api/v1/aitoolcatalogue/{id}/exceptions
POST /api/v1/aitoolcatalogue/{id}/corrections
```
...
|---|---|
| `aitoolcatalogue_aggregate` | authoritative subject |
| `aitoolcatalogue_version` | immutable submitted, approved, issued, or signed versions |
| `aitoolcatalogue_relationship` | governed relationships |
| `aitoolcatalogue_decision` | binding Decisions and rationale |
| `aitoolcatalogue_evidence_link` | provenance and evidence relationships |
| `aitoolcatalogue_event_outbox` | transactional Event publication |
| `aitoolcatalogue_idempotency` | duplicate-write protection |
| `aitoolcatalogue_exception` | exception, claim, dispute, or hold |
| `aitoolcatalogue_correction` | correction and supersession lineage |
| `aitoolcatalogue_audit` | privileged action evidence |
