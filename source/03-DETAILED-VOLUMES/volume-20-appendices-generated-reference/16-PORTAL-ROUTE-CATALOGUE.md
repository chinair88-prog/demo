# Portal Route Catalogue

| Metadata | Value |
|---|---|
| Volume | `20` |
| Chapter ID | `V20-16` |
| Subject | `PortalRouteCatalogue` |
...
This chapter defines how GTOS shall **define the generation source, schema, owner, evidence level, update cadence, validation, publication, retention, and use of the Portal Route Catalogue**.
...
|---|---|
| `PortalRouteCatalogue` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PortalRouteCatalogueVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PortalRouteCatalogueDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PortalRouteCatalogueEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PortalRouteCatalogueException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
PortalRouteCatalogue {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreatePortalRouteCatalogue`
- `ValidatePortalRouteCatalogue`
- `ApprovePortalRouteCatalogue`
- `ActivatePortalRouteCatalogue`
- `SuspendPortalRouteCatalogue`
- `CorrectPortalRouteCatalogue`
- `RetirePortalRouteCatalogue`

Reference Events:

- `PortalRouteCatalogueCreated`
- `PortalRouteCatalogueValidated`
- `PortalRouteCatalogueApproved`
- `PortalRouteCatalogueActivated`
- `PortalRouteCatalogueSuspended`
- `PortalRouteCatalogueCorrected`
- `PortalRouteCatalogueRetired`

Reference queries:

- `GetPortalRouteCatalogue`
- `ListPortalRouteCatalogueRecords`
- `GetPortalRouteCatalogueTimeline`
- `GetPortalRouteCatalogueEvidence`
- `GetPortalRouteCatalogueDecisionPackage`
- `GetPortalRouteCatalogueExceptions`
- `GetPortalRouteCatalogueAuditTrail`

Reference API surface:

```text
POST /api/v1/portalroutecatalogue/commands
GET  /api/v1/portalroutecatalogue/{id}
GET  /api/v1/portalroutecatalogue/{id}/timeline
GET  /api/v1/portalroutecatalogue/{id}/evidence
GET  /api/v1/portalroutecatalogue/{id}/decisions
POST /api/v1/portalroutecatalogue/{id}/exceptions
POST /api/v1/portalroutecatalogue/{id}/corrections
```
...
|---|---|
| `portalroutecatalogue_aggregate` | authoritative subject |
| `portalroutecatalogue_version` | immutable submitted, approved, issued, or signed versions |
| `portalroutecatalogue_relationship` | governed relationships |
| `portalroutecatalogue_decision` | binding Decisions and rationale |
| `portalroutecatalogue_evidence_link` | provenance and evidence relationships |
| `portalroutecatalogue_event_outbox` | transactional Event publication |
| `portalroutecatalogue_idempotency` | duplicate-write protection |
| `portalroutecatalogue_exception` | exception, claim, dispute, or hold |
| `portalroutecatalogue_correction` | correction and supersession lineage |
| `portalroutecatalogue_audit` | privileged action evidence |
