...
This chapter defines how GTOS shall **define the generation source, schema, owner, evidence level, update cadence, validation, publication, retention, and use of the SLO Catalogue**.
...
|---|---|
| `SLOCatalogue` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SLOCatalogueVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SLOCatalogueDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SLOCatalogueEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SLOCatalogueException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `SLOCatalogueCreated`
- `SLOCatalogueValidated`
- `SLOCatalogueApproved`
- `SLOCatalogueActivated`
- `SLOCatalogueSuspended`
- `SLOCatalogueCorrected`
- `SLOCatalogueRetired`

Reference queries:

- `GetSLOCatalogue`
- `ListSLOCatalogueRecords`
- `GetSLOCatalogueTimeline`
- `GetSLOCatalogueEvidence`
- `GetSLOCatalogueDecisionPackage`
- `GetSLOCatalogueExceptions`
- `GetSLOCatalogueAuditTrail`

Reference API surface:

```text
POST /api/v1/slocatalogue/commands
GET  /api/v1/slocatalogue/{id}
GET  /api/v1/slocatalogue/{id}/timeline
GET  /api/v1/slocatalogue/{id}/evidence
GET  /api/v1/slocatalogue/{id}/decisions
POST /api/v1/slocatalogue/{id}/exceptions
POST /api/v1/slocatalogue/{id}/corrections
```
...
|---|---|
| `slocatalogue_aggregate` | authoritative subject |
| `slocatalogue_version` | immutable submitted, approved, issued, or signed versions |
| `slocatalogue_relationship` | governed relationships |
| `slocatalogue_decision` | binding Decisions and rationale |
| `slocatalogue_evidence_link` | provenance and evidence relationships |
| `slocatalogue_event_outbox` | transactional Event publication |
| `slocatalogue_idempotency` | duplicate-write protection |
| `slocatalogue_exception` | exception, claim, dispute, or hold |
| `slocatalogue_correction` | correction and supersession lineage |
| `slocatalogue_audit` | privileged action evidence |
