...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for API Implementation**.
...
|---|---|
| `APIImplementation` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `APIImplementationVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `APIImplementationDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `APIImplementationEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `APIImplementationException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ValidateAPIImplementation`
- `ApproveAPIImplementation`
- `ActivateAPIImplementation`
- `SuspendAPIImplementation`
- `CorrectAPIImplementation`
- `RetireAPIImplementation`

Reference Events:

- `APIImplementationCreated`
- `APIImplementationValidated`
- `APIImplementationApproved`
- `APIImplementationActivated`
- `APIImplementationSuspended`
- `APIImplementationCorrected`
- `APIImplementationRetired`

Reference queries:

- `GetAPIImplementation`
- `ListAPIImplementationRecords`
- `GetAPIImplementationTimeline`
- `GetAPIImplementationEvidence`
- `GetAPIImplementationDecisionPackage`
- `GetAPIImplementationExceptions`
- `GetAPIImplementationAuditTrail`

Reference API surface:

```text
POST /api/v1/apiimplementation/commands
GET  /api/v1/apiimplementation/{id}
GET  /api/v1/apiimplementation/{id}/timeline
GET  /api/v1/apiimplementation/{id}/evidence
GET  /api/v1/apiimplementation/{id}/decisions
POST /api/v1/apiimplementation/{id}/exceptions
POST /api/v1/apiimplementation/{id}/corrections
```
...
|---|---|
| `apiimplementation_aggregate` | authoritative subject |
| `apiimplementation_version` | immutable submitted, approved, issued, or signed versions |
| `apiimplementation_relationship` | governed relationships |
| `apiimplementation_decision` | binding Decisions and rationale |
| `apiimplementation_evidence_link` | provenance and evidence relationships |
| `apiimplementation_event_outbox` | transactional Event publication |
| `apiimplementation_idempotency` | duplicate-write protection |
| `apiimplementation_exception` | exception, claim, dispute, or hold |
| `apiimplementation_correction` | correction and supersession lineage |
| `apiimplementation_audit` | privileged action evidence |
...
- `API Implementation` is part of the target GTOS handbook scope.
