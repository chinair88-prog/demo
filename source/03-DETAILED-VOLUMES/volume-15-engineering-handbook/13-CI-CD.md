...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for CI CD**.
...
|---|---|
| `CICD` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CICDVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CICDDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CICDEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CICDException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
```text
POST /api/v1/cicd/commands
GET  /api/v1/cicd/{id}
GET  /api/v1/cicd/{id}/timeline
GET  /api/v1/cicd/{id}/evidence
GET  /api/v1/cicd/{id}/decisions
POST /api/v1/cicd/{id}/exceptions
POST /api/v1/cicd/{id}/corrections
```
...
|---|---|
| `cicd_aggregate` | authoritative subject |
| `cicd_version` | immutable submitted, approved, issued, or signed versions |
| `cicd_relationship` | governed relationships |
| `cicd_decision` | binding Decisions and rationale |
| `cicd_evidence_link` | provenance and evidence relationships |
| `cicd_event_outbox` | transactional Event publication |
| `cicd_idempotency` | duplicate-write protection |
| `cicd_exception` | exception, claim, dispute, or hold |
| `cicd_correction` | correction and supersession lineage |
| `cicd_audit` | privileged action evidence |
...
- `CI CD` is part of the target GTOS handbook scope.
