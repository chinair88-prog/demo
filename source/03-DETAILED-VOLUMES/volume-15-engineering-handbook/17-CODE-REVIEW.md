...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Code Review**.
...
|---|---|
| `CodeReview` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CodeReviewVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CodeReviewDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CodeReviewEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CodeReviewException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `GetCodeReview`
- `ListCodeReviewRecords`
- `GetCodeReviewTimeline`
- `GetCodeReviewEvidence`
- `GetCodeReviewDecisionPackage`
- `GetCodeReviewExceptions`
- `GetCodeReviewAuditTrail`

Reference API surface:

```text
POST /api/v1/codereview/commands
GET  /api/v1/codereview/{id}
GET  /api/v1/codereview/{id}/timeline
GET  /api/v1/codereview/{id}/evidence
GET  /api/v1/codereview/{id}/decisions
POST /api/v1/codereview/{id}/exceptions
POST /api/v1/codereview/{id}/corrections
```
...
|---|---|
| `codereview_aggregate` | authoritative subject |
| `codereview_version` | immutable submitted, approved, issued, or signed versions |
| `codereview_relationship` | governed relationships |
| `codereview_decision` | binding Decisions and rationale |
| `codereview_evidence_link` | provenance and evidence relationships |
| `codereview_event_outbox` | transactional Event publication |
| `codereview_idempotency` | duplicate-write protection |
| `codereview_exception` | exception, claim, dispute, or hold |
| `codereview_correction` | correction and supersession lineage |
| `codereview_audit` | privileged action evidence |
...
- `Code Review` is part of the target GTOS handbook scope.
