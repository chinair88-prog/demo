# AI Control Center Profile — Profile Scope, Actors, Capabilities, and Constraints

| Metadata | Value |
|---|---|
| Volume | `19` |
| Chapter ID | `V19-06-00` |
| Subject | `AIControlCenterProfile` |
...
This chapter defines how GTOS shall **define scope, actors, business capabilities, jurisdictions, non-goals, assumptions and constraints for AI Control Center Profile**.
...
|---|---|
| `AIControlCenterProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIControlCenterProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIControlCenterProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIControlCenterProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIControlCenterProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
AIControlCenterProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateAIControlCenterProfile`
- `ValidateAIControlCenterProfile`
- `ApproveAIControlCenterProfile`
- `ActivateAIControlCenterProfile`
- `SuspendAIControlCenterProfile`
- `CorrectAIControlCenterProfile`
- `RetireAIControlCenterProfile`

Reference Events:

- `AIControlCenterProfileCreated`
- `AIControlCenterProfileValidated`
- `AIControlCenterProfileApproved`
- `AIControlCenterProfileActivated`
- `AIControlCenterProfileSuspended`
- `AIControlCenterProfileCorrected`
- `AIControlCenterProfileRetired`

Reference queries:

- `GetAIControlCenterProfile`
- `ListAIControlCenterProfileRecords`
- `GetAIControlCenterProfileTimeline`
- `GetAIControlCenterProfileEvidence`
- `GetAIControlCenterProfileDecisionPackage`
- `GetAIControlCenterProfileExceptions`
- `GetAIControlCenterProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/aicontrolcenterprofile/commands
GET  /api/v1/aicontrolcenterprofile/{id}
GET  /api/v1/aicontrolcenterprofile/{id}/timeline
GET  /api/v1/aicontrolcenterprofile/{id}/evidence
GET  /api/v1/aicontrolcenterprofile/{id}/decisions
POST /api/v1/aicontrolcenterprofile/{id}/exceptions
POST /api/v1/aicontrolcenterprofile/{id}/corrections
```
...
|---|---|
| `aicontrolcenterprofile_aggregate` | authoritative subject |
| `aicontrolcenterprofile_version` | immutable submitted, approved, issued, or signed versions |
| `aicontrolcenterprofile_relationship` | governed relationships |
| `aicontrolcenterprofile_decision` | binding Decisions and rationale |
| `aicontrolcenterprofile_evidence_link` | provenance and evidence relationships |
| `aicontrolcenterprofile_event_outbox` | transactional Event publication |
| `aicontrolcenterprofile_idempotency` | duplicate-write protection |
| `aicontrolcenterprofile_exception` | exception, claim, dispute, or hold |
| `aicontrolcenterprofile_correction` | correction and supersession lineage |
| `aicontrolcenterprofile_audit` | privileged action evidence |
