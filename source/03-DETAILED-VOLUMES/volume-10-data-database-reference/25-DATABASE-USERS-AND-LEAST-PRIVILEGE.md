...
| Volume | `10` |
| Chapter ID | `V10-25` |
| Subject | `DatabaseUsersandLeastPrivilege` |
...
This chapter defines how GTOS shall **define enterprise and Domain data ownership, semantics, persistence, quality, lineage, correction, protection, operations, and verification for Database Users and Least Privilege**.
...
|---|---|
| `DatabaseUsersandLeastPrivilege` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DatabaseUsersandLeastPrivilegeVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DatabaseUsersandLeastPrivilegeDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DatabaseUsersandLeastPrivilegeEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DatabaseUsersandLeastPrivilegeException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
DatabaseUsersandLeastPrivilege {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateDatabaseUsersandLeastPrivilege`
- `ValidateDatabaseUsersandLeastPrivilege`
- `ApproveDatabaseUsersandLeastPrivilege`
- `ActivateDatabaseUsersandLeastPrivilege`
- `SuspendDatabaseUsersandLeastPrivilege`
- `CorrectDatabaseUsersandLeastPrivilege`
- `RetireDatabaseUsersandLeastPrivilege`

Reference Events:

- `DatabaseUsersandLeastPrivilegeCreated`
- `DatabaseUsersandLeastPrivilegeValidated`
- `DatabaseUsersandLeastPrivilegeApproved`
- `DatabaseUsersandLeastPrivilegeActivated`
- `DatabaseUsersandLeastPrivilegeSuspended`
- `DatabaseUsersandLeastPrivilegeCorrected`
- `DatabaseUsersandLeastPrivilegeRetired`

Reference queries:

- `GetDatabaseUsersandLeastPrivilege`
- `ListDatabaseUsersandLeastPrivilegeRecords`
- `GetDatabaseUsersandLeastPrivilegeTimeline`
- `GetDatabaseUsersandLeastPrivilegeEvidence`
- `GetDatabaseUsersandLeastPrivilegeDecisionPackage`
- `GetDatabaseUsersandLeastPrivilegeExceptions`
- `GetDatabaseUsersandLeastPrivilegeAuditTrail`

Reference API surface:

```text
POST /api/v1/databaseusersandleastprivilege/commands
GET  /api/v1/databaseusersandleastprivilege/{id}
GET  /api/v1/databaseusersandleastprivilege/{id}/timeline
GET  /api/v1/databaseusersandleastprivilege/{id}/evidence
GET  /api/v1/databaseusersandleastprivilege/{id}/decisions
POST /api/v1/databaseusersandleastprivilege/{id}/exceptions
POST /api/v1/databaseusersandleastprivilege/{id}/corrections
```
...
|---|---|
| `databaseusersandleastprivilege_aggregate` | authoritative subject |
| `databaseusersandleastprivilege_version` | immutable submitted, approved, issued, or signed versions |
| `databaseusersandleastprivilege_relationship` | governed relationships |
| `databaseusersandleastprivilege_decision` | binding Decisions and rationale |
| `databaseusersandleastprivilege_evidence_link` | provenance and evidence relationships |
| `databaseusersandleastprivilege_event_outbox` | transactional Event publication |
| `databaseusersandleastprivilege_idempotency` | duplicate-write protection |
| `databaseusersandleastprivilege_exception` | exception, claim, dispute, or hold |
| `databaseusersandleastprivilege_correction` | correction and supersession lineage |
| `databaseusersandleastprivilege_audit` | privileged action evidence |
