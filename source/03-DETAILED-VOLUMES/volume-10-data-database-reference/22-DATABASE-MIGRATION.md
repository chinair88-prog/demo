...
This chapter defines how GTOS shall **define enterprise and Domain data ownership, semantics, persistence, quality, lineage, correction, protection, operations, and verification for Database Migration**.
...
|---|---|
| `DatabaseMigration` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DatabaseMigrationVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DatabaseMigrationDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DatabaseMigrationEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DatabaseMigrationException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveDatabaseMigration`
- `ActivateDatabaseMigration`
- `SuspendDatabaseMigration`
- `CorrectDatabaseMigration`
- `RetireDatabaseMigration`

Reference Events:

- `DatabaseMigrationCreated`
- `DatabaseMigrationValidated`
- `DatabaseMigrationApproved`
- `DatabaseMigrationActivated`
- `DatabaseMigrationSuspended`
- `DatabaseMigrationCorrected`
- `DatabaseMigrationRetired`

Reference queries:

- `GetDatabaseMigration`
- `ListDatabaseMigrationRecords`
- `GetDatabaseMigrationTimeline`
- `GetDatabaseMigrationEvidence`
- `GetDatabaseMigrationDecisionPackage`
- `GetDatabaseMigrationExceptions`
- `GetDatabaseMigrationAuditTrail`

Reference API surface:

```text
POST /api/v1/databasemigration/commands
GET  /api/v1/databasemigration/{id}
GET  /api/v1/databasemigration/{id}/timeline
GET  /api/v1/databasemigration/{id}/evidence
GET  /api/v1/databasemigration/{id}/decisions
POST /api/v1/databasemigration/{id}/exceptions
POST /api/v1/databasemigration/{id}/corrections
```
...
|---|---|
| `databasemigration_aggregate` | authoritative subject |
| `databasemigration_version` | immutable submitted, approved, issued, or signed versions |
| `databasemigration_relationship` | governed relationships |
| `databasemigration_decision` | binding Decisions and rationale |
| `databasemigration_evidence_link` | provenance and evidence relationships |
| `databasemigration_event_outbox` | transactional Event publication |
| `databasemigration_idempotency` | duplicate-write protection |
| `databasemigration_exception` | exception, claim, dispute, or hold |
| `databasemigration_correction` | correction and supersession lineage |
| `databasemigration_audit` | privileged action evidence |
