...
| Volume | `10` |
| Chapter ID | `V10-03` |
| Subject | `Database-per-ServicePolicy` |
...
This chapter defines how GTOS shall **define enterprise and Domain data ownership, semantics, persistence, quality, lineage, correction, protection, operations, and verification for Database-per-Service Policy**.
...
|---|---|
| `Database-per-ServicePolicy` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `Database-per-ServicePolicyVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `Database-per-ServicePolicyDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `Database-per-ServicePolicyEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `Database-per-ServicePolicyException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
Database-per-ServicePolicy {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateDatabase-per-ServicePolicy`
- `ValidateDatabase-per-ServicePolicy`
- `ApproveDatabase-per-ServicePolicy`
- `ActivateDatabase-per-ServicePolicy`
- `SuspendDatabase-per-ServicePolicy`
- `CorrectDatabase-per-ServicePolicy`
- `RetireDatabase-per-ServicePolicy`

Reference Events:

- `Database-per-ServicePolicyCreated`
- `Database-per-ServicePolicyValidated`
- `Database-per-ServicePolicyApproved`
- `Database-per-ServicePolicyActivated`
- `Database-per-ServicePolicySuspended`
- `Database-per-ServicePolicyCorrected`
- `Database-per-ServicePolicyRetired`

Reference queries:

- `GetDatabase-per-ServicePolicy`
- `ListDatabase-per-ServicePolicyRecords`
- `GetDatabase-per-ServicePolicyTimeline`
- `GetDatabase-per-ServicePolicyEvidence`
- `GetDatabase-per-ServicePolicyDecisionPackage`
- `GetDatabase-per-ServicePolicyExceptions`
- `GetDatabase-per-ServicePolicyAuditTrail`

Reference API surface:

```text
POST /api/v1/database_per_servicepolicy/commands
GET  /api/v1/database_per_servicepolicy/{id}
GET  /api/v1/database_per_servicepolicy/{id}/timeline
GET  /api/v1/database_per_servicepolicy/{id}/evidence
GET  /api/v1/database_per_servicepolicy/{id}/decisions
POST /api/v1/database_per_servicepolicy/{id}/exceptions
POST /api/v1/database_per_servicepolicy/{id}/corrections
```
...
|---|---|
| `database_per_servicepolicy_aggregate` | authoritative subject |
| `database_per_servicepolicy_version` | immutable submitted, approved, issued, or signed versions |
| `database_per_servicepolicy_relationship` | governed relationships |
| `database_per_servicepolicy_decision` | binding Decisions and rationale |
| `database_per_servicepolicy_evidence_link` | provenance and evidence relationships |
| `database_per_servicepolicy_event_outbox` | transactional Event publication |
| `database_per_servicepolicy_idempotency` | duplicate-write protection |
| `database_per_servicepolicy_exception` | exception, claim, dispute, or hold |
| `database_per_servicepolicy_correction` | correction and supersession lineage |
| `database_per_servicepolicy_audit` | privileged action evidence |
