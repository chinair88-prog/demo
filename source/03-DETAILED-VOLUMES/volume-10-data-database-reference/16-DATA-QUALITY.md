...
This chapter defines how GTOS shall **define enterprise and Domain data ownership, semantics, persistence, quality, lineage, correction, protection, operations, and verification for Data Quality**.
...
|---|---|
| `DataQuality` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DataQualityVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DataQualityDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DataQualityEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DataQualityException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `DataQualityApproved`
- `DataQualityActivated`
- `DataQualitySuspended`
- `DataQualityCorrected`
- `DataQualityRetired`
...
- `GetDataQualityTimeline`
- `GetDataQualityEvidence`
- `GetDataQualityDecisionPackage`
...
```text
POST /api/v1/dataquality/commands
GET  /api/v1/dataquality/{id}
GET  /api/v1/dataquality/{id}/timeline
GET  /api/v1/dataquality/{id}/evidence
GET  /api/v1/dataquality/{id}/decisions
POST /api/v1/dataquality/{id}/exceptions
POST /api/v1/dataquality/{id}/corrections
```
...
|---|---|
| `dataquality_aggregate` | authoritative subject |
| `dataquality_version` | immutable submitted, approved, issued, or signed versions |
| `dataquality_relationship` | governed relationships |
| `dataquality_decision` | binding Decisions and rationale |
| `dataquality_evidence_link` | provenance and evidence relationships |
| `dataquality_event_outbox` | transactional Event publication |
| `dataquality_idempotency` | duplicate-write protection |
| `dataquality_exception` | exception, claim, dispute, or hold |
| `dataquality_correction` | correction and supersession lineage |
| `dataquality_audit` | privileged action evidence |
