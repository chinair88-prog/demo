...
This chapter defines how GTOS shall **define enterprise and Domain data ownership, semantics, persistence, quality, lineage, correction, protection, operations, and verification for Backup and Restore**.
...
|---|---|
| `BackupandRestore` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `BackupandRestoreVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `BackupandRestoreDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `BackupandRestoreEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `BackupandRestoreException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveBackupandRestore`
- `ActivateBackupandRestore`
- `SuspendBackupandRestore`
- `CorrectBackupandRestore`
- `RetireBackupandRestore`

Reference Events:

- `BackupandRestoreCreated`
- `BackupandRestoreValidated`
- `BackupandRestoreApproved`
- `BackupandRestoreActivated`
- `BackupandRestoreSuspended`
- `BackupandRestoreCorrected`
- `BackupandRestoreRetired`

Reference queries:

- `GetBackupandRestore`
- `ListBackupandRestoreRecords`
- `GetBackupandRestoreTimeline`
- `GetBackupandRestoreEvidence`
- `GetBackupandRestoreDecisionPackage`
- `GetBackupandRestoreExceptions`
- `GetBackupandRestoreAuditTrail`

Reference API surface:

```text
POST /api/v1/backupandrestore/commands
GET  /api/v1/backupandrestore/{id}
GET  /api/v1/backupandrestore/{id}/timeline
GET  /api/v1/backupandrestore/{id}/evidence
GET  /api/v1/backupandrestore/{id}/decisions
POST /api/v1/backupandrestore/{id}/exceptions
POST /api/v1/backupandrestore/{id}/corrections
```
...
|---|---|
| `backupandrestore_aggregate` | authoritative subject |
| `backupandrestore_version` | immutable submitted, approved, issued, or signed versions |
| `backupandrestore_relationship` | governed relationships |
| `backupandrestore_decision` | binding Decisions and rationale |
| `backupandrestore_evidence_link` | provenance and evidence relationships |
| `backupandrestore_event_outbox` | transactional Event publication |
| `backupandrestore_idempotency` | duplicate-write protection |
| `backupandrestore_exception` | exception, claim, dispute, or hold |
| `backupandrestore_correction` | correction and supersession lineage |
| `backupandrestore_audit` | privileged action evidence |
