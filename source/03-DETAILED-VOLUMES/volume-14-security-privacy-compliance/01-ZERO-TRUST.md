...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Zero Trust**.
...
|---|---|
| `ZeroTrust` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ZeroTrustVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ZeroTrustDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ZeroTrustEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ZeroTrustException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `GetZeroTrust`
- `ListZeroTrustRecords`
- `GetZeroTrustTimeline`
- `GetZeroTrustEvidence`
- `GetZeroTrustDecisionPackage`
- `GetZeroTrustExceptions`
- `GetZeroTrustAuditTrail`

Reference API surface:

```text
POST /api/v1/zerotrust/commands
GET  /api/v1/zerotrust/{id}
GET  /api/v1/zerotrust/{id}/timeline
GET  /api/v1/zerotrust/{id}/evidence
GET  /api/v1/zerotrust/{id}/decisions
POST /api/v1/zerotrust/{id}/exceptions
POST /api/v1/zerotrust/{id}/corrections
```
...
|---|---|
| `zerotrust_aggregate` | authoritative subject |
| `zerotrust_version` | immutable submitted, approved, issued, or signed versions |
| `zerotrust_relationship` | governed relationships |
| `zerotrust_decision` | binding Decisions and rationale |
| `zerotrust_evidence_link` | provenance and evidence relationships |
| `zerotrust_event_outbox` | transactional Event publication |
| `zerotrust_idempotency` | duplicate-write protection |
| `zerotrust_exception` | exception, claim, dispute, or hold |
| `zerotrust_correction` | correction and supersession lineage |
| `zerotrust_audit` | privileged action evidence |
...
- `Zero Trust` is part of the target GTOS handbook scope.
