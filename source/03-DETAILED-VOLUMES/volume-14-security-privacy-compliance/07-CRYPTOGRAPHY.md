...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Cryptography**.
...
|---|---|
| `Cryptography` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CryptographyVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CryptographyDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CryptographyEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CryptographyException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CryptographyCreated`
- `CryptographyValidated`
- `CryptographyApproved`
- `CryptographyActivated`
- `CryptographySuspended`
- `CryptographyCorrected`
- `CryptographyRetired`

Reference queries:

- `GetCryptography`
- `ListCryptographyRecords`
- `GetCryptographyTimeline`
- `GetCryptographyEvidence`
- `GetCryptographyDecisionPackage`
- `GetCryptographyExceptions`
- `GetCryptographyAuditTrail`

Reference API surface:

```text
POST /api/v1/cryptography/commands
GET  /api/v1/cryptography/{id}
GET  /api/v1/cryptography/{id}/timeline
GET  /api/v1/cryptography/{id}/evidence
GET  /api/v1/cryptography/{id}/decisions
POST /api/v1/cryptography/{id}/exceptions
POST /api/v1/cryptography/{id}/corrections
```
...
|---|---|
| `cryptography_aggregate` | authoritative subject |
| `cryptography_version` | immutable submitted, approved, issued, or signed versions |
| `cryptography_relationship` | governed relationships |
| `cryptography_decision` | binding Decisions and rationale |
| `cryptography_evidence_link` | provenance and evidence relationships |
| `cryptography_event_outbox` | transactional Event publication |
| `cryptography_idempotency` | duplicate-write protection |
| `cryptography_exception` | exception, claim, dispute, or hold |
| `cryptography_correction` | correction and supersession lineage |
| `cryptography_audit` | privileged action evidence |
...
- `Cryptography` is part of the target GTOS handbook scope.
