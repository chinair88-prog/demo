...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Workflow Security**.
...
|---|---|
| `WorkflowSecurity` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WorkflowSecurityVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WorkflowSecurityDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WorkflowSecurityEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WorkflowSecurityException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveWorkflowSecurity`
- `ActivateWorkflowSecurity`
- `SuspendWorkflowSecurity`
- `CorrectWorkflowSecurity`
- `RetireWorkflowSecurity`

Reference Events:

- `WorkflowSecurityCreated`
- `WorkflowSecurityValidated`
- `WorkflowSecurityApproved`
- `WorkflowSecurityActivated`
- `WorkflowSecuritySuspended`
- `WorkflowSecurityCorrected`
- `WorkflowSecurityRetired`

Reference queries:

- `GetWorkflowSecurity`
- `ListWorkflowSecurityRecords`
- `GetWorkflowSecurityTimeline`
- `GetWorkflowSecurityEvidence`
- `GetWorkflowSecurityDecisionPackage`
- `GetWorkflowSecurityExceptions`
- `GetWorkflowSecurityAuditTrail`

Reference API surface:

```text
POST /api/v1/workflowsecurity/commands
GET  /api/v1/workflowsecurity/{id}
GET  /api/v1/workflowsecurity/{id}/timeline
GET  /api/v1/workflowsecurity/{id}/evidence
GET  /api/v1/workflowsecurity/{id}/decisions
POST /api/v1/workflowsecurity/{id}/exceptions
POST /api/v1/workflowsecurity/{id}/corrections
```
...
|---|---|
| `workflowsecurity_aggregate` | authoritative subject |
| `workflowsecurity_version` | immutable submitted, approved, issued, or signed versions |
| `workflowsecurity_relationship` | governed relationships |
| `workflowsecurity_decision` | binding Decisions and rationale |
| `workflowsecurity_evidence_link` | provenance and evidence relationships |
| `workflowsecurity_event_outbox` | transactional Event publication |
| `workflowsecurity_idempotency` | duplicate-write protection |
| `workflowsecurity_exception` | exception, claim, dispute, or hold |
| `workflowsecurity_correction` | correction and supersession lineage |
| `workflowsecurity_audit` | privileged action evidence |
...
- `Workflow Security` is part of the target GTOS handbook scope.
