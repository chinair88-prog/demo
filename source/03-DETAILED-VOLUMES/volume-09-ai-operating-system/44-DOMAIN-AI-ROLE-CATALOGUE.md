# Domain AI Role Catalogue

| Metadata | Value |
|---|---|
| Volume | `9` |
| Chapter ID | `V09-44` |
| Subject | `DomainAIRoleCatalogue` |
...
This chapter defines how GTOS shall **define the ownership, lifecycle, policy, evidence, Tools, security, evaluation, failure, recovery, and repository verification requirements for Domain AI Role Catalogue**.
...
|---|---|
| `DomainAIRoleCatalogue` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DomainAIRoleCatalogueVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DomainAIRoleCatalogueDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DomainAIRoleCatalogueEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DomainAIRoleCatalogueException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
DomainAIRoleCatalogue {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateDomainAIRoleCatalogue`
- `ValidateDomainAIRoleCatalogue`
- `ApproveDomainAIRoleCatalogue`
- `ActivateDomainAIRoleCatalogue`
- `SuspendDomainAIRoleCatalogue`
- `CorrectDomainAIRoleCatalogue`
- `RetireDomainAIRoleCatalogue`

Reference Events:

- `DomainAIRoleCatalogueCreated`
- `DomainAIRoleCatalogueValidated`
- `DomainAIRoleCatalogueApproved`
- `DomainAIRoleCatalogueActivated`
- `DomainAIRoleCatalogueSuspended`
- `DomainAIRoleCatalogueCorrected`
- `DomainAIRoleCatalogueRetired`

Reference queries:

- `GetDomainAIRoleCatalogue`
- `ListDomainAIRoleCatalogueRecords`
- `GetDomainAIRoleCatalogueTimeline`
- `GetDomainAIRoleCatalogueEvidence`
- `GetDomainAIRoleCatalogueDecisionPackage`
- `GetDomainAIRoleCatalogueExceptions`
- `GetDomainAIRoleCatalogueAuditTrail`

Reference API surface:

```text
POST /api/v1/domainairolecatalogue/commands
GET  /api/v1/domainairolecatalogue/{id}
GET  /api/v1/domainairolecatalogue/{id}/timeline
GET  /api/v1/domainairolecatalogue/{id}/evidence
GET  /api/v1/domainairolecatalogue/{id}/decisions
POST /api/v1/domainairolecatalogue/{id}/exceptions
POST /api/v1/domainairolecatalogue/{id}/corrections
```
...
|---|---|
| `domainairolecatalogue_aggregate` | authoritative subject |
| `domainairolecatalogue_version` | immutable submitted, approved, issued, or signed versions |
| `domainairolecatalogue_relationship` | governed relationships |
| `domainairolecatalogue_decision` | binding Decisions and rationale |
| `domainairolecatalogue_evidence_link` | provenance and evidence relationships |
| `domainairolecatalogue_event_outbox` | transactional Event publication |
| `domainairolecatalogue_idempotency` | duplicate-write protection |
| `domainairolecatalogue_exception` | exception, claim, dispute, or hold |
| `domainairolecatalogue_correction` | correction and supersession lineage |
| `domainairolecatalogue_audit` | privileged action evidence |
