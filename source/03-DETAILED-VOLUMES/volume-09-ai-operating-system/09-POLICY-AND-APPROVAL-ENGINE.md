...
| Volume | `9` |
| Chapter ID | `V09-09` |
| Subject | `PolicyandApprovalEngine` |
...
This chapter defines how GTOS shall **define the ownership, lifecycle, policy, evidence, Tools, security, evaluation, failure, recovery, and repository verification requirements for Policy and Approval Engine**.
...
|---|---|
| `PolicyandApprovalEngine` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PolicyandApprovalEngineVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PolicyandApprovalEngineDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PolicyandApprovalEngineEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PolicyandApprovalEngineException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
PolicyandApprovalEngine {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreatePolicyandApprovalEngine`
- `ValidatePolicyandApprovalEngine`
- `ApprovePolicyandApprovalEngine`
- `ActivatePolicyandApprovalEngine`
- `SuspendPolicyandApprovalEngine`
- `CorrectPolicyandApprovalEngine`
- `RetirePolicyandApprovalEngine`

Reference Events:

- `PolicyandApprovalEngineCreated`
- `PolicyandApprovalEngineValidated`
- `PolicyandApprovalEngineApproved`
- `PolicyandApprovalEngineActivated`
- `PolicyandApprovalEngineSuspended`
- `PolicyandApprovalEngineCorrected`
- `PolicyandApprovalEngineRetired`

Reference queries:

- `GetPolicyandApprovalEngine`
- `ListPolicyandApprovalEngineRecords`
- `GetPolicyandApprovalEngineTimeline`
- `GetPolicyandApprovalEngineEvidence`
- `GetPolicyandApprovalEngineDecisionPackage`
- `GetPolicyandApprovalEngineExceptions`
- `GetPolicyandApprovalEngineAuditTrail`

Reference API surface:

```text
POST /api/v1/policyandapprovalengine/commands
GET  /api/v1/policyandapprovalengine/{id}
GET  /api/v1/policyandapprovalengine/{id}/timeline
GET  /api/v1/policyandapprovalengine/{id}/evidence
GET  /api/v1/policyandapprovalengine/{id}/decisions
POST /api/v1/policyandapprovalengine/{id}/exceptions
POST /api/v1/policyandapprovalengine/{id}/corrections
```
...
|---|---|
| `policyandapprovalengine_aggregate` | authoritative subject |
| `policyandapprovalengine_version` | immutable submitted, approved, issued, or signed versions |
| `policyandapprovalengine_relationship` | governed relationships |
| `policyandapprovalengine_decision` | binding Decisions and rationale |
| `policyandapprovalengine_evidence_link` | provenance and evidence relationships |
| `policyandapprovalengine_event_outbox` | transactional Event publication |
| `policyandapprovalengine_idempotency` | duplicate-write protection |
| `policyandapprovalengine_exception` | exception, claim, dispute, or hold |
| `policyandapprovalengine_correction` | correction and supersession lineage |
| `policyandapprovalengine_audit` | privileged action evidence |
