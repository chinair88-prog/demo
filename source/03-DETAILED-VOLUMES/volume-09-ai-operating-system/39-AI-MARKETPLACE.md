...
This chapter defines how GTOS shall **define the ownership, lifecycle, policy, evidence, Tools, security, evaluation, failure, recovery, and repository verification requirements for AI Marketplace**.
...
|---|---|
| `AIMarketplace` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIMarketplaceVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIMarketplaceDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIMarketplaceEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIMarketplaceException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreateAIMarketplace`
- `ValidateAIMarketplace`
- `ApproveAIMarketplace`
- `ActivateAIMarketplace`
- `SuspendAIMarketplace`
- `CorrectAIMarketplace`
- `RetireAIMarketplace`

Reference Events:

- `AIMarketplaceCreated`
- `AIMarketplaceValidated`
- `AIMarketplaceApproved`
- `AIMarketplaceActivated`
- `AIMarketplaceSuspended`
- `AIMarketplaceCorrected`
- `AIMarketplaceRetired`

Reference queries:

- `GetAIMarketplace`
- `ListAIMarketplaceRecords`
- `GetAIMarketplaceTimeline`
- `GetAIMarketplaceEvidence`
- `GetAIMarketplaceDecisionPackage`
- `GetAIMarketplaceExceptions`
- `GetAIMarketplaceAuditTrail`

Reference API surface:

```text
POST /api/v1/aimarketplace/commands
GET  /api/v1/aimarketplace/{id}
GET  /api/v1/aimarketplace/{id}/timeline
GET  /api/v1/aimarketplace/{id}/evidence
GET  /api/v1/aimarketplace/{id}/decisions
POST /api/v1/aimarketplace/{id}/exceptions
POST /api/v1/aimarketplace/{id}/corrections
```
...
|---|---|
| `aimarketplace_aggregate` | authoritative subject |
| `aimarketplace_version` | immutable submitted, approved, issued, or signed versions |
| `aimarketplace_relationship` | governed relationships |
| `aimarketplace_decision` | binding Decisions and rationale |
| `aimarketplace_evidence_link` | provenance and evidence relationships |
| `aimarketplace_event_outbox` | transactional Event publication |
| `aimarketplace_idempotency` | duplicate-write protection |
| `aimarketplace_exception` | exception, claim, dispute, or hold |
| `aimarketplace_correction` | correction and supersession lineage |
| `aimarketplace_audit` | privileged action evidence |
