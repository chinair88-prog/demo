# Publication Chapter 19 — Pickup, Custody Transfer, and Handover

| Metadata | Value |
|---|---|
| Publication Chapter | `19` |
| Canonical Lifecycle Stage | `18` |
...
| Accountable capability | Shipping / Provider / handing and receiving parties |
...
This stage exists to **evidence physical handover of identified cargo and establish custody without conflating custody, possession, risk, and ownership**.
...
The accountable capability is **Shipping / Provider / handing and receiving parties**. Supporting Portals, workflow engines, reports, search indexes, integration adapters, and AI Agents may coordinate or present the work, but they do not silently become owners of the underlying business truth.
...
|---|---|
| `PickupAppointment` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PickupInstruction` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CargoHandover` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CustodyRelationship` | independent identified subject or governed subordinate entity; ownership must be declared |
| `SealRecord` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PackageCountObservation` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PickupDiscrepancy` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ProofOfPickup` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| `IDENTITY_CHECK` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `LOADING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `DISCREPANCY` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `HANDED_OVER` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `REFUSED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `FAILED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `CORRECTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    [*] --> SCHEDULED
    SCHEDULED --> ARRIVED
    ARRIVED --> IDENTITY_CHECK
    IDENTITY_CHECK --> LOADING
    LOADING --> DISCREPANCY
    DISCREPANCY --> HANDED_OVER
    HANDED_OVER --> REFUSED
    REFUSED --> FAILED
    FAILED --> CORRECTED
    CORRECTED --> CLOSED
...
| `ARRIVED` | `IDENTITY_CHECK` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `IDENTITY_CHECK` | `LOADING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `LOADING` | `DISCREPANCY` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `DISCREPANCY` | `HANDED_OVER` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `HANDED_OVER` | `REFUSED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REFUSED` | `FAILED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `FAILED` | `CORRECTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CORRECTED` | `CLOSED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `PickupScheduled`
- `PickupVehicleArrived`
- `CargoIdentityVerified`
- `PickupDiscrepancyObserved`
- `CustodyTransferred`
- `PickupRefused`
- `ProofOfPickupIssued`
...
- `ListPickupAppointmentRecords`
- `GetPickupAppointmentTimeline`
- `GetPickupAppointmentEvidence`
- `GetPickupAppointmentDecisionPackage`
- `GetPickupAppointmentExceptions`
- `GetPickupAppointmentAuditTrail`
- `GetPickupAppointmentCurrentPerspective`
...
```text
POST /api/v1/pickup_custody_and_handover/commands
GET  /api/v1/pickup_custody_and_handover/{id}
GET  /api/v1/pickup_custody_and_handover/{id}/timeline
GET  /api/v1/pickup_custody_and_handover/{id}/evidence
GET  /api/v1/pickup_custody_and_handover/{id}/decisions
POST /api/v1/pickup_custody_and_handover/{id}/exceptions
POST /api/v1/pickup_custody_and_handover/{id}/corrections
```
...
- read package and seal identifiers
- detect count or weight anomalies
- summarize discrepancy
- assist pickup fraud detection
...
- Shipping, provider, warehouse, and Event Bus capabilities are registered.
- A canonical custody aggregate is REFERENCE_ARCHITECTURE until verified.
