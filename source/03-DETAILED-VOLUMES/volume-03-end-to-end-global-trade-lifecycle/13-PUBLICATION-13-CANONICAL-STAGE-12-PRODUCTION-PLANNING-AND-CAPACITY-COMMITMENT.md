# Publication Chapter 13 — Production Planning and Capacity Commitment

| Metadata | Value |
|---|---|
| Publication Chapter | `13` |
| Canonical Lifecycle Stage | `12` |
...
| Accountable capability | Manufacturing / Supplier Factory |
...
This stage exists to **translate an acknowledged commitment into a feasible, approved, versioned production plan with capacity, material, lot, milestone, and quality dependencies**.
...
The accountable capability is **Manufacturing / Supplier Factory**. Supporting Portals, workflow engines, reports, search indexes, integration adapters, and AI Agents may coordinate or present the work, but they do not silently become owners of the underlying business truth.
...
|---|---|
| `ProductionPlan` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ProductionPlanVersion` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ProductionOrder` | independent identified subject or governed subordinate entity; ownership must be declared |
| `BatchPlan` | independent identified subject or governed subordinate entity; ownership must be declared |
| `MaterialRequirement` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CapacityCommitment` | independent identified subject or governed subordinate entity; ownership must be declared |
| `MilestoneBaseline` | independent identified subject or governed subordinate entity; ownership must be declared |
| `FactoryAssignment` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| State | Meaning |
|---|---|
| `PROPOSED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `FEASIBILITY_CHECK` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `MATERIAL_CHECK` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `CAPACITY_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
| `BASELINED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `REBASELINE_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `SUSPENDED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `CANCELLED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `COMPLETED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    [*] --> PROPOSED
    PROPOSED --> FEASIBILITY_CHECK
    FEASIBILITY_CHECK --> MATERIAL_CHECK
    MATERIAL_CHECK --> CAPACITY_PENDING
    CAPACITY_PENDING --> APPROVAL_PENDING
    APPROVAL_PENDING --> APPROVED
    APPROVED --> BASELINED
    BASELINED --> REBASELINE_PENDING
    REBASELINE_PENDING --> SUSPENDED
    SUSPENDED --> CANCELLED
    CANCELLED --> COMPLETED
```
...
|---|---|---|
| `PROPOSED` | `FEASIBILITY_CHECK` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `FEASIBILITY_CHECK` | `MATERIAL_CHECK` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `MATERIAL_CHECK` | `CAPACITY_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CAPACITY_PENDING` | `APPROVAL_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `APPROVAL_PENDING` | `APPROVED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `APPROVED` | `BASELINED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `BASELINED` | `REBASELINE_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REBASELINE_PENDING` | `SUSPENDED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `SUSPENDED` | `CANCELLED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CANCELLED` | `COMPLETED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `BaselineMilestones`
- `RebaselineProductionPlan`
- `SuspendProductionPlan`
...
- `ProductionFeasibilityAssessed`
- `CapacityCommitted`
- `ProductionPlanApproved`
- `ProductionPlanBaselined`
- `ProductionPlanRebaselined`
- `ProductionPlanSuspended`
...
- `GetProductionPlan`
- `ListProductionPlanRecords`
- `GetProductionPlanTimeline`
- `GetProductionPlanEvidence`
- `GetProductionPlanDecisionPackage`
- `GetProductionPlanExceptions`
- `GetProductionPlanAuditTrail`
- `GetProductionPlanCurrentPerspective`
...
```text
POST /api/v1/production_planning_and_capacity_commitment/commands
GET  /api/v1/production_planning_and_capacity_commitment/{id}
GET  /api/v1/production_planning_and_capacity_commitment/{id}/timeline
GET  /api/v1/production_planning_and_capacity_commitment/{id}/evidence
GET  /api/v1/production_planning_and_capacity_commitment/{id}/decisions
POST /api/v1/production_planning_and_capacity_commitment/{id}/exceptions
POST /api/v1/production_planning_and_capacity_commitment/{id}/corrections
```
...
- score capacity risk
- compare alternative production plans
...
- Manufacturing, supplier/provider, workflow, and event capabilities are registered.
- Detailed production-plan and capacity-commitment behavior is not treated as implemented without direct source evidence.
