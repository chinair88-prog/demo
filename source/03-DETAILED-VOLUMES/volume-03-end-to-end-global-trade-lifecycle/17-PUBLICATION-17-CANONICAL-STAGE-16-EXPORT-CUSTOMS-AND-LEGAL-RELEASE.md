# Publication Chapter 17 — Export Customs and Legal Release

| Metadata | Value |
|---|---|
| Publication Chapter | `17` |
| Canonical Lifecycle Stage | `16` |
...
This stage exists to **prepare, submit, amend, and track export declarations and authoritative legal-release outcomes without treating platform readiness as customs approval**.
...
|---|---|
| `ExportDeclaration` | independent identified subject or governed subordinate entity; ownership must be declared |
| `DeclarationVersion` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CustomsDocumentSet` | independent identified subject or governed subordinate entity; ownership must be declared |
| `BrokerMandate` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CustomsSubmission` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CustomsHold` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CustomsInspection` | independent identified subject or governed subordinate entity; ownership must be declared |
| `LegalReleaseObservation` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| State | Meaning |
|---|---|
| `PREPARATION` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
| `READY_TO_SUBMIT` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `SUBMITTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `ACCEPTED_FOR_PROCESSING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `HOLD` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `INSPECTION` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `AMENDMENT_REQUIRED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    [*] --> PREPARATION
    PREPARATION --> VALIDATION_PENDING
    VALIDATION_PENDING --> READY_TO_SUBMIT
    READY_TO_SUBMIT --> SUBMITTED
    SUBMITTED --> ACCEPTED_FOR_PROCESSING
    ACCEPTED_FOR_PROCESSING --> HOLD
    HOLD --> INSPECTION
...
|---|---|---|
| `PREPARATION` | `VALIDATION_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `VALIDATION_PENDING` | `READY_TO_SUBMIT` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `READY_TO_SUBMIT` | `SUBMITTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `SUBMITTED` | `ACCEPTED_FOR_PROCESSING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `ACCEPTED_FOR_PROCESSING` | `HOLD` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `HOLD` | `INSPECTION` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `INSPECTION` | `AMENDMENT_REQUIRED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `AMENDMENT_REQUIRED` | `RELEASED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `RELEASED` | `REJECTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REJECTED` | `CANCELLED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `RecordCustomsResponse`
- `AmendExportDeclaration`
- `RecordCustomsHold`
- `RecordExportRelease`
...
- `ExportDeclarationPrepared`
- `ExportDeclarationSubmitted`
- `CustomsHoldRecorded`
- `CustomsInspectionRequested`
- `ExportDeclarationAmended`
- `ExportReleased`
- `ExportRejected`
...
- `ListExportDeclarationRecords`
- `GetExportDeclarationTimeline`
- `GetExportDeclarationEvidence`
- `GetExportDeclarationDecisionPackage`
- `GetExportDeclarationExceptions`
- `GetExportDeclarationAuditTrail`
- `GetExportDeclarationCurrentPerspective`
...
```text
POST /api/v1/export_customs_and_legal_release/commands
GET  /api/v1/export_customs_and_legal_release/{id}
GET  /api/v1/export_customs_and_legal_release/{id}/timeline
GET  /api/v1/export_customs_and_legal_release/{id}/evidence
GET  /api/v1/export_customs_and_legal_release/{id}/decisions
POST /api/v1/export_customs_and_legal_release/{id}/exceptions
POST /api/v1/export_customs_and_legal_release/{id}/corrections
```
...
- Regulatory, provider, and shipping capabilities are registered.
- The external customs authority owns legal release; platform records are qualified observations.
- Adapters and route behavior require repository verification.
