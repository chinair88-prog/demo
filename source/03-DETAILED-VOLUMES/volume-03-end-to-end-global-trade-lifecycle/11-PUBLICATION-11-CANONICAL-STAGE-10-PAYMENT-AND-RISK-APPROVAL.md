# Publication Chapter 11 — Payment and Risk Approval

| Metadata | Value |
|---|---|
| Publication Chapter | `11` |
| Canonical Lifecycle Stage | `10` |
...
| Accountable capability | Payment / Finance Control / external payment providers |
...
This stage exists to **decide whether a financial obligation may become an executable payment intent and preserve risk, approval, execution, provider outcome, settlement, and reconciliation as separate facts**.
...
The accountable capability is **Payment / Finance Control / external payment providers**. Supporting Portals, workflow engines, reports, search indexes, integration adapters, and AI Agents may coordinate or present the work, but they do not silently become owners of the underlying business truth.
...
|---|---|
| `PaymentApprovalCase` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PaymentRiskAssessment` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ApprovalDecision` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PaymentIntent` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ProviderExecutionReference` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| `RISK_ASSESSMENT` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
| `EXECUTION_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `OUTCOME_UNKNOWN` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `AUTHORIZED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `CAPTURED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `FAILED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    [*] --> REQUESTED
    REQUESTED --> RISK_ASSESSMENT
    RISK_ASSESSMENT --> APPROVAL_PENDING
    APPROVAL_PENDING --> APPROVED
    APPROVED --> REJECTED
    REJECTED --> EXECUTION_PENDING
    EXECUTION_PENDING --> OUTCOME_UNKNOWN
    OUTCOME_UNKNOWN --> AUTHORIZED
    AUTHORIZED --> CAPTURED
    CAPTURED --> FAILED
    FAILED --> CANCELLED
...
|---|---|---|
| `REQUESTED` | `RISK_ASSESSMENT` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `RISK_ASSESSMENT` | `APPROVAL_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `APPROVAL_PENDING` | `APPROVED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `APPROVED` | `REJECTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REJECTED` | `EXECUTION_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `EXECUTION_PENDING` | `OUTCOME_UNKNOWN` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `OUTCOME_UNKNOWN` | `AUTHORIZED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `AUTHORIZED` | `CAPTURED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CAPTURED` | `FAILED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `FAILED` | `CANCELLED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CANCELLED` | `RECONCILIATION_REQUIRED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `GetPaymentApprovalCase`
- `ListPaymentApprovalCaseRecords`
- `GetPaymentApprovalCaseTimeline`
- `GetPaymentApprovalCaseEvidence`
- `GetPaymentApprovalCaseDecisionPackage`
- `GetPaymentApprovalCaseExceptions`
- `GetPaymentApprovalCaseAuditTrail`
- `GetPaymentApprovalCaseCurrentPerspective`
...
```text
POST /api/v1/payment_and_risk_approval/commands
GET  /api/v1/payment_and_risk_approval/{id}
GET  /api/v1/payment_and_risk_approval/{id}/timeline
GET  /api/v1/payment_and_risk_approval/{id}/evidence
GET  /api/v1/payment_and_risk_approval/{id}/decisions
POST /api/v1/payment_and_risk_approval/{id}/exceptions
POST /api/v1/payment_and_risk_approval/{id}/corrections
```
...
- detect duplicate or anomalous payment requests
- recommend a provider route
- prioritize reconciliation cases
...
- Payment, wallet, billing, and finance-related modules are registered.
- The existing Payment Domain chapter set is the repository-grounded baseline for intent, provider, unknown-outcome, refund, settlement, and reconciliation semantics.
- No claim is made that every PI-to-payment integration path is already implemented.
