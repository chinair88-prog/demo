# Publication Chapter 25 — Checkout, Customer Order, and Commitment

| Metadata | Value |
|---|---|
| Publication Chapter | `25` |
| Canonical Lifecycle Stage | `24` |
...
| Accountable capability | Order / Shop / Marketplace |
...
This stage exists to **convert shopping intent into an accepted customer order using frozen price, inventory, payment, and delivery context**.
...
The accountable capability is **Order / Shop / Marketplace**. Supporting Portals, workflow engines, reports, search indexes, integration adapters, and AI Agents may coordinate or present the work, but they do not silently become owners of the underlying business truth.
...
|---|---|
| `Cart` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CartLine` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CheckoutSession` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PriceSnapshot` | independent identified subject or governed subordinate entity; ownership must be declared |
| `InventoryReservation` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CustomerOrder` | independent identified subject or governed subordinate entity; ownership must be declared |
| `OrderLine` | independent identified subject or governed subordinate entity; ownership must be declared |
| `DeliveryPromise` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| State | Meaning |
|---|---|
| `CART_ACTIVE` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `CHECKOUT_STARTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
| `ORDER_ACCEPTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `ORDER_REJECTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `CANCEL_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `CANCELLED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `FULFILMENT_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    [*] --> CART_ACTIVE
    CART_ACTIVE --> CHECKOUT_STARTED
    CHECKOUT_STARTED --> VALIDATION_PENDING
    VALIDATION_PENDING --> PAYMENT_PENDING
    PAYMENT_PENDING --> ORDER_ACCEPTED
    ORDER_ACCEPTED --> ORDER_REJECTED
    ORDER_REJECTED --> CANCEL_PENDING
    CANCEL_PENDING --> CANCELLED
    CANCELLED --> FULFILMENT_PENDING
```
...
|---|---|---|
| `CART_ACTIVE` | `CHECKOUT_STARTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CHECKOUT_STARTED` | `VALIDATION_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `VALIDATION_PENDING` | `PAYMENT_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `PAYMENT_PENDING` | `ORDER_ACCEPTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `ORDER_ACCEPTED` | `ORDER_REJECTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `ORDER_REJECTED` | `CANCEL_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CANCEL_PENDING` | `CANCELLED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CANCELLED` | `FULFILMENT_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `GetCartTimeline`
- `GetCartEvidence`
- `GetCartDecisionPackage`
- `GetCartExceptions`
- `GetCartAuditTrail`
- `GetCartCurrentPerspective`
...
```text
POST /api/v1/checkout_customer_order_and_commitment/commands
GET  /api/v1/checkout_customer_order_and_commitment/{id}
GET  /api/v1/checkout_customer_order_and_commitment/{id}/timeline
GET  /api/v1/checkout_customer_order_and_commitment/{id}/evidence
GET  /api/v1/checkout_customer_order_and_commitment/{id}/decisions
POST /api/v1/checkout_customer_order_and_commitment/{id}/exceptions
POST /api/v1/checkout_customer_order_and_commitment/{id}/corrections
```
...
- assist the cart
- triage fraud and anomalies
- estimate delivery promise
...
- The Order Domain chapter set provides a repository-grounded sales-order baseline.
- Checkout and cross-service acceptance paths must be verified against frontend and backend source before full implementation status.
