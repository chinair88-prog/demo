# Publication Chapter 15 — Quality Inspection, Nonconformance, and Release

| Metadata | Value |
|---|---|
| Publication Chapter | `15` |
| Canonical Lifecycle Stage | `14` |
...
| Accountable capability | Quality / external inspector or laboratory |
...
This stage exists to **plan and execute inspection, record measurements and defects, manage nonconformance, and make explicit release, hold, rework, or rejection decisions**.
...
The accountable capability is **Quality / external inspector or laboratory**. Supporting Portals, workflow engines, reports, search indexes, integration adapters, and AI Agents may coordinate or present the work, but they do not silently become owners of the underlying business truth.
...
|---|---|
| `InspectionPlan` | independent identified subject or governed subordinate entity; ownership must be declared |
| `Inspection` | independent identified subject or governed subordinate entity; ownership must be declared |
| `Sample` | independent identified subject or governed subordinate entity; ownership must be declared |
| `Measurement` | independent identified subject or governed subordinate entity; ownership must be declared |
| `Defect` | independent identified subject or governed subordinate entity; ownership must be declared |
| `Nonconformance` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CorrectiveAction` | independent identified subject or governed subordinate entity; ownership must be declared |
| `QualityDecision` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ReleaseCertificate` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| State | Meaning |
|---|---|
| `PLANNED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `READY` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `IN_PROGRESS` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `EVIDENCE_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `DECISION_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `RELEASED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `CONDITIONALLY_RELEASED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `HELD` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `REWORK_REQUIRED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `REJECTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `REVOKED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    ...
    EVIDENCE_PENDING --> DECISION_PENDING
    DECISION_PENDING --> RELEASED
    RELEASED --> CONDITIONALLY_RELEASED
    CONDITIONALLY_RELEASED --> HELD
    HELD --> REWORK_REQUIRED
...
|---|---|---|
| `PLANNED` | `READY` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `READY` | `IN_PROGRESS` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `IN_PROGRESS` | `EVIDENCE_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `EVIDENCE_PENDING` | `DECISION_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `DECISION_PENDING` | `RELEASED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `RELEASED` | `CONDITIONALLY_RELEASED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CONDITIONALLY_RELEASED` | `HELD` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `HELD` | `REWORK_REQUIRED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REWORK_REQUIRED` | `REJECTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REJECTED` | `REVOKED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REVOKED` | `CLOSED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `GetInspectionPlan`
- `ListInspectionPlanRecords`
- `GetInspectionPlanTimeline`
- `GetInspectionPlanEvidence`
- `GetInspectionPlanDecisionPackage`
- `GetInspectionPlanExceptions`
- `GetInspectionPlanAuditTrail`
- `GetInspectionPlanCurrentPerspective`
...
```text
POST /api/v1/quality_inspection_nonconformance_and_release/commands
GET  /api/v1/quality_inspection_nonconformance_and_release/{id}
GET  /api/v1/quality_inspection_nonconformance_and_release/{id}/timeline
GET  /api/v1/quality_inspection_nonconformance_and_release/{id}/evidence
GET  /api/v1/quality_inspection_nonconformance_and_release/{id}/decisions
POST /api/v1/quality_inspection_nonconformance_and_release/{id}/exceptions
POST /api/v1/quality_inspection_nonconformance_and_release/{id}/corrections
```
...
- assist defect detection
- detect measurement anomalies
- classify nonconformance
- prepare a release-decision briefing
...
- Quality, regulatory, document, and AI vision capabilities are registered or referenced.
- A complete end-to-end quality aggregate and release authority require targeted repository inspection.
