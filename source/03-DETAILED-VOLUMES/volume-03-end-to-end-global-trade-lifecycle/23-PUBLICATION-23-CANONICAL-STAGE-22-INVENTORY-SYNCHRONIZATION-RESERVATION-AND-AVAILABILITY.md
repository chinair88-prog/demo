# Publication Chapter 23 — Inventory Synchronization, Reservation, and Availability

| Metadata | Value |
|---|---|
| Publication Chapter | `23` |
| Canonical Lifecycle Stage | `22` |
...
| Accountable capability | Inventory / Warehouse |
...
This stage exists to **derive governed stock positions and channel eligibility from receipt, quality, reservation, allocation, and adjustment facts**.
...
The accountable capability is **Inventory / Warehouse**. Supporting Portals, workflow engines, reports, search indexes, integration adapters, and AI Agents may coordinate or present the work, but they do not silently become owners of the underlying business truth.
...
|---|---|
| `InventoryPosition` | independent identified subject or governed subordinate entity; ownership must be declared |
| `InventoryLot` | independent identified subject or governed subordinate entity; ownership must be declared |
| `StockMovement` | independent identified subject or governed subordinate entity; ownership must be declared |
| `Reservation` | independent identified subject or governed subordinate entity; ownership must be declared |
| `Allocation` | independent identified subject or governed subordinate entity; ownership must be declared |
| `AvailabilityProjection` | independent identified subject or governed subordinate entity; ownership must be declared |
| `InventoryAdjustment` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CycleCount` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| State | Meaning |
|---|---|
| `PENDING_RECEIPT` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `QUARANTINED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `AVAILABLE` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `RESERVED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `ALLOCATED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `PICKING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `IN_TRANSIT_INTERNAL` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `DAMAGED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `BLOCKED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `EXPIRED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `DEPLETED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    [*] --> PENDING_RECEIPT
    PENDING_RECEIPT --> QUARANTINED
    QUARANTINED --> AVAILABLE
    AVAILABLE --> RESERVED
    RESERVED --> ALLOCATED
    ALLOCATED --> PICKING
    PICKING --> IN_TRANSIT_INTERNAL
    IN_TRANSIT_INTERNAL --> DAMAGED
    DAMAGED --> BLOCKED
    BLOCKED --> EXPIRED
    EXPIRED --> DEPLETED
```
...
|---|---|---|
| `PENDING_RECEIPT` | `QUARANTINED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `QUARANTINED` | `AVAILABLE` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `AVAILABLE` | `RESERVED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `RESERVED` | `ALLOCATED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `ALLOCATED` | `PICKING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `PICKING` | `IN_TRANSIT_INTERNAL` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `IN_TRANSIT_INTERNAL` | `DAMAGED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `DAMAGED` | `BLOCKED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `BLOCKED` | `EXPIRED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `EXPIRED` | `DEPLETED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `ListInventoryPositionRecords`
- `GetInventoryPositionTimeline`
- `GetInventoryPositionEvidence`
- `GetInventoryPositionDecisionPackage`
- `GetInventoryPositionExceptions`
- `GetInventoryPositionAuditTrail`
- `GetInventoryPositionCurrentPerspective`
...
```text
POST /api/v1/inventory_synchronization_reservation_and_availability/commands
GET  /api/v1/inventory_synchronization_reservation_and_availability/{id}
GET  /api/v1/inventory_synchronization_reservation_and_availability/{id}/timeline
GET  /api/v1/inventory_synchronization_reservation_and_availability/{id}/evidence
GET  /api/v1/inventory_synchronization_reservation_and_availability/{id}/decisions
POST /api/v1/inventory_synchronization_reservation_and_availability/{id}/exceptions
POST /api/v1/inventory_synchronization_reservation_and_availability/{id}/corrections
```
...
- forecast demand and depletion
- detect stock anomalies
- recommend replenishment
- predict reservation risk
...
- Inventory and warehouse services are registered.
- Reservation, allocation, adjustment, and availability semantics require direct repository evidence before VERIFIED_IMPLEMENTED status.
