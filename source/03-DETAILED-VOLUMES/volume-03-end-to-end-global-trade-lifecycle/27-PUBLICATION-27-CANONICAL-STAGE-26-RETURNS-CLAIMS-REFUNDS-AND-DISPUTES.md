# Publication Chapter 27 — Returns, Claims, Refunds, and Disputes

| Metadata | Value |
|---|---|
| Publication Chapter | `27` |
| Canonical Lifecycle Stage | `26` |
...
| Accountable capability | Returns / CRM / Payment / Finance |
...
This stage exists to **govern post-delivery remediation through return authorization, reverse logistics, inspection, claims, refunds, chargebacks, and disputes**.
...
The accountable capability is **Returns / CRM / Payment / Finance**. Supporting Portals, workflow engines, reports, search indexes, integration adapters, and AI Agents may coordinate or present the work, but they do not silently become owners of the underlying business truth.
...
|---|---|
| `ReturnRequest` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ReturnAuthorization` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ReturnShipment` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ReturnReceipt` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ReturnInspection` | independent identified subject or governed subordinate entity; ownership must be declared |
| `Claim` | independent identified subject or governed subordinate entity; ownership must be declared |
| `RefundIntent` | independent identified subject or governed subordinate entity; ownership must be declared |
| `Dispute` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ResolutionDecision` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| State | Meaning |
|---|---|
| `REQUESTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `ELIGIBILITY_REVIEW` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `AUTHORIZED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `REJECTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `RETURN_IN_TRANSIT` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `RECEIVED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `INSPECTION_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `REFUND_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `REFUNDED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `CLAIM_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `DISPUTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `RESOLVED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `CLOSED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    ...
    INSPECTION_PENDING --> REFUND_PENDING
    REFUND_PENDING --> REFUNDED
    REFUNDED --> CLAIM_PENDING
    CLAIM_PENDING --> DISPUTED
    DISPUTED --> RESOLVED
    RESOLVED --> CLOSED
```
...
|---|---|---|
| `REQUESTED` | `ELIGIBILITY_REVIEW` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `ELIGIBILITY_REVIEW` | `AUTHORIZED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `AUTHORIZED` | `REJECTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REJECTED` | `RETURN_IN_TRANSIT` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `RETURN_IN_TRANSIT` | `RECEIVED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `RECEIVED` | `INSPECTION_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `INSPECTION_PENDING` | `REFUND_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REFUND_PENDING` | `REFUNDED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REFUNDED` | `CLAIM_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CLAIM_PENDING` | `DISPUTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `DISPUTED` | `RESOLVED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `RESOLVED` | `CLOSED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `GetReturnRequest`
- `ListReturnRequestRecords`
- `GetReturnRequestTimeline`
- `GetReturnRequestEvidence`
- `GetReturnRequestDecisionPackage`
- `GetReturnRequestExceptions`
- `GetReturnRequestAuditTrail`
- `GetReturnRequestCurrentPerspective`
...
```text
POST /api/v1/returns_claims_refunds_and_disputes/commands
...
POST /api/v1/returns_claims_refunds_and_disputes/{id}/corrections
```
...
- recommend evidence and routing
...
- Returns, CRM/customer, payment, and finance capabilities are registered.
- Full case, claim, dispute, and refund integration requires targeted repository evidence.
