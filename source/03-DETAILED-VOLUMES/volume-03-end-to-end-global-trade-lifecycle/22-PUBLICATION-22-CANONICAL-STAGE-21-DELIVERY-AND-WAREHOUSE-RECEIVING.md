# Publication Chapter 22 — Delivery and Warehouse Receiving

| Metadata | Value |
|---|---|
| Publication Chapter | `22` |
| Canonical Lifecycle Stage | `21` |
...
| Accountable capability | Warehouse / Shipping / Quality |
...
This stage exists to **evidence destination delivery and warehouse receipt, count, condition, quarantine, and discrepancy without automatically creating available inventory**.
...
The accountable capability is **Warehouse / Shipping / Quality**. Supporting Portals, workflow engines, reports, search indexes, integration adapters, and AI Agents may coordinate or present the work, but they do not silently become owners of the underlying business truth.
...
|---|---|
| `DeliveryAppointment` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ProofOfDelivery` | independent identified subject or governed subordinate entity; ownership must be declared |
| `WarehouseReceipt` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ReceiptLine` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PackageReceipt` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ReceivingInspection` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ReceivingDiscrepancy` | independent identified subject or governed subordinate entity; ownership must be declared |
| `QuarantineRecord` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| State | Meaning |
|---|---|
| `APPOINTMENT_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `SCHEDULED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `ARRIVED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `UNLOADING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `COUNTING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `INSPECTION` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `DISCREPANCY` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    [*] --> APPOINTMENT_PENDING
    APPOINTMENT_PENDING --> SCHEDULED
    SCHEDULED --> ARRIVED
    ARRIVED --> UNLOADING
    UNLOADING --> COUNTING
    COUNTING --> INSPECTION
    INSPECTION --> DISCREPANCY
    DISCREPANCY --> QUARANTINED
...
|---|---|---|
| `APPOINTMENT_PENDING` | `SCHEDULED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `SCHEDULED` | `ARRIVED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `ARRIVED` | `UNLOADING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `UNLOADING` | `COUNTING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `COUNTING` | `INSPECTION` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `INSPECTION` | `DISCREPANCY` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `DISCREPANCY` | `QUARANTINED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `QUARANTINED` | `RECEIVED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `RECEIVED` | `REJECTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REJECTED` | `CLOSED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `WarehouseReceivingStarted`
- `ReceivingDiscrepancyOpened`
- `GoodsQuarantined`
- `WarehouseReceiptAccepted`
- `WarehouseReceiptCorrected`
...
- `ListDeliveryAppointmentRecords`
- `GetDeliveryAppointmentTimeline`
- `GetDeliveryAppointmentEvidence`
- `GetDeliveryAppointmentDecisionPackage`
- `GetDeliveryAppointmentExceptions`
- `GetDeliveryAppointmentAuditTrail`
- `GetDeliveryAppointmentCurrentPerspective`
...
```text
POST /api/v1/delivery_and_warehouse_receiving/commands
GET  /api/v1/delivery_and_warehouse_receiving/{id}
GET  /api/v1/delivery_and_warehouse_receiving/{id}/timeline
GET  /api/v1/delivery_and_warehouse_receiving/{id}/evidence
GET  /api/v1/delivery_and_warehouse_receiving/{id}/decisions
POST /api/v1/delivery_and_warehouse_receiving/{id}/exceptions
POST /api/v1/delivery_and_warehouse_receiving/{id}/corrections
```
...
- assist damage detection
- detect count anomalies
- reconcile documents with receipt
...
- Warehouse, inventory, shipping, and quality-related capabilities are registered.
- Receiving and inventory ownership remain separate even where current implementation overlaps.
