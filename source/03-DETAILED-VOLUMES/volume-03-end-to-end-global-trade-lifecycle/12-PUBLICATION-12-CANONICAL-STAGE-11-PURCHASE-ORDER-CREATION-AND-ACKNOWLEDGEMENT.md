# Publication Chapter 12 — Purchase Order Creation and Acknowledgement

| Metadata | Value |
|---|---|
| Publication Chapter | `12` |
| Canonical Lifecycle Stage | `11` |
...
| Accountable capability | Procurement / Purchase Order |
...
This stage exists to **create a buyer-controlled execution commitment and obtain explicit supplier acknowledgement without collapsing the PO into a sales order, contract, production order, or payment**.
...
The accountable capability is **Procurement / Purchase Order**. Supporting Portals, workflow engines, reports, search indexes, integration adapters, and AI Agents may coordinate or present the work, but they do not silently become owners of the underlying business truth.
...
|---|---|
| `PurchaseOrder` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PurchaseOrderVersion` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PurchaseOrderLine` | independent identified subject or governed subordinate entity; ownership must be declared |
| `DeliverySchedule` | independent identified subject or governed subordinate entity; ownership must be declared |
| `SupplierAcknowledgement` | independent identified subject or governed subordinate entity; ownership must be declared |
| `POAmendment` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| `ACKNOWLEDGEMENT_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `ACKNOWLEDGED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `REJECTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `AMENDMENT_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `PARTIALLY_FULFILLED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `FULFILLED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    ...
    APPROVED --> ISSUED
    ISSUED --> ACKNOWLEDGEMENT_PENDING
    ACKNOWLEDGEMENT_PENDING --> ACKNOWLEDGED
    ACKNOWLEDGED --> REJECTED
    REJECTED --> AMENDMENT_PENDING
    AMENDMENT_PENDING --> PARTIALLY_FULFILLED
...
|---|---|---|
| `DRAFT` | `APPROVAL_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
| `ISSUED` | `ACKNOWLEDGEMENT_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `ACKNOWLEDGEMENT_PENDING` | `ACKNOWLEDGED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `ACKNOWLEDGED` | `REJECTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REJECTED` | `AMENDMENT_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `AMENDMENT_PENDING` | `PARTIALLY_FULFILLED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `PARTIALLY_FULFILLED` | `FULFILLED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `FULFILLED` | `CANCELLED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CANCELLED` | `CLOSED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `GetPurchaseOrderEvidence`
- `GetPurchaseOrderDecisionPackage`
- `GetPurchaseOrderExceptions`
- `GetPurchaseOrderAuditTrail`
- `GetPurchaseOrderCurrentPerspective`
...
```text
POST /api/v1/purchase_order_creation_and_acknowledgement/commands
GET  /api/v1/purchase_order_creation_and_acknowledgement/{id}
GET  /api/v1/purchase_order_creation_and_acknowledgement/{id}/timeline
GET  /api/v1/purchase_order_creation_and_acknowledgement/{id}/evidence
GET  /api/v1/purchase_order_creation_and_acknowledgement/{id}/decisions
POST /api/v1/purchase_order_creation_and_acknowledgement/{id}/exceptions
POST /api/v1/purchase_order_creation_and_acknowledgement/{id}/corrections
```
...
- compare the PO with source terms
- detect missing clauses and schedule conflicts
- summarize supplier deviations
- predict acknowledgement delay
...
- Procurement, order, contract, and manufacturing capabilities are registered.
- Purchase Order truth must not be inferred from a generic customer sales-order aggregate.
- The dedicated PO aggregate and acknowledgement workflow remain REQUIRES_REPOSITORY_REVIEW.
