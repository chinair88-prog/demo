# Publication Chapter 16 — Packing List and Commercial Invoice

| Metadata | Value |
|---|---|
| Publication Chapter | `16` |
| Canonical Lifecycle Stage | `15` |
...
| Accountable capability | Trade Documents / Finance |
...
This stage exists to **produce shipment-ready, versioned commercial documents that reconcile products, packages, quantities, values, weights, and terms with production and quality evidence**.
...
The accountable capability is **Trade Documents / Finance**. Supporting Portals, workflow engines, reports, search indexes, integration adapters, and AI Agents may coordinate or present the work, but they do not silently become owners of the underlying business truth.
...
|---|---|
| `PackingList` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PackingListVersion` | independent identified subject or governed subordinate entity; ownership must be declared |
| `Package` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CargoUnit` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CommercialInvoice` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CommercialInvoiceLine` | independent identified subject or governed subordinate entity; ownership must be declared |
| `DocumentReconciliation` | independent identified subject or governed subordinate entity; ownership must be declared |
| `DocumentSignature` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| `AMENDED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `ACCEPTED_FOR_SHIPMENT` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    ...
    ISSUED --> AMENDED
    AMENDED --> ACCEPTED_FOR_SHIPMENT
    ACCEPTED_FOR_SHIPMENT --> REJECTED
    REJECTED --> SUPERSEDED
...
| `APPROVAL_PENDING` | `ISSUED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `ISSUED` | `AMENDED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `AMENDED` | `ACCEPTED_FOR_SHIPMENT` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `ACCEPTED_FOR_SHIPMENT` | `REJECTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REJECTED` | `SUPERSEDED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `SUPERSEDED` | `CANCELLED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `ApproveShipmentDocuments`
- `IssueShipmentDocuments`
- `AmendShipmentDocuments`
...
- `PackingListCreated`
- `CommercialInvoiceCreated`
- `ShipmentDocumentsValidated`
- `ShipmentDocumentsIssued`
- `ShipmentDocumentsAmended`
- `DocumentDiscrepancyDetected`
...
- `GetPackingList`
- `ListPackingListRecords`
- `GetPackingListTimeline`
- `GetPackingListEvidence`
- `GetPackingListDecisionPackage`
- `GetPackingListExceptions`
- `GetPackingListAuditTrail`
- `GetPackingListCurrentPerspective`
...
```text
POST /api/v1/packing_list_and_commercial_invoice/commands
GET  /api/v1/packing_list_and_commercial_invoice/{id}
GET  /api/v1/packing_list_and_commercial_invoice/{id}/timeline
GET  /api/v1/packing_list_and_commercial_invoice/{id}/evidence
GET  /api/v1/packing_list_and_commercial_invoice/{id}/decisions
POST /api/v1/packing_list_and_commercial_invoice/{id}/exceptions
POST /api/v1/packing_list_and_commercial_invoice/{id}/corrections
```
...
- reconcile documents
- suggest HS codes with confidence
- detect weight and value anomalies
...
- Document, finance, trade, and shipping capabilities are registered.
- Exact document schemas and generation behavior are not asserted implemented without executable evidence.
