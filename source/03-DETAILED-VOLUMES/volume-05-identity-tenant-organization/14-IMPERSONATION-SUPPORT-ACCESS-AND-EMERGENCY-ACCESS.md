# Impersonation, Support Access, and Emergency Access

| Metadata | Value |
|---|---|
| Volume | `5` |
| Chapter ID | `V05-14` |
| Subject | `PrivilegedAccess` |
...
| Accountable owner | Security Operations |
...
This chapter defines how GTOS shall **govern support impersonation, break-glass access, reason, approval, scope, notification, review, and revocation**.
...
The accountable owner is **Security Operations**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `PrivilegedAccess` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PrivilegedAccessVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PrivilegedAccessDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PrivilegedAccessEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PrivilegedAccessException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ValidatePrivilegedAccess`
- `ApprovePrivilegedAccess`
- `ActivatePrivilegedAccess`
- `SuspendPrivilegedAccess`
- `CorrectPrivilegedAccess`
- `RetirePrivilegedAccess`

Reference Events:

- `PrivilegedAccessCreated`
- `PrivilegedAccessValidated`
- `PrivilegedAccessApproved`
- `PrivilegedAccessActivated`
- `PrivilegedAccessSuspended`
- `PrivilegedAccessCorrected`
- `PrivilegedAccessRetired`

Reference queries:

- `GetPrivilegedAccess`
- `ListPrivilegedAccessRecords`
- `GetPrivilegedAccessTimeline`
- `GetPrivilegedAccessEvidence`
- `GetPrivilegedAccessDecisionPackage`
- `GetPrivilegedAccessExceptions`
- `GetPrivilegedAccessAuditTrail`

Reference API surface:

```text
POST /api/v1/privilegedaccess/commands
GET  /api/v1/privilegedaccess/{id}
GET  /api/v1/privilegedaccess/{id}/timeline
GET  /api/v1/privilegedaccess/{id}/evidence
GET  /api/v1/privilegedaccess/{id}/decisions
POST /api/v1/privilegedaccess/{id}/exceptions
POST /api/v1/privilegedaccess/{id}/corrections
```
...
|---|---|
| `privilegedaccess_aggregate` | authoritative subject |
| `privilegedaccess_version` | immutable submitted, approved, issued, or signed versions |
| `privilegedaccess_relationship` | governed relationships |
| `privilegedaccess_decision` | binding Decisions and rationale |
| `privilegedaccess_evidence_link` | provenance and evidence relationships |
| `privilegedaccess_event_outbox` | transactional Event publication |
| `privilegedaccess_idempotency` | duplicate-write protection |
| `privilegedaccess_exception` | exception, claim, dispute, or hold |
| `privilegedaccess_correction` | correction and supersession lineage |
| `privilegedaccess_audit` | privileged action evidence |
