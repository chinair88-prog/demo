...
This chapter defines how GTOS shall **govern interactive and machine session creation, refresh, idle and absolute timeout, logout, revocation, and device visibility**.
...
|---|---|
| `SessionLifecycle` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SessionLifecycleVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SessionLifecycleDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SessionLifecycleEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SessionLifecycleException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveSessionLifecycle`
- `ActivateSessionLifecycle`
- `SuspendSessionLifecycle`
- `CorrectSessionLifecycle`
- `RetireSessionLifecycle`

Reference Events:

- `SessionLifecycleCreated`
- `SessionLifecycleValidated`
- `SessionLifecycleApproved`
- `SessionLifecycleActivated`
- `SessionLifecycleSuspended`
- `SessionLifecycleCorrected`
- `SessionLifecycleRetired`

Reference queries:

- `GetSessionLifecycle`
- `ListSessionLifecycleRecords`
- `GetSessionLifecycleTimeline`
- `GetSessionLifecycleEvidence`
- `GetSessionLifecycleDecisionPackage`
- `GetSessionLifecycleExceptions`
- `GetSessionLifecycleAuditTrail`

Reference API surface:

```text
POST /api/v1/sessionlifecycle/commands
GET  /api/v1/sessionlifecycle/{id}
GET  /api/v1/sessionlifecycle/{id}/timeline
GET  /api/v1/sessionlifecycle/{id}/evidence
GET  /api/v1/sessionlifecycle/{id}/decisions
POST /api/v1/sessionlifecycle/{id}/exceptions
POST /api/v1/sessionlifecycle/{id}/corrections
```
...
|---|---|
| `sessionlifecycle_aggregate` | authoritative subject |
| `sessionlifecycle_version` | immutable submitted, approved, issued, or signed versions |
| `sessionlifecycle_relationship` | governed relationships |
| `sessionlifecycle_decision` | binding Decisions and rationale |
| `sessionlifecycle_evidence_link` | provenance and evidence relationships |
| `sessionlifecycle_event_outbox` | transactional Event publication |
| `sessionlifecycle_idempotency` | duplicate-write protection |
| `sessionlifecycle_exception` | exception, claim, dispute, or hold |
| `sessionlifecycle_correction` | correction and supersession lineage |
| `sessionlifecycle_audit` | privileged action evidence |
