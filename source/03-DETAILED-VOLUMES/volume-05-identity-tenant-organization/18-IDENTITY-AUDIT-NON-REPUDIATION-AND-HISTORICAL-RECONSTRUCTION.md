# Identity Audit, Non-Repudiation, and Historical Reconstruction

| Metadata | Value |
|---|---|
| Volume | `5` |
| Chapter ID | `V05-18` |
| Subject | `IdentityAudit` |
...
| Accountable owner | Security / Audit |
...
This chapter defines how GTOS shall **reconstruct authentication, token, mandate, permission, session, privileged action, correction, and revocation at a point in time**.
...
The accountable owner is **Security / Audit**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `IdentityAudit` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `IdentityAuditVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `IdentityAuditDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `IdentityAuditEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `IdentityAuditException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveIdentityAudit`
- `ActivateIdentityAudit`
- `SuspendIdentityAudit`
- `CorrectIdentityAudit`
- `RetireIdentityAudit`

Reference Events:

- `IdentityAuditCreated`
- `IdentityAuditValidated`
- `IdentityAuditApproved`
- `IdentityAuditActivated`
- `IdentityAuditSuspended`
- `IdentityAuditCorrected`
- `IdentityAuditRetired`

Reference queries:

- `GetIdentityAudit`
- `ListIdentityAuditRecords`
- `GetIdentityAuditTimeline`
- `GetIdentityAuditEvidence`
- `GetIdentityAuditDecisionPackage`
- `GetIdentityAuditExceptions`
- `GetIdentityAuditAuditTrail`

Reference API surface:

```text
POST /api/v1/identityaudit/commands
GET  /api/v1/identityaudit/{id}
GET  /api/v1/identityaudit/{id}/timeline
GET  /api/v1/identityaudit/{id}/evidence
GET  /api/v1/identityaudit/{id}/decisions
POST /api/v1/identityaudit/{id}/exceptions
POST /api/v1/identityaudit/{id}/corrections
```
...
|---|---|
| `identityaudit_aggregate` | authoritative subject |
| `identityaudit_version` | immutable submitted, approved, issued, or signed versions |
| `identityaudit_relationship` | governed relationships |
| `identityaudit_decision` | binding Decisions and rationale |
| `identityaudit_evidence_link` | provenance and evidence relationships |
| `identityaudit_event_outbox` | transactional Event publication |
| `identityaudit_idempotency` | duplicate-write protection |
| `identityaudit_exception` | exception, claim, dispute, or hold |
| `identityaudit_correction` | correction and supersession lineage |
| `identityaudit_audit` | privileged action evidence |
