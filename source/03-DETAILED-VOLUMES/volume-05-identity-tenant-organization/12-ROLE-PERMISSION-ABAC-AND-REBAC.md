# Role, Permission, ABAC, and ReBAC

| Metadata | Value |
|---|---|
| Volume | `5` |
| Chapter ID | `V05-12` |
| Subject | `AuthorizationModel` |
...
| Accountable owner | Security Domain |
...
This chapter defines how GTOS shall **derive action eligibility from role, relationship, attributes, resource state, mandate, Policy, tenant, and jurisdiction**.
...
The accountable owner is **Security Domain**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `AuthorizationModel` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AuthorizationModelVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AuthorizationModelDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AuthorizationModelEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AuthorizationModelException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveAuthorizationModel`
- `ActivateAuthorizationModel`
- `SuspendAuthorizationModel`
- `CorrectAuthorizationModel`
- `RetireAuthorizationModel`

Reference Events:

- `AuthorizationModelCreated`
- `AuthorizationModelValidated`
- `AuthorizationModelApproved`
- `AuthorizationModelActivated`
- `AuthorizationModelSuspended`
- `AuthorizationModelCorrected`
- `AuthorizationModelRetired`

Reference queries:

- `GetAuthorizationModel`
- `ListAuthorizationModelRecords`
- `GetAuthorizationModelTimeline`
- `GetAuthorizationModelEvidence`
- `GetAuthorizationModelDecisionPackage`
- `GetAuthorizationModelExceptions`
- `GetAuthorizationModelAuditTrail`

Reference API surface:

```text
POST /api/v1/authorizationmodel/commands
GET  /api/v1/authorizationmodel/{id}
GET  /api/v1/authorizationmodel/{id}/timeline
GET  /api/v1/authorizationmodel/{id}/evidence
GET  /api/v1/authorizationmodel/{id}/decisions
POST /api/v1/authorizationmodel/{id}/exceptions
POST /api/v1/authorizationmodel/{id}/corrections
```
...
|---|---|
| `authorizationmodel_aggregate` | authoritative subject |
| `authorizationmodel_version` | immutable submitted, approved, issued, or signed versions |
| `authorizationmodel_relationship` | governed relationships |
| `authorizationmodel_decision` | binding Decisions and rationale |
| `authorizationmodel_evidence_link` | provenance and evidence relationships |
| `authorizationmodel_event_outbox` | transactional Event publication |
| `authorizationmodel_idempotency` | duplicate-write protection |
| `authorizationmodel_exception` | exception, claim, dispute, or hold |
| `authorizationmodel_correction` | correction and supersession lineage |
| `authorizationmodel_audit` | privileged action evidence |
