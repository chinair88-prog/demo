# Device, Browser, and Mobile Identity
...
| Accountable owner | Security Platform |
...
This chapter defines how GTOS shall **identify devices and sessions with risk context without treating device identity as person authority**.
...
The accountable owner is **Security Platform**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `DeviceIdentity` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DeviceIdentityVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DeviceIdentityDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DeviceIdentityEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DeviceIdentityException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveDeviceIdentity`
- `ActivateDeviceIdentity`
- `SuspendDeviceIdentity`
- `CorrectDeviceIdentity`
- `RetireDeviceIdentity`

Reference Events:

- `DeviceIdentityCreated`
- `DeviceIdentityValidated`
- `DeviceIdentityApproved`
- `DeviceIdentityActivated`
- `DeviceIdentitySuspended`
- `DeviceIdentityCorrected`
- `DeviceIdentityRetired`

Reference queries:

- `GetDeviceIdentity`
- `ListDeviceIdentityRecords`
- `GetDeviceIdentityTimeline`
- `GetDeviceIdentityEvidence`
- `GetDeviceIdentityDecisionPackage`
- `GetDeviceIdentityExceptions`
- `GetDeviceIdentityAuditTrail`

Reference API surface:

```text
POST /api/v1/deviceidentity/commands
GET  /api/v1/deviceidentity/{id}
GET  /api/v1/deviceidentity/{id}/timeline
GET  /api/v1/deviceidentity/{id}/evidence
GET  /api/v1/deviceidentity/{id}/decisions
POST /api/v1/deviceidentity/{id}/exceptions
POST /api/v1/deviceidentity/{id}/corrections
```
...
|---|---|
| `deviceidentity_aggregate` | authoritative subject |
| `deviceidentity_version` | immutable submitted, approved, issued, or signed versions |
| `deviceidentity_relationship` | governed relationships |
| `deviceidentity_decision` | binding Decisions and rationale |
| `deviceidentity_evidence_link` | provenance and evidence relationships |
| `deviceidentity_event_outbox` | transactional Event publication |
| `deviceidentity_idempotency` | duplicate-write protection |
| `deviceidentity_exception` | exception, claim, dispute, or hold |
| `deviceidentity_correction` | correction and supersession lineage |
| `deviceidentity_audit` | privileged action evidence |
