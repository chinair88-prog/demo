# Membership, Employment, and Participant Relationships

| Metadata | Value |
|---|---|
| Volume | `5` |
| Chapter ID | `V05-11` |
| Subject | `MembershipRelationship` |
...
| Accountable owner | Identity / Employee / Customer |
...
This chapter defines how GTOS shall **model employment, membership, supplier, provider, broker, carrier, customer, and operator relationships as independent facts**.
...
The accountable owner is **Identity / Employee / Customer**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `MembershipRelationship` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MembershipRelationshipVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MembershipRelationshipDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MembershipRelationshipEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MembershipRelationshipException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveMembershipRelationship`
- `ActivateMembershipRelationship`
- `SuspendMembershipRelationship`
- `CorrectMembershipRelationship`
- `RetireMembershipRelationship`

Reference Events:

- `MembershipRelationshipCreated`
- `MembershipRelationshipValidated`
- `MembershipRelationshipApproved`
- `MembershipRelationshipActivated`
- `MembershipRelationshipSuspended`
- `MembershipRelationshipCorrected`
- `MembershipRelationshipRetired`

Reference queries:

- `GetMembershipRelationship`
- `ListMembershipRelationshipRecords`
- `GetMembershipRelationshipTimeline`
- `GetMembershipRelationshipEvidence`
- `GetMembershipRelationshipDecisionPackage`
- `GetMembershipRelationshipExceptions`
- `GetMembershipRelationshipAuditTrail`

Reference API surface:

```text
POST /api/v1/membershiprelationship/commands
GET  /api/v1/membershiprelationship/{id}
GET  /api/v1/membershiprelationship/{id}/timeline
GET  /api/v1/membershiprelationship/{id}/evidence
GET  /api/v1/membershiprelationship/{id}/decisions
POST /api/v1/membershiprelationship/{id}/exceptions
POST /api/v1/membershiprelationship/{id}/corrections
```
...
|---|---|
| `membershiprelationship_aggregate` | authoritative subject |
| `membershiprelationship_version` | immutable submitted, approved, issued, or signed versions |
| `membershiprelationship_relationship` | governed relationships |
| `membershiprelationship_decision` | binding Decisions and rationale |
| `membershiprelationship_evidence_link` | provenance and evidence relationships |
| `membershiprelationship_event_outbox` | transactional Event publication |
| `membershiprelationship_idempotency` | duplicate-write protection |
| `membershiprelationship_exception` | exception, claim, dispute, or hold |
| `membershiprelationship_correction` | correction and supersession lineage |
| `membershiprelationship_audit` | privileged action evidence |
...
- cryptographic authentication and secure credential lifecycle
- tenant and represented-principal isolation
- revocation, audit and point-in-time authorization reconstruction
- confused-deputy, impersonation and replay prevention
