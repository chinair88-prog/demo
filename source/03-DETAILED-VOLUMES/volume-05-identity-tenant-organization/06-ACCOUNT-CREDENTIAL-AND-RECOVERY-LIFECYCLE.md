# Account, Credential, and Recovery Lifecycle

| Metadata | Value |
|---|---|
| Volume | `5` |
| Chapter ID | `V05-06` |
| Subject | `CredentialLifecycle` |
...
| Accountable owner | Auth Service |
...
This chapter defines how GTOS shall **govern account creation, verification, password, passkey, federation, recovery, lockout, revocation, and compromise**.
...
The accountable owner is **Auth Service**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `CredentialLifecycle` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CredentialLifecycleVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CredentialLifecycleDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CredentialLifecycleEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CredentialLifecycleException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveCredentialLifecycle`
- `ActivateCredentialLifecycle`
- `SuspendCredentialLifecycle`
- `CorrectCredentialLifecycle`
- `RetireCredentialLifecycle`

Reference Events:

- `CredentialLifecycleCreated`
- `CredentialLifecycleValidated`
- `CredentialLifecycleApproved`
- `CredentialLifecycleActivated`
- `CredentialLifecycleSuspended`
- `CredentialLifecycleCorrected`
- `CredentialLifecycleRetired`

Reference queries:

- `GetCredentialLifecycle`
- `ListCredentialLifecycleRecords`
- `GetCredentialLifecycleTimeline`
- `GetCredentialLifecycleEvidence`
- `GetCredentialLifecycleDecisionPackage`
- `GetCredentialLifecycleExceptions`
- `GetCredentialLifecycleAuditTrail`

Reference API surface:

```text
POST /api/v1/credentiallifecycle/commands
GET  /api/v1/credentiallifecycle/{id}
GET  /api/v1/credentiallifecycle/{id}/timeline
GET  /api/v1/credentiallifecycle/{id}/evidence
GET  /api/v1/credentiallifecycle/{id}/decisions
POST /api/v1/credentiallifecycle/{id}/exceptions
POST /api/v1/credentiallifecycle/{id}/corrections
```
...
|---|---|
| `credentiallifecycle_aggregate` | authoritative subject |
| `credentiallifecycle_version` | immutable submitted, approved, issued, or signed versions |
| `credentiallifecycle_relationship` | governed relationships |
| `credentiallifecycle_decision` | binding Decisions and rationale |
| `credentiallifecycle_evidence_link` | provenance and evidence relationships |
| `credentiallifecycle_event_outbox` | transactional Event publication |
| `credentiallifecycle_idempotency` | duplicate-write protection |
| `credentiallifecycle_exception` | exception, claim, dispute, or hold |
| `credentiallifecycle_correction` | correction and supersession lineage |
| `credentiallifecycle_audit` | privileged action evidence |
