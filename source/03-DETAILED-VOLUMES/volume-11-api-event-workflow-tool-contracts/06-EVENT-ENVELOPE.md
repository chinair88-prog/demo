...
This chapter defines how GTOS shall **define interoperable semantics, schemas, compatibility, security, failure, testing, and lifecycle governance for Event Envelope**.
...
|---|---|
| `EventEnvelope` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `EventEnvelopeVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `EventEnvelopeDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `EventEnvelopeEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `EventEnvelopeException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveEventEnvelope`
- `ActivateEventEnvelope`
- `SuspendEventEnvelope`
- `CorrectEventEnvelope`
- `RetireEventEnvelope`

Reference Events:

- `EventEnvelopeCreated`
- `EventEnvelopeValidated`
- `EventEnvelopeApproved`
- `EventEnvelopeActivated`
- `EventEnvelopeSuspended`
- `EventEnvelopeCorrected`
- `EventEnvelopeRetired`

Reference queries:

- `GetEventEnvelope`
- `ListEventEnvelopeRecords`
- `GetEventEnvelopeTimeline`
- `GetEventEnvelopeEvidence`
- `GetEventEnvelopeDecisionPackage`
- `GetEventEnvelopeExceptions`
- `GetEventEnvelopeAuditTrail`

Reference API surface:

```text
POST /api/v1/eventenvelope/commands
GET  /api/v1/eventenvelope/{id}
GET  /api/v1/eventenvelope/{id}/timeline
GET  /api/v1/eventenvelope/{id}/evidence
GET  /api/v1/eventenvelope/{id}/decisions
POST /api/v1/eventenvelope/{id}/exceptions
POST /api/v1/eventenvelope/{id}/corrections
```
...
|---|---|
| `eventenvelope_aggregate` | authoritative subject |
| `eventenvelope_version` | immutable submitted, approved, issued, or signed versions |
| `eventenvelope_relationship` | governed relationships |
| `eventenvelope_decision` | binding Decisions and rationale |
| `eventenvelope_evidence_link` | provenance and evidence relationships |
| `eventenvelope_event_outbox` | transactional Event publication |
| `eventenvelope_idempotency` | duplicate-write protection |
| `eventenvelope_exception` | exception, claim, dispute, or hold |
| `eventenvelope_correction` | correction and supersession lineage |
| `eventenvelope_audit` | privileged action evidence |
