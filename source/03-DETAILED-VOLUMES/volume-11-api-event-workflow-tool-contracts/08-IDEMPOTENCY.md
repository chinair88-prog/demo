...
This chapter defines how GTOS shall **define interoperable semantics, schemas, compatibility, security, failure, testing, and lifecycle governance for Idempotency**.
...
|---|---|
| `Idempotency` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `IdempotencyVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `IdempotencyDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `IdempotencyEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `IdempotencyException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `IdempotencyApproved`
- `IdempotencyActivated`
- `IdempotencySuspended`
- `IdempotencyCorrected`
- `IdempotencyRetired`
...
- `GetIdempotencyTimeline`
- `GetIdempotencyEvidence`
- `GetIdempotencyDecisionPackage`
- `GetIdempotencyExceptions`
- `GetIdempotencyAuditTrail`

Reference API surface:

```text
POST /api/v1/idempotency/commands
GET  /api/v1/idempotency/{id}
GET  /api/v1/idempotency/{id}/timeline
GET  /api/v1/idempotency/{id}/evidence
GET  /api/v1/idempotency/{id}/decisions
POST /api/v1/idempotency/{id}/exceptions
POST /api/v1/idempotency/{id}/corrections
```
...
|---|---|
| `idempotency_aggregate` | authoritative subject |
| `idempotency_version` | immutable submitted, approved, issued, or signed versions |
| `idempotency_relationship` | governed relationships |
| `idempotency_decision` | binding Decisions and rationale |
| `idempotency_evidence_link` | provenance and evidence relationships |
| `idempotency_event_outbox` | transactional Event publication |
| `idempotency_idempotency` | duplicate-write protection |
| `idempotency_exception` | exception, claim, dispute, or hold |
| `idempotency_correction` | correction and supersession lineage |
| `idempotency_audit` | privileged action evidence |
