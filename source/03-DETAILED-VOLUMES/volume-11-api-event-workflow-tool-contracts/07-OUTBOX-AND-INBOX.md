...
This chapter defines how GTOS shall **define interoperable semantics, schemas, compatibility, security, failure, testing, and lifecycle governance for Outbox and Inbox**.
...
|---|---|
| `OutboxandInbox` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `OutboxandInboxVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `OutboxandInboxDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `OutboxandInboxEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `OutboxandInboxException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreateOutboxandInbox`
- `ValidateOutboxandInbox`
- `ApproveOutboxandInbox`
- `ActivateOutboxandInbox`
- `SuspendOutboxandInbox`
- `CorrectOutboxandInbox`
- `RetireOutboxandInbox`

Reference Events:

- `OutboxandInboxCreated`
- `OutboxandInboxValidated`
- `OutboxandInboxApproved`
- `OutboxandInboxActivated`
- `OutboxandInboxSuspended`
- `OutboxandInboxCorrected`
- `OutboxandInboxRetired`

Reference queries:

- `GetOutboxandInbox`
- `ListOutboxandInboxRecords`
- `GetOutboxandInboxTimeline`
- `GetOutboxandInboxEvidence`
- `GetOutboxandInboxDecisionPackage`
- `GetOutboxandInboxExceptions`
- `GetOutboxandInboxAuditTrail`

Reference API surface:

```text
POST /api/v1/outboxandinbox/commands
GET  /api/v1/outboxandinbox/{id}
GET  /api/v1/outboxandinbox/{id}/timeline
GET  /api/v1/outboxandinbox/{id}/evidence
GET  /api/v1/outboxandinbox/{id}/decisions
POST /api/v1/outboxandinbox/{id}/exceptions
POST /api/v1/outboxandinbox/{id}/corrections
```
...
|---|---|
| `outboxandinbox_aggregate` | authoritative subject |
| `outboxandinbox_version` | immutable submitted, approved, issued, or signed versions |
| `outboxandinbox_relationship` | governed relationships |
| `outboxandinbox_decision` | binding Decisions and rationale |
| `outboxandinbox_evidence_link` | provenance and evidence relationships |
| `outboxandinbox_event_outbox` | transactional Event publication |
| `outboxandinbox_idempotency` | duplicate-write protection |
| `outboxandinbox_exception` | exception, claim, dispute, or hold |
| `outboxandinbox_correction` | correction and supersession lineage |
| `outboxandinbox_audit` | privileged action evidence |
