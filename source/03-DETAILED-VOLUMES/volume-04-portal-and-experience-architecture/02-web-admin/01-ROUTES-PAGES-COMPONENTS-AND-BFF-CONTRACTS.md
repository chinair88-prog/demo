# Web Admin — Routes, Pages, Components, and BFF Contracts

| Metadata | Value |
|---|---|
| Volume | `4` |
| Chapter ID | `V04-P02-01` |
| Subject | `WebAdminRouteRegistry` |
...
This chapter defines how GTOS shall **inventory routes, layouts, pages, components, server/client boundaries, BFFs, APIs, realtime channels and ownership for platform and tenant administrators, covering configuration, identity, catalog, policy, operations, finance, reporting**.
...
|---|---|
| `WebAdminRouteRegistry` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebAdminRouteRegistryVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebAdminRouteRegistryDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebAdminRouteRegistryEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebAdminRouteRegistryException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
WebAdminRouteRegistry {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateWebAdminRouteRegistry`
- `ValidateWebAdminRouteRegistry`
- `ApproveWebAdminRouteRegistry`
- `ActivateWebAdminRouteRegistry`
- `SuspendWebAdminRouteRegistry`
- `CorrectWebAdminRouteRegistry`
- `RetireWebAdminRouteRegistry`

Reference Events:

- `WebAdminRouteRegistryCreated`
- `WebAdminRouteRegistryValidated`
- `WebAdminRouteRegistryApproved`
- `WebAdminRouteRegistryActivated`
- `WebAdminRouteRegistrySuspended`
- `WebAdminRouteRegistryCorrected`
- `WebAdminRouteRegistryRetired`

Reference queries:

- `GetWebAdminRouteRegistry`
- `ListWebAdminRouteRegistryRecords`
- `GetWebAdminRouteRegistryTimeline`
- `GetWebAdminRouteRegistryEvidence`
- `GetWebAdminRouteRegistryDecisionPackage`
- `GetWebAdminRouteRegistryExceptions`
- `GetWebAdminRouteRegistryAuditTrail`

Reference API surface:

```text
POST /api/v1/webadminrouteregistry/commands
GET  /api/v1/webadminrouteregistry/{id}
GET  /api/v1/webadminrouteregistry/{id}/timeline
GET  /api/v1/webadminrouteregistry/{id}/evidence
GET  /api/v1/webadminrouteregistry/{id}/decisions
POST /api/v1/webadminrouteregistry/{id}/exceptions
POST /api/v1/webadminrouteregistry/{id}/corrections
```
...
|---|---|
| `webadminrouteregistry_aggregate` | authoritative subject |
| `webadminrouteregistry_version` | immutable submitted, approved, issued, or signed versions |
| `webadminrouteregistry_relationship` | governed relationships |
| `webadminrouteregistry_decision` | binding Decisions and rationale |
| `webadminrouteregistry_evidence_link` | provenance and evidence relationships |
| `webadminrouteregistry_event_outbox` | transactional Event publication |
| `webadminrouteregistry_idempotency` | duplicate-write protection |
| `webadminrouteregistry_exception` | exception, claim, dispute, or hold |
| `webadminrouteregistry_correction` | correction and supersession lineage |
| `webadminrouteregistry_audit` | privileged action evidence |
