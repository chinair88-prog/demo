# Portal Cross-Route and Persona Matrix

| Metadata | Value |
|---|---|
| Volume | `4` |
| Chapter ID | `V04-SYNTH` |
| Subject | `PortalMatrix` |
...
| Accountable owner | Experience Architecture Council |
...
This chapter defines how GTOS shall **connect every Portal persona, route, page, permission, Domain dependency, realtime channel, AI interaction, analytics signal and E2E journey**.
...
The accountable owner is **Experience Architecture Council**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `PortalMatrix` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PortalMatrixVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PortalMatrixDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PortalMatrixEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PortalMatrixException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreatePortalMatrix`
- `ValidatePortalMatrix`
- `ApprovePortalMatrix`
- `ActivatePortalMatrix`
- `SuspendPortalMatrix`
- `CorrectPortalMatrix`
- `RetirePortalMatrix`
...
- `PortalMatrixCreated`
- `PortalMatrixValidated`
- `PortalMatrixApproved`
- `PortalMatrixActivated`
- `PortalMatrixSuspended`
- `PortalMatrixCorrected`
- `PortalMatrixRetired`

Reference queries:

- `GetPortalMatrix`
- `ListPortalMatrixRecords`
- `GetPortalMatrixTimeline`
- `GetPortalMatrixEvidence`
- `GetPortalMatrixDecisionPackage`
- `GetPortalMatrixExceptions`
- `GetPortalMatrixAuditTrail`

Reference API surface:

```text
POST /api/v1/portalmatrix/commands
GET  /api/v1/portalmatrix/{id}
GET  /api/v1/portalmatrix/{id}/timeline
GET  /api/v1/portalmatrix/{id}/evidence
GET  /api/v1/portalmatrix/{id}/decisions
POST /api/v1/portalmatrix/{id}/exceptions
POST /api/v1/portalmatrix/{id}/corrections
```
...
|---|---|
| `portalmatrix_aggregate` | authoritative subject |
| `portalmatrix_version` | immutable submitted, approved, issued, or signed versions |
| `portalmatrix_relationship` | governed relationships |
| `portalmatrix_decision` | binding Decisions and rationale |
| `portalmatrix_evidence_link` | provenance and evidence relationships |
| `portalmatrix_event_outbox` | transactional Event publication |
| `portalmatrix_idempotency` | duplicate-write protection |
| `portalmatrix_exception` | exception, claim, dispute, or hold |
| `portalmatrix_correction` | correction and supersession lineage |
| `portalmatrix_audit` | privileged action evidence |
