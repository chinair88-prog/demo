# Scenario Simulator — UX States, Accessibility, Localization, Analytics, and E2E

| Metadata | Value |
|---|---|
| Volume | `4` |
| Chapter ID | `V04-P12-03` |
| Subject | `ScenarioSimulatorExperienceStandard` |
...
| Accountable owner | Portal Architecture / Scenario Simulator |
...
This chapter defines how GTOS shall **specify loading, empty, partial, stale, error, denied, offline, accessibility, RTL/LTR, analytics, audit and test journeys for planners, analysts and executives, covering assumptions, scenarios, simulations, sensitivity, comparison, Decision support**.
...
The accountable owner is **Portal Architecture / Scenario Simulator**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `ScenarioSimulatorExperienceStandard` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ScenarioSimulatorExperienceStandardVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ScenarioSimulatorExperienceStandardDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ScenarioSimulatorExperienceStandardEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ScenarioSimulatorExperienceStandardException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ScenarioSimulatorExperienceStandard {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateScenarioSimulatorExperienceStandard`
- `ValidateScenarioSimulatorExperienceStandard`
- `ApproveScenarioSimulatorExperienceStandard`
- `ActivateScenarioSimulatorExperienceStandard`
- `SuspendScenarioSimulatorExperienceStandard`
- `CorrectScenarioSimulatorExperienceStandard`
- `RetireScenarioSimulatorExperienceStandard`

Reference Events:

- `ScenarioSimulatorExperienceStandardCreated`
- `ScenarioSimulatorExperienceStandardValidated`
- `ScenarioSimulatorExperienceStandardApproved`
- `ScenarioSimulatorExperienceStandardActivated`
- `ScenarioSimulatorExperienceStandardSuspended`
- `ScenarioSimulatorExperienceStandardCorrected`
- `ScenarioSimulatorExperienceStandardRetired`

Reference queries:

- `GetScenarioSimulatorExperienceStandard`
- `ListScenarioSimulatorExperienceStandardRecords`
- `GetScenarioSimulatorExperienceStandardTimeline`
- `GetScenarioSimulatorExperienceStandardEvidence`
- `GetScenarioSimulatorExperienceStandardDecisionPackage`
- `GetScenarioSimulatorExperienceStandardExceptions`
- `GetScenarioSimulatorExperienceStandardAuditTrail`

Reference API surface:

```text
POST /api/v1/scenariosimulatorexperiencestandard/commands
GET  /api/v1/scenariosimulatorexperiencestandard/{id}
GET  /api/v1/scenariosimulatorexperiencestandard/{id}/timeline
GET  /api/v1/scenariosimulatorexperiencestandard/{id}/evidence
GET  /api/v1/scenariosimulatorexperiencestandard/{id}/decisions
POST /api/v1/scenariosimulatorexperiencestandard/{id}/exceptions
POST /api/v1/scenariosimulatorexperiencestandard/{id}/corrections
```
...
|---|---|
| `scenariosimulatorexperiencestandard_aggregate` | authoritative subject |
| `scenariosimulatorexperiencestandard_version` | immutable submitted, approved, issued, or signed versions |
| `scenariosimulatorexperiencestandard_relationship` | governed relationships |
| `scenariosimulatorexperiencestandard_decision` | binding Decisions and rationale |
| `scenariosimulatorexperiencestandard_evidence_link` | provenance and evidence relationships |
| `scenariosimulatorexperiencestandard_event_outbox` | transactional Event publication |
| `scenariosimulatorexperiencestandard_idempotency` | duplicate-write protection |
| `scenariosimulatorexperiencestandard_exception` | exception, claim, dispute, or hold |
| `scenariosimulatorexperiencestandard_correction` | correction and supersession lineage |
| `scenariosimulatorexperiencestandard_audit` | privileged action evidence |
...
- Scenario Simulator: primary experience for planners, analysts and executives.
