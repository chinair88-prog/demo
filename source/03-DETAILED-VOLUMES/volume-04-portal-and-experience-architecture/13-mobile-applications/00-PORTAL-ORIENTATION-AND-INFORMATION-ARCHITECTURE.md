# Mobile Applications — Portal Orientation and Information Architecture

| Metadata | Value |
|---|---|
| Volume | `4` |
| Chapter ID | `V04-P13-00` |
| Subject | `MobileApplicationsPortal` |
...
This chapter defines how GTOS shall **define personas, navigation, route ownership, domain dependencies and experience boundaries for customers, partners, providers, warehouse and field users, covering offline capture, notifications, scanning, evidence, approvals, tracking**.
...
|---|---|
| `MobileApplicationsPortal` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MobileApplicationsPortalVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MobileApplicationsPortalDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MobileApplicationsPortalEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MobileApplicationsPortalException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
MobileApplicationsPortal {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateMobileApplicationsPortal`
- `ValidateMobileApplicationsPortal`
- `ApproveMobileApplicationsPortal`
- `ActivateMobileApplicationsPortal`
- `SuspendMobileApplicationsPortal`
- `CorrectMobileApplicationsPortal`
- `RetireMobileApplicationsPortal`

Reference Events:

- `MobileApplicationsPortalCreated`
- `MobileApplicationsPortalValidated`
- `MobileApplicationsPortalApproved`
- `MobileApplicationsPortalActivated`
- `MobileApplicationsPortalSuspended`
- `MobileApplicationsPortalCorrected`
- `MobileApplicationsPortalRetired`

Reference queries:

- `GetMobileApplicationsPortal`
- `ListMobileApplicationsPortalRecords`
- `GetMobileApplicationsPortalTimeline`
- `GetMobileApplicationsPortalEvidence`
- `GetMobileApplicationsPortalDecisionPackage`
- `GetMobileApplicationsPortalExceptions`
- `GetMobileApplicationsPortalAuditTrail`

Reference API surface:

```text
POST /api/v1/mobileapplicationsportal/commands
GET  /api/v1/mobileapplicationsportal/{id}
GET  /api/v1/mobileapplicationsportal/{id}/timeline
GET  /api/v1/mobileapplicationsportal/{id}/evidence
GET  /api/v1/mobileapplicationsportal/{id}/decisions
POST /api/v1/mobileapplicationsportal/{id}/exceptions
POST /api/v1/mobileapplicationsportal/{id}/corrections
```
...
|---|---|
| `mobileapplicationsportal_aggregate` | authoritative subject |
| `mobileapplicationsportal_version` | immutable submitted, approved, issued, or signed versions |
| `mobileapplicationsportal_relationship` | governed relationships |
| `mobileapplicationsportal_decision` | binding Decisions and rationale |
| `mobileapplicationsportal_evidence_link` | provenance and evidence relationships |
| `mobileapplicationsportal_event_outbox` | transactional Event publication |
| `mobileapplicationsportal_idempotency` | duplicate-write protection |
| `mobileapplicationsportal_exception` | exception, claim, dispute, or hold |
| `mobileapplicationsportal_correction` | correction and supersession lineage |
| `mobileapplicationsportal_audit` | privileged action evidence |
