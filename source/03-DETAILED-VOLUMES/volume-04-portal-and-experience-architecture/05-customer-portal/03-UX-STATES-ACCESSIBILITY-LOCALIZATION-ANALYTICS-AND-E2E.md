# Customer Portal — UX States, Accessibility, Localization, Analytics, and E2E

| Metadata | Value |
|---|---|
| Volume | `4` |
| Chapter ID | `V04-P05-03` |
| Subject | `CustomerPortalExperienceStandard` |
...
| Accountable owner | Portal Architecture / Customer Portal |
...
This chapter defines how GTOS shall **specify loading, empty, partial, stale, error, denied, offline, accessibility, RTL/LTR, analytics, audit and test journeys for buyer organizations and users, covering identity, demand, RFQ, quotation, PI, payment, PO, production, quality, shipping, inventory, finance**.
...
The accountable owner is **Portal Architecture / Customer Portal**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `CustomerPortalExperienceStandard` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CustomerPortalExperienceStandardVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CustomerPortalExperienceStandardDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CustomerPortalExperienceStandardEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CustomerPortalExperienceStandardException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
CustomerPortalExperienceStandard {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateCustomerPortalExperienceStandard`
- `ValidateCustomerPortalExperienceStandard`
- `ApproveCustomerPortalExperienceStandard`
- `ActivateCustomerPortalExperienceStandard`
- `SuspendCustomerPortalExperienceStandard`
- `CorrectCustomerPortalExperienceStandard`
- `RetireCustomerPortalExperienceStandard`

Reference Events:

- `CustomerPortalExperienceStandardCreated`
- `CustomerPortalExperienceStandardValidated`
- `CustomerPortalExperienceStandardApproved`
- `CustomerPortalExperienceStandardActivated`
- `CustomerPortalExperienceStandardSuspended`
- `CustomerPortalExperienceStandardCorrected`
- `CustomerPortalExperienceStandardRetired`

Reference queries:

- `GetCustomerPortalExperienceStandard`
- `ListCustomerPortalExperienceStandardRecords`
- `GetCustomerPortalExperienceStandardTimeline`
- `GetCustomerPortalExperienceStandardEvidence`
- `GetCustomerPortalExperienceStandardDecisionPackage`
- `GetCustomerPortalExperienceStandardExceptions`
- `GetCustomerPortalExperienceStandardAuditTrail`

Reference API surface:

```text
POST /api/v1/customerportalexperiencestandard/commands
GET  /api/v1/customerportalexperiencestandard/{id}
GET  /api/v1/customerportalexperiencestandard/{id}/timeline
GET  /api/v1/customerportalexperiencestandard/{id}/evidence
GET  /api/v1/customerportalexperiencestandard/{id}/decisions
POST /api/v1/customerportalexperiencestandard/{id}/exceptions
POST /api/v1/customerportalexperiencestandard/{id}/corrections
```
...
|---|---|
| `customerportalexperiencestandard_aggregate` | authoritative subject |
| `customerportalexperiencestandard_version` | immutable submitted, approved, issued, or signed versions |
| `customerportalexperiencestandard_relationship` | governed relationships |
| `customerportalexperiencestandard_decision` | binding Decisions and rationale |
| `customerportalexperiencestandard_evidence_link` | provenance and evidence relationships |
| `customerportalexperiencestandard_event_outbox` | transactional Event publication |
| `customerportalexperiencestandard_idempotency` | duplicate-write protection |
| `customerportalexperiencestandard_exception` | exception, claim, dispute, or hold |
| `customerportalexperiencestandard_correction` | correction and supersession lineage |
| `customerportalexperiencestandard_audit` | privileged action evidence |
...
- Customer Portal: primary experience for buyer organizations and users.
