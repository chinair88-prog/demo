# Purchase Order — Portals, AI, Security, SLOs, Tests, and Operations

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-08-04` |
| Subject | `PurchaseOrderOperatingProfile` |
...
| Accountable owner | Commerce Architecture / Purchase Order |
...
This chapter defines how GTOS shall **define Portal journeys, AI participation, trust boundaries, SLOs, runbooks, tests, migration, examples, and conformance for Purchase Order**.
...
The accountable owner is **Commerce Architecture / Purchase Order**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `PurchaseOrderOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PurchaseOrderOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PurchaseOrderOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PurchaseOrderOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PurchaseOrderOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
PurchaseOrderOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreatePurchaseOrderOperatingProfile`
- `ValidatePurchaseOrderOperatingProfile`
- `ApprovePurchaseOrderOperatingProfile`
- `ActivatePurchaseOrderOperatingProfile`
- `SuspendPurchaseOrderOperatingProfile`
- `CorrectPurchaseOrderOperatingProfile`
- `RetirePurchaseOrderOperatingProfile`

Reference Events:

- `PurchaseOrderOperatingProfileCreated`
- `PurchaseOrderOperatingProfileValidated`
- `PurchaseOrderOperatingProfileApproved`
- `PurchaseOrderOperatingProfileActivated`
- `PurchaseOrderOperatingProfileSuspended`
- `PurchaseOrderOperatingProfileCorrected`
- `PurchaseOrderOperatingProfileRetired`

Reference queries:

- `GetPurchaseOrderOperatingProfile`
- `ListPurchaseOrderOperatingProfileRecords`
- `GetPurchaseOrderOperatingProfileTimeline`
- `GetPurchaseOrderOperatingProfileEvidence`
- `GetPurchaseOrderOperatingProfileDecisionPackage`
- `GetPurchaseOrderOperatingProfileExceptions`
- `GetPurchaseOrderOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/purchaseorderoperatingprofile/commands
GET  /api/v1/purchaseorderoperatingprofile/{id}
GET  /api/v1/purchaseorderoperatingprofile/{id}/timeline
GET  /api/v1/purchaseorderoperatingprofile/{id}/evidence
GET  /api/v1/purchaseorderoperatingprofile/{id}/decisions
POST /api/v1/purchaseorderoperatingprofile/{id}/exceptions
POST /api/v1/purchaseorderoperatingprofile/{id}/corrections
```
...
|---|---|
| `purchaseorderoperatingprofile_aggregate` | authoritative subject |
| `purchaseorderoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `purchaseorderoperatingprofile_relationship` | governed relationships |
| `purchaseorderoperatingprofile_decision` | binding Decisions and rationale |
| `purchaseorderoperatingprofile_evidence_link` | provenance and evidence relationships |
| `purchaseorderoperatingprofile_event_outbox` | transactional Event publication |
| `purchaseorderoperatingprofile_idempotency` | duplicate-write protection |
| `purchaseorderoperatingprofile_exception` | exception, claim, dispute, or hold |
| `purchaseorderoperatingprofile_correction` | correction and supersession lineage |
| `purchaseorderoperatingprofile_audit` | privileged action evidence |
...
- Named capability `Purchase Order` is part of the target handbook scope.
