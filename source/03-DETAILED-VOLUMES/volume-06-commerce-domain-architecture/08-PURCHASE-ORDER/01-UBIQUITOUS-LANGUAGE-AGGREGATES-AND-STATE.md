# Purchase Order — Ubiquitous Language, Aggregates, and State

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-08-01` |
| Subject | `PurchaseOrderAggregateModel` |
...
This chapter defines how GTOS shall **define entities, value objects, relationships, aggregates, invariants, lifecycle state, effective time, correction, and concurrency for Purchase Order**.
...
|---|---|
| `PurchaseOrderAggregateModel` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PurchaseOrderAggregateModelVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PurchaseOrderAggregateModelDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PurchaseOrderAggregateModelEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PurchaseOrderAggregateModelException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
PurchaseOrderAggregateModel {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreatePurchaseOrderAggregateModel`
- `ValidatePurchaseOrderAggregateModel`
- `ApprovePurchaseOrderAggregateModel`
- `ActivatePurchaseOrderAggregateModel`
- `SuspendPurchaseOrderAggregateModel`
- `CorrectPurchaseOrderAggregateModel`
- `RetirePurchaseOrderAggregateModel`

Reference Events:

- `PurchaseOrderAggregateModelCreated`
- `PurchaseOrderAggregateModelValidated`
- `PurchaseOrderAggregateModelApproved`
- `PurchaseOrderAggregateModelActivated`
- `PurchaseOrderAggregateModelSuspended`
- `PurchaseOrderAggregateModelCorrected`
- `PurchaseOrderAggregateModelRetired`

Reference queries:

- `GetPurchaseOrderAggregateModel`
- `ListPurchaseOrderAggregateModelRecords`
- `GetPurchaseOrderAggregateModelTimeline`
- `GetPurchaseOrderAggregateModelEvidence`
- `GetPurchaseOrderAggregateModelDecisionPackage`
- `GetPurchaseOrderAggregateModelExceptions`
- `GetPurchaseOrderAggregateModelAuditTrail`

Reference API surface:

```text
POST /api/v1/purchaseorderaggregatemodel/commands
GET  /api/v1/purchaseorderaggregatemodel/{id}
GET  /api/v1/purchaseorderaggregatemodel/{id}/timeline
GET  /api/v1/purchaseorderaggregatemodel/{id}/evidence
GET  /api/v1/purchaseorderaggregatemodel/{id}/decisions
POST /api/v1/purchaseorderaggregatemodel/{id}/exceptions
POST /api/v1/purchaseorderaggregatemodel/{id}/corrections
```
...
|---|---|
| `purchaseorderaggregatemodel_aggregate` | authoritative subject |
| `purchaseorderaggregatemodel_version` | immutable submitted, approved, issued, or signed versions |
| `purchaseorderaggregatemodel_relationship` | governed relationships |
| `purchaseorderaggregatemodel_decision` | binding Decisions and rationale |
| `purchaseorderaggregatemodel_evidence_link` | provenance and evidence relationships |
| `purchaseorderaggregatemodel_event_outbox` | transactional Event publication |
| `purchaseorderaggregatemodel_idempotency` | duplicate-write protection |
| `purchaseorderaggregatemodel_exception` | exception, claim, dispute, or hold |
| `purchaseorderaggregatemodel_correction` | correction and supersession lineage |
| `purchaseorderaggregatemodel_audit` | privileged action evidence |
