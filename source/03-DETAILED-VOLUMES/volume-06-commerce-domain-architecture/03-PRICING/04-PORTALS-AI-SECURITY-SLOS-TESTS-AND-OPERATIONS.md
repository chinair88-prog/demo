# Pricing — Portals, AI, Security, SLOs, Tests, and Operations

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-03-04` |
| Subject | `PricingOperatingProfile` |
...
| Accountable owner | Commerce Architecture / Pricing |
...
This chapter defines how GTOS shall **define Portal journeys, AI participation, trust boundaries, SLOs, runbooks, tests, migration, examples, and conformance for Pricing**.
...
The accountable owner is **Commerce Architecture / Pricing**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `PricingOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PricingOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PricingOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PricingOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PricingOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
PricingOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreatePricingOperatingProfile`
- `ValidatePricingOperatingProfile`
- `ApprovePricingOperatingProfile`
- `ActivatePricingOperatingProfile`
- `SuspendPricingOperatingProfile`
- `CorrectPricingOperatingProfile`
- `RetirePricingOperatingProfile`

Reference Events:

- `PricingOperatingProfileCreated`
- `PricingOperatingProfileValidated`
- `PricingOperatingProfileApproved`
- `PricingOperatingProfileActivated`
- `PricingOperatingProfileSuspended`
- `PricingOperatingProfileCorrected`
- `PricingOperatingProfileRetired`

Reference queries:

- `GetPricingOperatingProfile`
- `ListPricingOperatingProfileRecords`
- `GetPricingOperatingProfileTimeline`
- `GetPricingOperatingProfileEvidence`
- `GetPricingOperatingProfileDecisionPackage`
- `GetPricingOperatingProfileExceptions`
- `GetPricingOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/pricingoperatingprofile/commands
GET  /api/v1/pricingoperatingprofile/{id}
GET  /api/v1/pricingoperatingprofile/{id}/timeline
GET  /api/v1/pricingoperatingprofile/{id}/evidence
GET  /api/v1/pricingoperatingprofile/{id}/decisions
POST /api/v1/pricingoperatingprofile/{id}/exceptions
POST /api/v1/pricingoperatingprofile/{id}/corrections
```
...
|---|---|
| `pricingoperatingprofile_aggregate` | authoritative subject |
| `pricingoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `pricingoperatingprofile_relationship` | governed relationships |
| `pricingoperatingprofile_decision` | binding Decisions and rationale |
| `pricingoperatingprofile_evidence_link` | provenance and evidence relationships |
| `pricingoperatingprofile_event_outbox` | transactional Event publication |
| `pricingoperatingprofile_idempotency` | duplicate-write protection |
| `pricingoperatingprofile_exception` | exception, claim, dispute, or hold |
| `pricingoperatingprofile_correction` | correction and supersession lineage |
| `pricingoperatingprofile_audit` | privileged action evidence |
...
- Named capability `Pricing` is part of the target handbook scope.
