# Pricing — Orientation and Evidence Baseline

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-03-00` |
| Subject | `PricingDomain` |
...
This chapter defines how GTOS shall **define the scope, non-goals, module and database evidence requirements, ownership boundaries, current status, and target responsibilities of Pricing**.
...
|---|---|
| `PricingDomain` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PricingDomainVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PricingDomainDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PricingDomainEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PricingDomainException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreatePricingDomain`
- `ValidatePricingDomain`
- `ApprovePricingDomain`
- `ActivatePricingDomain`
- `SuspendPricingDomain`
- `CorrectPricingDomain`
- `RetirePricingDomain`

Reference Events:

- `PricingDomainCreated`
- `PricingDomainValidated`
- `PricingDomainApproved`
- `PricingDomainActivated`
- `PricingDomainSuspended`
- `PricingDomainCorrected`
- `PricingDomainRetired`

Reference queries:

- `GetPricingDomain`
- `ListPricingDomainRecords`
- `GetPricingDomainTimeline`
- `GetPricingDomainEvidence`
- `GetPricingDomainDecisionPackage`
- `GetPricingDomainExceptions`
- `GetPricingDomainAuditTrail`

Reference API surface:

```text
POST /api/v1/pricingdomain/commands
GET  /api/v1/pricingdomain/{id}
GET  /api/v1/pricingdomain/{id}/timeline
GET  /api/v1/pricingdomain/{id}/evidence
GET  /api/v1/pricingdomain/{id}/decisions
POST /api/v1/pricingdomain/{id}/exceptions
POST /api/v1/pricingdomain/{id}/corrections
```
...
|---|---|
| `pricingdomain_aggregate` | authoritative subject |
| `pricingdomain_version` | immutable submitted, approved, issued, or signed versions |
| `pricingdomain_relationship` | governed relationships |
| `pricingdomain_decision` | binding Decisions and rationale |
| `pricingdomain_evidence_link` | provenance and evidence relationships |
| `pricingdomain_event_outbox` | transactional Event publication |
| `pricingdomain_idempotency` | duplicate-write protection |
| `pricingdomain_exception` | exception, claim, dispute, or hold |
| `pricingdomain_correction` | correction and supersession lineage |
| `pricingdomain_audit` | privileged action evidence |
