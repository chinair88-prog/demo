# RFQ and Procurement — Ubiquitous Language, Aggregates, and State

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-04-01` |
| Subject | `RFQandProcurementAggregateModel` |
...
This chapter defines how GTOS shall **define entities, value objects, relationships, aggregates, invariants, lifecycle state, effective time, correction, and concurrency for RFQ and Procurement**.
...
|---|---|
| `RFQandProcurementAggregateModel` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementAggregateModelVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementAggregateModelDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementAggregateModelEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementAggregateModelException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
RFQandProcurementAggregateModel {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateRFQandProcurementAggregateModel`
- `ValidateRFQandProcurementAggregateModel`
- `ApproveRFQandProcurementAggregateModel`
- `ActivateRFQandProcurementAggregateModel`
- `SuspendRFQandProcurementAggregateModel`
- `CorrectRFQandProcurementAggregateModel`
- `RetireRFQandProcurementAggregateModel`

Reference Events:

- `RFQandProcurementAggregateModelCreated`
- `RFQandProcurementAggregateModelValidated`
- `RFQandProcurementAggregateModelApproved`
- `RFQandProcurementAggregateModelActivated`
- `RFQandProcurementAggregateModelSuspended`
- `RFQandProcurementAggregateModelCorrected`
- `RFQandProcurementAggregateModelRetired`

Reference queries:

- `GetRFQandProcurementAggregateModel`
- `ListRFQandProcurementAggregateModelRecords`
- `GetRFQandProcurementAggregateModelTimeline`
- `GetRFQandProcurementAggregateModelEvidence`
- `GetRFQandProcurementAggregateModelDecisionPackage`
- `GetRFQandProcurementAggregateModelExceptions`
- `GetRFQandProcurementAggregateModelAuditTrail`

Reference API surface:

```text
POST /api/v1/rfqandprocurementaggregatemodel/commands
GET  /api/v1/rfqandprocurementaggregatemodel/{id}
GET  /api/v1/rfqandprocurementaggregatemodel/{id}/timeline
GET  /api/v1/rfqandprocurementaggregatemodel/{id}/evidence
GET  /api/v1/rfqandprocurementaggregatemodel/{id}/decisions
POST /api/v1/rfqandprocurementaggregatemodel/{id}/exceptions
POST /api/v1/rfqandprocurementaggregatemodel/{id}/corrections
```
...
|---|---|
| `rfqandprocurementaggregatemodel_aggregate` | authoritative subject |
| `rfqandprocurementaggregatemodel_version` | immutable submitted, approved, issued, or signed versions |
| `rfqandprocurementaggregatemodel_relationship` | governed relationships |
| `rfqandprocurementaggregatemodel_decision` | binding Decisions and rationale |
| `rfqandprocurementaggregatemodel_evidence_link` | provenance and evidence relationships |
| `rfqandprocurementaggregatemodel_event_outbox` | transactional Event publication |
| `rfqandprocurementaggregatemodel_idempotency` | duplicate-write protection |
| `rfqandprocurementaggregatemodel_exception` | exception, claim, dispute, or hold |
| `rfqandprocurementaggregatemodel_correction` | correction and supersession lineage |
| `rfqandprocurementaggregatemodel_audit` | privileged action evidence |
