# RFQ and Procurement — Portals, AI, Security, SLOs, Tests, and Operations

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-04-04` |
| Subject | `RFQandProcurementOperatingProfile` |
...
| Accountable owner | Commerce Architecture / RFQ and Procurement |
...
This chapter defines how GTOS shall **define Portal journeys, AI participation, trust boundaries, SLOs, runbooks, tests, migration, examples, and conformance for RFQ and Procurement**.
...
The accountable owner is **Commerce Architecture / RFQ and Procurement**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `RFQandProcurementOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
RFQandProcurementOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateRFQandProcurementOperatingProfile`
- `ValidateRFQandProcurementOperatingProfile`
- `ApproveRFQandProcurementOperatingProfile`
- `ActivateRFQandProcurementOperatingProfile`
- `SuspendRFQandProcurementOperatingProfile`
- `CorrectRFQandProcurementOperatingProfile`
- `RetireRFQandProcurementOperatingProfile`

Reference Events:

- `RFQandProcurementOperatingProfileCreated`
- `RFQandProcurementOperatingProfileValidated`
- `RFQandProcurementOperatingProfileApproved`
- `RFQandProcurementOperatingProfileActivated`
- `RFQandProcurementOperatingProfileSuspended`
- `RFQandProcurementOperatingProfileCorrected`
- `RFQandProcurementOperatingProfileRetired`

Reference queries:

- `GetRFQandProcurementOperatingProfile`
- `ListRFQandProcurementOperatingProfileRecords`
- `GetRFQandProcurementOperatingProfileTimeline`
- `GetRFQandProcurementOperatingProfileEvidence`
- `GetRFQandProcurementOperatingProfileDecisionPackage`
- `GetRFQandProcurementOperatingProfileExceptions`
- `GetRFQandProcurementOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/rfqandprocurementoperatingprofile/commands
GET  /api/v1/rfqandprocurementoperatingprofile/{id}
GET  /api/v1/rfqandprocurementoperatingprofile/{id}/timeline
GET  /api/v1/rfqandprocurementoperatingprofile/{id}/evidence
GET  /api/v1/rfqandprocurementoperatingprofile/{id}/decisions
POST /api/v1/rfqandprocurementoperatingprofile/{id}/exceptions
POST /api/v1/rfqandprocurementoperatingprofile/{id}/corrections
```
...
|---|---|
| `rfqandprocurementoperatingprofile_aggregate` | authoritative subject |
| `rfqandprocurementoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `rfqandprocurementoperatingprofile_relationship` | governed relationships |
| `rfqandprocurementoperatingprofile_decision` | binding Decisions and rationale |
| `rfqandprocurementoperatingprofile_evidence_link` | provenance and evidence relationships |
| `rfqandprocurementoperatingprofile_event_outbox` | transactional Event publication |
| `rfqandprocurementoperatingprofile_idempotency` | duplicate-write protection |
| `rfqandprocurementoperatingprofile_exception` | exception, claim, dispute, or hold |
| `rfqandprocurementoperatingprofile_correction` | correction and supersession lineage |
| `rfqandprocurementoperatingprofile_audit` | privileged action evidence |
...
- Named capability `RFQ and Procurement` is part of the target handbook scope.
