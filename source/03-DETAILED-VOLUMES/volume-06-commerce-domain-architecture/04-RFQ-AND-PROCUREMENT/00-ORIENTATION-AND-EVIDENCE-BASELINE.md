# RFQ and Procurement — Orientation and Evidence Baseline

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-04-00` |
| Subject | `RFQandProcurementDomain` |
...
This chapter defines how GTOS shall **define the scope, non-goals, module and database evidence requirements, ownership boundaries, current status, and target responsibilities of RFQ and Procurement**.
...
|---|---|
| `RFQandProcurementDomain` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementDomainVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementDomainDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementDomainEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementDomainException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
RFQandProcurementDomain {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateRFQandProcurementDomain`
- `ValidateRFQandProcurementDomain`
- `ApproveRFQandProcurementDomain`
- `ActivateRFQandProcurementDomain`
- `SuspendRFQandProcurementDomain`
- `CorrectRFQandProcurementDomain`
- `RetireRFQandProcurementDomain`

Reference Events:

- `RFQandProcurementDomainCreated`
- `RFQandProcurementDomainValidated`
- `RFQandProcurementDomainApproved`
- `RFQandProcurementDomainActivated`
- `RFQandProcurementDomainSuspended`
- `RFQandProcurementDomainCorrected`
- `RFQandProcurementDomainRetired`

Reference queries:

- `GetRFQandProcurementDomain`
- `ListRFQandProcurementDomainRecords`
- `GetRFQandProcurementDomainTimeline`
- `GetRFQandProcurementDomainEvidence`
- `GetRFQandProcurementDomainDecisionPackage`
- `GetRFQandProcurementDomainExceptions`
- `GetRFQandProcurementDomainAuditTrail`

Reference API surface:

```text
POST /api/v1/rfqandprocurementdomain/commands
GET  /api/v1/rfqandprocurementdomain/{id}
GET  /api/v1/rfqandprocurementdomain/{id}/timeline
GET  /api/v1/rfqandprocurementdomain/{id}/evidence
GET  /api/v1/rfqandprocurementdomain/{id}/decisions
POST /api/v1/rfqandprocurementdomain/{id}/exceptions
POST /api/v1/rfqandprocurementdomain/{id}/corrections
```
...
|---|---|
| `rfqandprocurementdomain_aggregate` | authoritative subject |
| `rfqandprocurementdomain_version` | immutable submitted, approved, issued, or signed versions |
| `rfqandprocurementdomain_relationship` | governed relationships |
| `rfqandprocurementdomain_decision` | binding Decisions and rationale |
| `rfqandprocurementdomain_evidence_link` | provenance and evidence relationships |
| `rfqandprocurementdomain_event_outbox` | transactional Event publication |
| `rfqandprocurementdomain_idempotency` | duplicate-write protection |
| `rfqandprocurementdomain_exception` | exception, claim, dispute, or hold |
| `rfqandprocurementdomain_correction` | correction and supersession lineage |
| `rfqandprocurementdomain_audit` | privileged action evidence |
