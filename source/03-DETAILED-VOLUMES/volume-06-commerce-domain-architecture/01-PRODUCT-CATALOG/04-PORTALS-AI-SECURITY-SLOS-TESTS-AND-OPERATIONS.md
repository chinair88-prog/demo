# Product Catalog — Portals, AI, Security, SLOs, Tests, and Operations

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-01-04` |
| Subject | `ProductCatalogOperatingProfile` |
...
| Accountable owner | Commerce Architecture / Product Catalog |
...
This chapter defines how GTOS shall **define Portal journeys, AI participation, trust boundaries, SLOs, runbooks, tests, migration, examples, and conformance for Product Catalog**.
...
The accountable owner is **Commerce Architecture / Product Catalog**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `ProductCatalogOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProductCatalogOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProductCatalogOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProductCatalogOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProductCatalogOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ProductCatalogOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateProductCatalogOperatingProfile`
- `ValidateProductCatalogOperatingProfile`
- `ApproveProductCatalogOperatingProfile`
- `ActivateProductCatalogOperatingProfile`
- `SuspendProductCatalogOperatingProfile`
- `CorrectProductCatalogOperatingProfile`
- `RetireProductCatalogOperatingProfile`

Reference Events:

- `ProductCatalogOperatingProfileCreated`
- `ProductCatalogOperatingProfileValidated`
- `ProductCatalogOperatingProfileApproved`
- `ProductCatalogOperatingProfileActivated`
- `ProductCatalogOperatingProfileSuspended`
- `ProductCatalogOperatingProfileCorrected`
- `ProductCatalogOperatingProfileRetired`

Reference queries:

- `GetProductCatalogOperatingProfile`
- `ListProductCatalogOperatingProfileRecords`
- `GetProductCatalogOperatingProfileTimeline`
- `GetProductCatalogOperatingProfileEvidence`
- `GetProductCatalogOperatingProfileDecisionPackage`
- `GetProductCatalogOperatingProfileExceptions`
- `GetProductCatalogOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/productcatalogoperatingprofile/commands
GET  /api/v1/productcatalogoperatingprofile/{id}
GET  /api/v1/productcatalogoperatingprofile/{id}/timeline
GET  /api/v1/productcatalogoperatingprofile/{id}/evidence
GET  /api/v1/productcatalogoperatingprofile/{id}/decisions
POST /api/v1/productcatalogoperatingprofile/{id}/exceptions
POST /api/v1/productcatalogoperatingprofile/{id}/corrections
```
...
|---|---|
| `productcatalogoperatingprofile_aggregate` | authoritative subject |
| `productcatalogoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `productcatalogoperatingprofile_relationship` | governed relationships |
| `productcatalogoperatingprofile_decision` | binding Decisions and rationale |
| `productcatalogoperatingprofile_evidence_link` | provenance and evidence relationships |
| `productcatalogoperatingprofile_event_outbox` | transactional Event publication |
| `productcatalogoperatingprofile_idempotency` | duplicate-write protection |
| `productcatalogoperatingprofile_exception` | exception, claim, dispute, or hold |
| `productcatalogoperatingprofile_correction` | correction and supersession lineage |
| `productcatalogoperatingprofile_audit` | privileged action evidence |
...
- Named capability `Product Catalog` is part of the target handbook scope.
