# Contract — Portals, AI, Security, SLOs, Tests, and Operations

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-06-04` |
| Subject | `ContractOperatingProfile` |
...
| Accountable owner | Commerce Architecture / Contract |
...
This chapter defines how GTOS shall **define Portal journeys, AI participation, trust boundaries, SLOs, runbooks, tests, migration, examples, and conformance for Contract**.
...
The accountable owner is **Commerce Architecture / Contract**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `ContractOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ContractOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ContractOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ContractOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ContractOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ContractOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateContractOperatingProfile`
- `ValidateContractOperatingProfile`
- `ApproveContractOperatingProfile`
- `ActivateContractOperatingProfile`
- `SuspendContractOperatingProfile`
- `CorrectContractOperatingProfile`
- `RetireContractOperatingProfile`

Reference Events:

- `ContractOperatingProfileCreated`
- `ContractOperatingProfileValidated`
- `ContractOperatingProfileApproved`
- `ContractOperatingProfileActivated`
- `ContractOperatingProfileSuspended`
- `ContractOperatingProfileCorrected`
- `ContractOperatingProfileRetired`

Reference queries:

- `GetContractOperatingProfile`
- `ListContractOperatingProfileRecords`
- `GetContractOperatingProfileTimeline`
- `GetContractOperatingProfileEvidence`
- `GetContractOperatingProfileDecisionPackage`
- `GetContractOperatingProfileExceptions`
- `GetContractOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/contractoperatingprofile/commands
GET  /api/v1/contractoperatingprofile/{id}
GET  /api/v1/contractoperatingprofile/{id}/timeline
GET  /api/v1/contractoperatingprofile/{id}/evidence
GET  /api/v1/contractoperatingprofile/{id}/decisions
POST /api/v1/contractoperatingprofile/{id}/exceptions
POST /api/v1/contractoperatingprofile/{id}/corrections
```
...
|---|---|
| `contractoperatingprofile_aggregate` | authoritative subject |
| `contractoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `contractoperatingprofile_relationship` | governed relationships |
| `contractoperatingprofile_decision` | binding Decisions and rationale |
| `contractoperatingprofile_evidence_link` | provenance and evidence relationships |
| `contractoperatingprofile_event_outbox` | transactional Event publication |
| `contractoperatingprofile_idempotency` | duplicate-write protection |
| `contractoperatingprofile_exception` | exception, claim, dispute, or hold |
| `contractoperatingprofile_correction` | correction and supersession lineage |
| `contractoperatingprofile_audit` | privileged action evidence |
...
- Named capability `Contract` is part of the target handbook scope.
