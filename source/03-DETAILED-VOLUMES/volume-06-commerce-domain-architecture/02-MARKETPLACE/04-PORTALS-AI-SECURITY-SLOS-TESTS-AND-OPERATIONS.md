# Marketplace — Portals, AI, Security, SLOs, Tests, and Operations

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-02-04` |
| Subject | `MarketplaceOperatingProfile` |
...
| Accountable owner | Commerce Architecture / Marketplace |
...
This chapter defines how GTOS shall **define Portal journeys, AI participation, trust boundaries, SLOs, runbooks, tests, migration, examples, and conformance for Marketplace**.
...
The accountable owner is **Commerce Architecture / Marketplace**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `MarketplaceOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MarketplaceOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MarketplaceOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MarketplaceOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MarketplaceOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
MarketplaceOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateMarketplaceOperatingProfile`
- `ValidateMarketplaceOperatingProfile`
- `ApproveMarketplaceOperatingProfile`
- `ActivateMarketplaceOperatingProfile`
- `SuspendMarketplaceOperatingProfile`
- `CorrectMarketplaceOperatingProfile`
- `RetireMarketplaceOperatingProfile`

Reference Events:

- `MarketplaceOperatingProfileCreated`
- `MarketplaceOperatingProfileValidated`
- `MarketplaceOperatingProfileApproved`
- `MarketplaceOperatingProfileActivated`
- `MarketplaceOperatingProfileSuspended`
- `MarketplaceOperatingProfileCorrected`
- `MarketplaceOperatingProfileRetired`

Reference queries:

- `GetMarketplaceOperatingProfile`
- `ListMarketplaceOperatingProfileRecords`
- `GetMarketplaceOperatingProfileTimeline`
- `GetMarketplaceOperatingProfileEvidence`
- `GetMarketplaceOperatingProfileDecisionPackage`
- `GetMarketplaceOperatingProfileExceptions`
- `GetMarketplaceOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/marketplaceoperatingprofile/commands
GET  /api/v1/marketplaceoperatingprofile/{id}
GET  /api/v1/marketplaceoperatingprofile/{id}/timeline
GET  /api/v1/marketplaceoperatingprofile/{id}/evidence
GET  /api/v1/marketplaceoperatingprofile/{id}/decisions
POST /api/v1/marketplaceoperatingprofile/{id}/exceptions
POST /api/v1/marketplaceoperatingprofile/{id}/corrections
```
...
|---|---|
| `marketplaceoperatingprofile_aggregate` | authoritative subject |
| `marketplaceoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `marketplaceoperatingprofile_relationship` | governed relationships |
| `marketplaceoperatingprofile_decision` | binding Decisions and rationale |
| `marketplaceoperatingprofile_evidence_link` | provenance and evidence relationships |
| `marketplaceoperatingprofile_event_outbox` | transactional Event publication |
| `marketplaceoperatingprofile_idempotency` | duplicate-write protection |
| `marketplaceoperatingprofile_exception` | exception, claim, dispute, or hold |
| `marketplaceoperatingprofile_correction` | correction and supersession lineage |
| `marketplaceoperatingprofile_audit` | privileged action evidence |
...
- Named capability `Marketplace` is part of the target handbook scope.
