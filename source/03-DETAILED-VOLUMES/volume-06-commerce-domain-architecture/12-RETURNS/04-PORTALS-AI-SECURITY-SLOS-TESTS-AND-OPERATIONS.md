# Returns — Portals, AI, Security, SLOs, Tests, and Operations

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-12-04` |
| Subject | `ReturnsOperatingProfile` |
...
| Accountable owner | Commerce Architecture / Returns |
...
This chapter defines how GTOS shall **define Portal journeys, AI participation, trust boundaries, SLOs, runbooks, tests, migration, examples, and conformance for Returns**.
...
The accountable owner is **Commerce Architecture / Returns**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `ReturnsOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReturnsOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReturnsOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReturnsOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReturnsOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ReturnsOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateReturnsOperatingProfile`
- `ValidateReturnsOperatingProfile`
- `ApproveReturnsOperatingProfile`
- `ActivateReturnsOperatingProfile`
- `SuspendReturnsOperatingProfile`
- `CorrectReturnsOperatingProfile`
- `RetireReturnsOperatingProfile`

Reference Events:

- `ReturnsOperatingProfileCreated`
- `ReturnsOperatingProfileValidated`
- `ReturnsOperatingProfileApproved`
- `ReturnsOperatingProfileActivated`
- `ReturnsOperatingProfileSuspended`
- `ReturnsOperatingProfileCorrected`
- `ReturnsOperatingProfileRetired`

Reference queries:

- `GetReturnsOperatingProfile`
- `ListReturnsOperatingProfileRecords`
- `GetReturnsOperatingProfileTimeline`
- `GetReturnsOperatingProfileEvidence`
- `GetReturnsOperatingProfileDecisionPackage`
- `GetReturnsOperatingProfileExceptions`
- `GetReturnsOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/returnsoperatingprofile/commands
GET  /api/v1/returnsoperatingprofile/{id}
GET  /api/v1/returnsoperatingprofile/{id}/timeline
GET  /api/v1/returnsoperatingprofile/{id}/evidence
GET  /api/v1/returnsoperatingprofile/{id}/decisions
POST /api/v1/returnsoperatingprofile/{id}/exceptions
POST /api/v1/returnsoperatingprofile/{id}/corrections
```
...
|---|---|
| `returnsoperatingprofile_aggregate` | authoritative subject |
| `returnsoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `returnsoperatingprofile_relationship` | governed relationships |
| `returnsoperatingprofile_decision` | binding Decisions and rationale |
| `returnsoperatingprofile_evidence_link` | provenance and evidence relationships |
| `returnsoperatingprofile_event_outbox` | transactional Event publication |
| `returnsoperatingprofile_idempotency` | duplicate-write protection |
| `returnsoperatingprofile_exception` | exception, claim, dispute, or hold |
| `returnsoperatingprofile_correction` | correction and supersession lineage |
| `returnsoperatingprofile_audit` | privileged action evidence |
...
- Named capability `Returns` is part of the target handbook scope.
