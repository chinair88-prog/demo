# Wallet — Portals, AI, Security, SLOs, Tests, and Operations

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-09-04` |
| Subject | `WalletOperatingProfile` |
...
| Accountable owner | Commerce Architecture / Wallet |
...
This chapter defines how GTOS shall **define Portal journeys, AI participation, trust boundaries, SLOs, runbooks, tests, migration, examples, and conformance for Wallet**.
...
The accountable owner is **Commerce Architecture / Wallet**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `WalletOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WalletOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WalletOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WalletOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WalletOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
WalletOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateWalletOperatingProfile`
- `ValidateWalletOperatingProfile`
- `ApproveWalletOperatingProfile`
- `ActivateWalletOperatingProfile`
- `SuspendWalletOperatingProfile`
- `CorrectWalletOperatingProfile`
- `RetireWalletOperatingProfile`

Reference Events:

- `WalletOperatingProfileCreated`
- `WalletOperatingProfileValidated`
- `WalletOperatingProfileApproved`
- `WalletOperatingProfileActivated`
- `WalletOperatingProfileSuspended`
- `WalletOperatingProfileCorrected`
- `WalletOperatingProfileRetired`

Reference queries:

- `GetWalletOperatingProfile`
- `ListWalletOperatingProfileRecords`
- `GetWalletOperatingProfileTimeline`
- `GetWalletOperatingProfileEvidence`
- `GetWalletOperatingProfileDecisionPackage`
- `GetWalletOperatingProfileExceptions`
- `GetWalletOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/walletoperatingprofile/commands
GET  /api/v1/walletoperatingprofile/{id}
GET  /api/v1/walletoperatingprofile/{id}/timeline
GET  /api/v1/walletoperatingprofile/{id}/evidence
GET  /api/v1/walletoperatingprofile/{id}/decisions
POST /api/v1/walletoperatingprofile/{id}/exceptions
POST /api/v1/walletoperatingprofile/{id}/corrections
```
...
|---|---|
| `walletoperatingprofile_aggregate` | authoritative subject |
| `walletoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `walletoperatingprofile_relationship` | governed relationships |
| `walletoperatingprofile_decision` | binding Decisions and rationale |
| `walletoperatingprofile_evidence_link` | provenance and evidence relationships |
| `walletoperatingprofile_event_outbox` | transactional Event publication |
| `walletoperatingprofile_idempotency` | duplicate-write protection |
| `walletoperatingprofile_exception` | exception, claim, dispute, or hold |
| `walletoperatingprofile_correction` | correction and supersession lineage |
| `walletoperatingprofile_audit` | privileged action evidence |
...
- Named capability `Wallet` is part of the target handbook scope.
