# Instagram — Portals, CRM, AI Assistance, Human Handoff, SLOs, and Tests

| Metadata | Value |
|---|---|
| Volume | `8` |
| Chapter ID | `V08-04-03` |
| Subject | `InstagramOperatingProfile` |
...
| Accountable owner | Engagement Architecture / Instagram |
...
This chapter defines how GTOS shall **define Portal experience, CRM linkage, AI drafting, human handoff, security, SLOs, operations, tests and conformance for Instagram**.
...
The accountable owner is **Engagement Architecture / Instagram**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `InstagramOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `InstagramOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `InstagramOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `InstagramOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `InstagramOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
InstagramOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateInstagramOperatingProfile`
- `ValidateInstagramOperatingProfile`
- `ApproveInstagramOperatingProfile`
- `ActivateInstagramOperatingProfile`
- `SuspendInstagramOperatingProfile`
- `CorrectInstagramOperatingProfile`
- `RetireInstagramOperatingProfile`

Reference Events:

- `InstagramOperatingProfileCreated`
- `InstagramOperatingProfileValidated`
- `InstagramOperatingProfileApproved`
- `InstagramOperatingProfileActivated`
- `InstagramOperatingProfileSuspended`
- `InstagramOperatingProfileCorrected`
- `InstagramOperatingProfileRetired`

Reference queries:

- `GetInstagramOperatingProfile`
- `ListInstagramOperatingProfileRecords`
- `GetInstagramOperatingProfileTimeline`
- `GetInstagramOperatingProfileEvidence`
- `GetInstagramOperatingProfileDecisionPackage`
- `GetInstagramOperatingProfileExceptions`
- `GetInstagramOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/instagramoperatingprofile/commands
GET  /api/v1/instagramoperatingprofile/{id}
GET  /api/v1/instagramoperatingprofile/{id}/timeline
GET  /api/v1/instagramoperatingprofile/{id}/evidence
GET  /api/v1/instagramoperatingprofile/{id}/decisions
POST /api/v1/instagramoperatingprofile/{id}/exceptions
POST /api/v1/instagramoperatingprofile/{id}/corrections
```
...
|---|---|
| `instagramoperatingprofile_aggregate` | authoritative subject |
| `instagramoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `instagramoperatingprofile_relationship` | governed relationships |
| `instagramoperatingprofile_decision` | binding Decisions and rationale |
| `instagramoperatingprofile_evidence_link` | provenance and evidence relationships |
| `instagramoperatingprofile_event_outbox` | transactional Event publication |
| `instagramoperatingprofile_idempotency` | duplicate-write protection |
| `instagramoperatingprofile_exception` | exception, claim, dispute, or hold |
| `instagramoperatingprofile_correction` | correction and supersession lineage |
| `instagramoperatingprofile_audit` | privileged action evidence |
