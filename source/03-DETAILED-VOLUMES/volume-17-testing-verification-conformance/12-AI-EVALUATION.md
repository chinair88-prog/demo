...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for AI Evaluation**.
...
|---|---|
| `AIEvaluation` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIEvaluationVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIEvaluationDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIEvaluationEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIEvaluationException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `AIEvaluationCreated`
- `AIEvaluationValidated`
- `AIEvaluationApproved`
- `AIEvaluationActivated`
- `AIEvaluationSuspended`
- `AIEvaluationCorrected`
- `AIEvaluationRetired`

Reference queries:

- `GetAIEvaluation`
- `ListAIEvaluationRecords`
- `GetAIEvaluationTimeline`
- `GetAIEvaluationEvidence`
- `GetAIEvaluationDecisionPackage`
- `GetAIEvaluationExceptions`
- `GetAIEvaluationAuditTrail`

Reference API surface:

```text
POST /api/v1/aievaluation/commands
GET  /api/v1/aievaluation/{id}
GET  /api/v1/aievaluation/{id}/timeline
GET  /api/v1/aievaluation/{id}/evidence
GET  /api/v1/aievaluation/{id}/decisions
POST /api/v1/aievaluation/{id}/exceptions
POST /api/v1/aievaluation/{id}/corrections
```
...
|---|---|
| `aievaluation_aggregate` | authoritative subject |
| `aievaluation_version` | immutable submitted, approved, issued, or signed versions |
| `aievaluation_relationship` | governed relationships |
| `aievaluation_decision` | binding Decisions and rationale |
| `aievaluation_evidence_link` | provenance and evidence relationships |
| `aievaluation_event_outbox` | transactional Event publication |
| `aievaluation_idempotency` | duplicate-write protection |
| `aievaluation_exception` | exception, claim, dispute, or hold |
| `aievaluation_correction` | correction and supersession lineage |
| `aievaluation_audit` | privileged action evidence |
...
- `AI Evaluation` is part of the target GTOS handbook scope.
