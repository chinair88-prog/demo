...
| Volume | `17` |
| Chapter ID | `V17-09` |
| Subject | `SecurityAdversarialTests` |
...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Security Adversarial Tests**.
...
|---|---|
| `SecurityAdversarialTests` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SecurityAdversarialTestsVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SecurityAdversarialTestsDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SecurityAdversarialTestsEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SecurityAdversarialTestsException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
SecurityAdversarialTests {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateSecurityAdversarialTests`
- `ValidateSecurityAdversarialTests`
- `ApproveSecurityAdversarialTests`
- `ActivateSecurityAdversarialTests`
- `SuspendSecurityAdversarialTests`
- `CorrectSecurityAdversarialTests`
- `RetireSecurityAdversarialTests`

Reference Events:

- `SecurityAdversarialTestsCreated`
- `SecurityAdversarialTestsValidated`
- `SecurityAdversarialTestsApproved`
- `SecurityAdversarialTestsActivated`
- `SecurityAdversarialTestsSuspended`
- `SecurityAdversarialTestsCorrected`
- `SecurityAdversarialTestsRetired`

Reference queries:

- `GetSecurityAdversarialTests`
- `ListSecurityAdversarialTestsRecords`
- `GetSecurityAdversarialTestsTimeline`
- `GetSecurityAdversarialTestsEvidence`
- `GetSecurityAdversarialTestsDecisionPackage`
- `GetSecurityAdversarialTestsExceptions`
- `GetSecurityAdversarialTestsAuditTrail`

Reference API surface:

```text
POST /api/v1/securityadversarialtests/commands
GET  /api/v1/securityadversarialtests/{id}
GET  /api/v1/securityadversarialtests/{id}/timeline
GET  /api/v1/securityadversarialtests/{id}/evidence
GET  /api/v1/securityadversarialtests/{id}/decisions
POST /api/v1/securityadversarialtests/{id}/exceptions
POST /api/v1/securityadversarialtests/{id}/corrections
```
...
|---|---|
| `securityadversarialtests_aggregate` | authoritative subject |
| `securityadversarialtests_version` | immutable submitted, approved, issued, or signed versions |
| `securityadversarialtests_relationship` | governed relationships |
| `securityadversarialtests_decision` | binding Decisions and rationale |
| `securityadversarialtests_evidence_link` | provenance and evidence relationships |
| `securityadversarialtests_event_outbox` | transactional Event publication |
| `securityadversarialtests_idempotency` | duplicate-write protection |
| `securityadversarialtests_exception` | exception, claim, dispute, or hold |
| `securityadversarialtests_correction` | correction and supersession lineage |
| `securityadversarialtests_audit` | privileged action evidence |
...
- `Security Adversarial Tests` is part of the target GTOS handbook scope.
