# Workflow Restart and Compensation Tests

| Metadata | Value |
|---|---|
| Volume | `17` |
| Chapter ID | `V17-11` |
| Subject | `WorkflowRestartandCompensationTests` |
...
| Accountable owner | Quality Engineering Council |
...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Workflow Restart and Compensation Tests**.
...
The accountable owner is **Quality Engineering Council**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `WorkflowRestartandCompensationTests` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WorkflowRestartandCompensationTestsVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WorkflowRestartandCompensationTestsDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WorkflowRestartandCompensationTestsEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WorkflowRestartandCompensationTestsException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
WorkflowRestartandCompensationTests {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateWorkflowRestartandCompensationTests`
- `ValidateWorkflowRestartandCompensationTests`
- `ApproveWorkflowRestartandCompensationTests`
- `ActivateWorkflowRestartandCompensationTests`
- `SuspendWorkflowRestartandCompensationTests`
- `CorrectWorkflowRestartandCompensationTests`
- `RetireWorkflowRestartandCompensationTests`

Reference Events:

- `WorkflowRestartandCompensationTestsCreated`
- `WorkflowRestartandCompensationTestsValidated`
- `WorkflowRestartandCompensationTestsApproved`
- `WorkflowRestartandCompensationTestsActivated`
- `WorkflowRestartandCompensationTestsSuspended`
- `WorkflowRestartandCompensationTestsCorrected`
- `WorkflowRestartandCompensationTestsRetired`

Reference queries:

- `GetWorkflowRestartandCompensationTests`
- `ListWorkflowRestartandCompensationTestsRecords`
- `GetWorkflowRestartandCompensationTestsTimeline`
- `GetWorkflowRestartandCompensationTestsEvidence`
- `GetWorkflowRestartandCompensationTestsDecisionPackage`
- `GetWorkflowRestartandCompensationTestsExceptions`
- `GetWorkflowRestartandCompensationTestsAuditTrail`

Reference API surface:

```text
POST /api/v1/workflowrestartandcompensationtests/commands
GET  /api/v1/workflowrestartandcompensationtests/{id}
GET  /api/v1/workflowrestartandcompensationtests/{id}/timeline
GET  /api/v1/workflowrestartandcompensationtests/{id}/evidence
GET  /api/v1/workflowrestartandcompensationtests/{id}/decisions
POST /api/v1/workflowrestartandcompensationtests/{id}/exceptions
POST /api/v1/workflowrestartandcompensationtests/{id}/corrections
```
...
|---|---|
| `workflowrestartandcompensationtests_aggregate` | authoritative subject |
| `workflowrestartandcompensationtests_version` | immutable submitted, approved, issued, or signed versions |
| `workflowrestartandcompensationtests_relationship` | governed relationships |
| `workflowrestartandcompensationtests_decision` | binding Decisions and rationale |
| `workflowrestartandcompensationtests_evidence_link` | provenance and evidence relationships |
| `workflowrestartandcompensationtests_event_outbox` | transactional Event publication |
| `workflowrestartandcompensationtests_idempotency` | duplicate-write protection |
| `workflowrestartandcompensationtests_exception` | exception, claim, dispute, or hold |
| `workflowrestartandcompensationtests_correction` | correction and supersession lineage |
| `workflowrestartandcompensationtests_audit` | privileged action evidence |
...
- `Workflow Restart and Compensation Tests` is part of the target GTOS handbook scope.
