...
| Volume | `17` |
| Chapter ID | `V17-16` |
| Subject | `ResilienceandFaultInjection` |
...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Resilience and Fault Injection**.
...
|---|---|
| `ResilienceandFaultInjection` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ResilienceandFaultInjectionVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ResilienceandFaultInjectionDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ResilienceandFaultInjectionEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ResilienceandFaultInjectionException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ResilienceandFaultInjection {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateResilienceandFaultInjection`
- `ValidateResilienceandFaultInjection`
- `ApproveResilienceandFaultInjection`
- `ActivateResilienceandFaultInjection`
- `SuspendResilienceandFaultInjection`
- `CorrectResilienceandFaultInjection`
- `RetireResilienceandFaultInjection`

Reference Events:

- `ResilienceandFaultInjectionCreated`
- `ResilienceandFaultInjectionValidated`
- `ResilienceandFaultInjectionApproved`
- `ResilienceandFaultInjectionActivated`
- `ResilienceandFaultInjectionSuspended`
- `ResilienceandFaultInjectionCorrected`
- `ResilienceandFaultInjectionRetired`

Reference queries:

- `GetResilienceandFaultInjection`
- `ListResilienceandFaultInjectionRecords`
- `GetResilienceandFaultInjectionTimeline`
- `GetResilienceandFaultInjectionEvidence`
- `GetResilienceandFaultInjectionDecisionPackage`
- `GetResilienceandFaultInjectionExceptions`
- `GetResilienceandFaultInjectionAuditTrail`

Reference API surface:

```text
POST /api/v1/resilienceandfaultinjection/commands
GET  /api/v1/resilienceandfaultinjection/{id}
GET  /api/v1/resilienceandfaultinjection/{id}/timeline
GET  /api/v1/resilienceandfaultinjection/{id}/evidence
GET  /api/v1/resilienceandfaultinjection/{id}/decisions
POST /api/v1/resilienceandfaultinjection/{id}/exceptions
POST /api/v1/resilienceandfaultinjection/{id}/corrections
```
...
|---|---|
| `resilienceandfaultinjection_aggregate` | authoritative subject |
| `resilienceandfaultinjection_version` | immutable submitted, approved, issued, or signed versions |
| `resilienceandfaultinjection_relationship` | governed relationships |
| `resilienceandfaultinjection_decision` | binding Decisions and rationale |
| `resilienceandfaultinjection_evidence_link` | provenance and evidence relationships |
| `resilienceandfaultinjection_event_outbox` | transactional Event publication |
| `resilienceandfaultinjection_idempotency` | duplicate-write protection |
| `resilienceandfaultinjection_exception` | exception, claim, dispute, or hold |
| `resilienceandfaultinjection_correction` | correction and supersession lineage |
| `resilienceandfaultinjection_audit` | privileged action evidence |
...
- `Resilience and Fault Injection` is part of the target GTOS handbook scope.
