# Quotation — Portals, AI, Security, SLOs, Tests, and Operations

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-05-04` |
| Subject | `QuotationOperatingProfile` |
...
| Accountable owner | Commerce Architecture / Quotation |
...
This chapter defines how GTOS shall **define Portal journeys, AI participation, trust boundaries, SLOs, runbooks, tests, migration, examples, and conformance for Quotation**.
...
The accountable owner is **Commerce Architecture / Quotation**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `QuotationOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `QuotationOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `QuotationOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `QuotationOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `QuotationOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
QuotationOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateQuotationOperatingProfile`
- `ValidateQuotationOperatingProfile`
- `ApproveQuotationOperatingProfile`
- `ActivateQuotationOperatingProfile`
- `SuspendQuotationOperatingProfile`
- `CorrectQuotationOperatingProfile`
- `RetireQuotationOperatingProfile`

Reference Events:

- `QuotationOperatingProfileCreated`
- `QuotationOperatingProfileValidated`
- `QuotationOperatingProfileApproved`
- `QuotationOperatingProfileActivated`
- `QuotationOperatingProfileSuspended`
- `QuotationOperatingProfileCorrected`
- `QuotationOperatingProfileRetired`

Reference queries:

- `GetQuotationOperatingProfile`
- `ListQuotationOperatingProfileRecords`
- `GetQuotationOperatingProfileTimeline`
- `GetQuotationOperatingProfileEvidence`
- `GetQuotationOperatingProfileDecisionPackage`
- `GetQuotationOperatingProfileExceptions`
- `GetQuotationOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/quotationoperatingprofile/commands
GET  /api/v1/quotationoperatingprofile/{id}
GET  /api/v1/quotationoperatingprofile/{id}/timeline
GET  /api/v1/quotationoperatingprofile/{id}/evidence
GET  /api/v1/quotationoperatingprofile/{id}/decisions
POST /api/v1/quotationoperatingprofile/{id}/exceptions
POST /api/v1/quotationoperatingprofile/{id}/corrections
```
...
|---|---|
| `quotationoperatingprofile_aggregate` | authoritative subject |
| `quotationoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `quotationoperatingprofile_relationship` | governed relationships |
| `quotationoperatingprofile_decision` | binding Decisions and rationale |
| `quotationoperatingprofile_evidence_link` | provenance and evidence relationships |
| `quotationoperatingprofile_event_outbox` | transactional Event publication |
| `quotationoperatingprofile_idempotency` | duplicate-write protection |
| `quotationoperatingprofile_exception` | exception, claim, dispute, or hold |
| `quotationoperatingprofile_correction` | correction and supersession lineage |
| `quotationoperatingprofile_audit` | privileged action evidence |
...
- Named capability `Quotation` is part of the target handbook scope.
