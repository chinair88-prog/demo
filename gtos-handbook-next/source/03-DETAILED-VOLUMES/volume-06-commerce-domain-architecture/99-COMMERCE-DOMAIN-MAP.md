# Commerce Domain Map and Ownership Matrix

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-SYNTH` |
| Subject | `CommerceDomainMap` |
...
| Accountable owner | Commerce Architecture Council |
...
This chapter defines how GTOS shall **connect Product, Marketplace, Pricing, RFQ, Quotation, Contract, PI, PO, Order, Payment, Wallet, Billing, Finance, Returns, Manufacturing, and Trade Core without shared truth ownership**.
...
The accountable owner is **Commerce Architecture Council**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `CommerceDomainMap` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CommerceDomainMapVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CommerceDomainMapDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CommerceDomainMapEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `CommerceDomainMapException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreateCommerceDomainMap`
- `ValidateCommerceDomainMap`
- `ApproveCommerceDomainMap`
- `ActivateCommerceDomainMap`
- `SuspendCommerceDomainMap`
- `CorrectCommerceDomainMap`
- `RetireCommerceDomainMap`

Reference Events:

- `CommerceDomainMapCreated`
- `CommerceDomainMapValidated`
- `CommerceDomainMapApproved`
- `CommerceDomainMapActivated`
- `CommerceDomainMapSuspended`
- `CommerceDomainMapCorrected`
- `CommerceDomainMapRetired`

Reference queries:

- `GetCommerceDomainMap`
- `ListCommerceDomainMapRecords`
- `GetCommerceDomainMapTimeline`
- `GetCommerceDomainMapEvidence`
- `GetCommerceDomainMapDecisionPackage`
- `GetCommerceDomainMapExceptions`
- `GetCommerceDomainMapAuditTrail`

Reference API surface:

```text
POST /api/v1/commercedomainmap/commands
GET  /api/v1/commercedomainmap/{id}
GET  /api/v1/commercedomainmap/{id}/timeline
GET  /api/v1/commercedomainmap/{id}/evidence
GET  /api/v1/commercedomainmap/{id}/decisions
POST /api/v1/commercedomainmap/{id}/exceptions
POST /api/v1/commercedomainmap/{id}/corrections
```
...
|---|---|
| `commercedomainmap_aggregate` | authoritative subject |
| `commercedomainmap_version` | immutable submitted, approved, issued, or signed versions |
| `commercedomainmap_relationship` | governed relationships |
| `commercedomainmap_decision` | binding Decisions and rationale |
| `commercedomainmap_evidence_link` | provenance and evidence relationships |
| `commercedomainmap_event_outbox` | transactional Event publication |
| `commercedomainmap_idempotency` | duplicate-write protection |
| `commercedomainmap_exception` | exception, claim, dispute, or hold |
| `commercedomainmap_correction` | correction and supersession lineage |
| `commercedomainmap_audit` | privileged action evidence |
