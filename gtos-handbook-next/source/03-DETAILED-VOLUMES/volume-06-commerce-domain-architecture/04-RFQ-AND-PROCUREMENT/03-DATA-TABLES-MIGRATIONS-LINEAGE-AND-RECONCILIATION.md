# RFQ and Procurement — Data, Tables, Migrations, Lineage, and Reconciliation

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-04-03` |
| Subject | `RFQandProcurementDataModel` |
...
This chapter defines how GTOS shall **define database ownership, schemas, table families, keys, constraints, indexes, migrations, lineage, projections, reconciliation, retention, backup, and correction for RFQ and Procurement**.
...
|---|---|
| `RFQandProcurementDataModel` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementDataModelVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementDataModelDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementDataModelEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RFQandProcurementDataModelException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
RFQandProcurementDataModel {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateRFQandProcurementDataModel`
- `ValidateRFQandProcurementDataModel`
- `ApproveRFQandProcurementDataModel`
- `ActivateRFQandProcurementDataModel`
- `SuspendRFQandProcurementDataModel`
- `CorrectRFQandProcurementDataModel`
- `RetireRFQandProcurementDataModel`

Reference Events:

- `RFQandProcurementDataModelCreated`
- `RFQandProcurementDataModelValidated`
- `RFQandProcurementDataModelApproved`
- `RFQandProcurementDataModelActivated`
- `RFQandProcurementDataModelSuspended`
- `RFQandProcurementDataModelCorrected`
- `RFQandProcurementDataModelRetired`

Reference queries:

- `GetRFQandProcurementDataModel`
- `ListRFQandProcurementDataModelRecords`
- `GetRFQandProcurementDataModelTimeline`
- `GetRFQandProcurementDataModelEvidence`
- `GetRFQandProcurementDataModelDecisionPackage`
- `GetRFQandProcurementDataModelExceptions`
- `GetRFQandProcurementDataModelAuditTrail`

Reference API surface:

```text
POST /api/v1/rfqandprocurementdatamodel/commands
GET  /api/v1/rfqandprocurementdatamodel/{id}
GET  /api/v1/rfqandprocurementdatamodel/{id}/timeline
GET  /api/v1/rfqandprocurementdatamodel/{id}/evidence
GET  /api/v1/rfqandprocurementdatamodel/{id}/decisions
POST /api/v1/rfqandprocurementdatamodel/{id}/exceptions
POST /api/v1/rfqandprocurementdatamodel/{id}/corrections
```
...
|---|---|
| `rfqandprocurementdatamodel_aggregate` | authoritative subject |
| `rfqandprocurementdatamodel_version` | immutable submitted, approved, issued, or signed versions |
| `rfqandprocurementdatamodel_relationship` | governed relationships |
| `rfqandprocurementdatamodel_decision` | binding Decisions and rationale |
| `rfqandprocurementdatamodel_evidence_link` | provenance and evidence relationships |
| `rfqandprocurementdatamodel_event_outbox` | transactional Event publication |
| `rfqandprocurementdatamodel_idempotency` | duplicate-write protection |
| `rfqandprocurementdatamodel_exception` | exception, claim, dispute, or hold |
| `rfqandprocurementdatamodel_correction` | correction and supersession lineage |
| `rfqandprocurementdatamodel_audit` | privileged action evidence |
