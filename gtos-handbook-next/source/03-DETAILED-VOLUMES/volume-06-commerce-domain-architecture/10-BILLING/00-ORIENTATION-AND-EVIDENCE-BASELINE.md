# Billing — Orientation and Evidence Baseline

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-10-00` |
| Subject | `BillingDomain` |
...
This chapter defines how GTOS shall **define the scope, non-goals, module and database evidence requirements, ownership boundaries, current status, and target responsibilities of Billing**.
...
|---|---|
| `BillingDomain` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `BillingDomainVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `BillingDomainDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `BillingDomainEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `BillingDomainException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreateBillingDomain`
- `ValidateBillingDomain`
- `ApproveBillingDomain`
- `ActivateBillingDomain`
- `SuspendBillingDomain`
- `CorrectBillingDomain`
- `RetireBillingDomain`

Reference Events:

- `BillingDomainCreated`
- `BillingDomainValidated`
- `BillingDomainApproved`
- `BillingDomainActivated`
- `BillingDomainSuspended`
- `BillingDomainCorrected`
- `BillingDomainRetired`

Reference queries:

- `GetBillingDomain`
- `ListBillingDomainRecords`
- `GetBillingDomainTimeline`
- `GetBillingDomainEvidence`
- `GetBillingDomainDecisionPackage`
- `GetBillingDomainExceptions`
- `GetBillingDomainAuditTrail`

Reference API surface:

```text
POST /api/v1/billingdomain/commands
GET  /api/v1/billingdomain/{id}
GET  /api/v1/billingdomain/{id}/timeline
GET  /api/v1/billingdomain/{id}/evidence
GET  /api/v1/billingdomain/{id}/decisions
POST /api/v1/billingdomain/{id}/exceptions
POST /api/v1/billingdomain/{id}/corrections
```
...
|---|---|
| `billingdomain_aggregate` | authoritative subject |
| `billingdomain_version` | immutable submitted, approved, issued, or signed versions |
| `billingdomain_relationship` | governed relationships |
| `billingdomain_decision` | binding Decisions and rationale |
| `billingdomain_evidence_link` | provenance and evidence relationships |
| `billingdomain_event_outbox` | transactional Event publication |
| `billingdomain_idempotency` | duplicate-write protection |
| `billingdomain_exception` | exception, claim, dispute, or hold |
| `billingdomain_correction` | correction and supersession lineage |
| `billingdomain_audit` | privileged action evidence |
