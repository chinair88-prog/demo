# Billing — Portals, AI, Security, SLOs, Tests, and Operations

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-10-04` |
| Subject | `BillingOperatingProfile` |
...
| Accountable owner | Commerce Architecture / Billing |
...
This chapter defines how GTOS shall **define Portal journeys, AI participation, trust boundaries, SLOs, runbooks, tests, migration, examples, and conformance for Billing**.
...
The accountable owner is **Commerce Architecture / Billing**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `BillingOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `BillingOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `BillingOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `BillingOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `BillingOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
BillingOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateBillingOperatingProfile`
- `ValidateBillingOperatingProfile`
- `ApproveBillingOperatingProfile`
- `ActivateBillingOperatingProfile`
- `SuspendBillingOperatingProfile`
- `CorrectBillingOperatingProfile`
- `RetireBillingOperatingProfile`

Reference Events:

- `BillingOperatingProfileCreated`
- `BillingOperatingProfileValidated`
- `BillingOperatingProfileApproved`
- `BillingOperatingProfileActivated`
- `BillingOperatingProfileSuspended`
- `BillingOperatingProfileCorrected`
- `BillingOperatingProfileRetired`

Reference queries:

- `GetBillingOperatingProfile`
- `ListBillingOperatingProfileRecords`
- `GetBillingOperatingProfileTimeline`
- `GetBillingOperatingProfileEvidence`
- `GetBillingOperatingProfileDecisionPackage`
- `GetBillingOperatingProfileExceptions`
- `GetBillingOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/billingoperatingprofile/commands
GET  /api/v1/billingoperatingprofile/{id}
GET  /api/v1/billingoperatingprofile/{id}/timeline
GET  /api/v1/billingoperatingprofile/{id}/evidence
GET  /api/v1/billingoperatingprofile/{id}/decisions
POST /api/v1/billingoperatingprofile/{id}/exceptions
POST /api/v1/billingoperatingprofile/{id}/corrections
```
...
|---|---|
| `billingoperatingprofile_aggregate` | authoritative subject |
| `billingoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `billingoperatingprofile_relationship` | governed relationships |
| `billingoperatingprofile_decision` | binding Decisions and rationale |
| `billingoperatingprofile_evidence_link` | provenance and evidence relationships |
| `billingoperatingprofile_event_outbox` | transactional Event publication |
| `billingoperatingprofile_idempotency` | duplicate-write protection |
| `billingoperatingprofile_exception` | exception, claim, dispute, or hold |
| `billingoperatingprofile_correction` | correction and supersession lineage |
| `billingoperatingprofile_audit` | privileged action evidence |
...
- Named capability `Billing` is part of the target handbook scope.
