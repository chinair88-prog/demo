# Manufacturing — Portals, AI, Security, SLOs, Tests, and Operations

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-13-04` |
| Subject | `ManufacturingOperatingProfile` |
...
| Accountable owner | Commerce Architecture / Manufacturing |
...
This chapter defines how GTOS shall **define Portal journeys, AI participation, trust boundaries, SLOs, runbooks, tests, migration, examples, and conformance for Manufacturing**.
...
The accountable owner is **Commerce Architecture / Manufacturing**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `ManufacturingOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ManufacturingOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ManufacturingOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ManufacturingOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ManufacturingOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ManufacturingOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateManufacturingOperatingProfile`
- `ValidateManufacturingOperatingProfile`
- `ApproveManufacturingOperatingProfile`
- `ActivateManufacturingOperatingProfile`
- `SuspendManufacturingOperatingProfile`
- `CorrectManufacturingOperatingProfile`
- `RetireManufacturingOperatingProfile`

Reference Events:

- `ManufacturingOperatingProfileCreated`
- `ManufacturingOperatingProfileValidated`
- `ManufacturingOperatingProfileApproved`
- `ManufacturingOperatingProfileActivated`
- `ManufacturingOperatingProfileSuspended`
- `ManufacturingOperatingProfileCorrected`
- `ManufacturingOperatingProfileRetired`

Reference queries:

- `GetManufacturingOperatingProfile`
- `ListManufacturingOperatingProfileRecords`
- `GetManufacturingOperatingProfileTimeline`
- `GetManufacturingOperatingProfileEvidence`
- `GetManufacturingOperatingProfileDecisionPackage`
- `GetManufacturingOperatingProfileExceptions`
- `GetManufacturingOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/manufacturingoperatingprofile/commands
GET  /api/v1/manufacturingoperatingprofile/{id}
GET  /api/v1/manufacturingoperatingprofile/{id}/timeline
GET  /api/v1/manufacturingoperatingprofile/{id}/evidence
GET  /api/v1/manufacturingoperatingprofile/{id}/decisions
POST /api/v1/manufacturingoperatingprofile/{id}/exceptions
POST /api/v1/manufacturingoperatingprofile/{id}/corrections
```
...
|---|---|
| `manufacturingoperatingprofile_aggregate` | authoritative subject |
| `manufacturingoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `manufacturingoperatingprofile_relationship` | governed relationships |
| `manufacturingoperatingprofile_decision` | binding Decisions and rationale |
| `manufacturingoperatingprofile_evidence_link` | provenance and evidence relationships |
| `manufacturingoperatingprofile_event_outbox` | transactional Event publication |
| `manufacturingoperatingprofile_idempotency` | duplicate-write protection |
| `manufacturingoperatingprofile_exception` | exception, claim, dispute, or hold |
| `manufacturingoperatingprofile_correction` | correction and supersession lineage |
| `manufacturingoperatingprofile_audit` | privileged action evidence |
...
- Named capability `Manufacturing` is part of the target handbook scope.
