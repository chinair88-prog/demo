# Finance Control — Orientation and Evidence Baseline

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-11-00` |
| Subject | `FinanceControlDomain` |
...
This chapter defines how GTOS shall **define the scope, non-goals, module and database evidence requirements, ownership boundaries, current status, and target responsibilities of Finance Control**.
...
|---|---|
| `FinanceControlDomain` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FinanceControlDomainVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FinanceControlDomainDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FinanceControlDomainEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FinanceControlDomainException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
FinanceControlDomain {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateFinanceControlDomain`
- `ValidateFinanceControlDomain`
- `ApproveFinanceControlDomain`
- `ActivateFinanceControlDomain`
- `SuspendFinanceControlDomain`
- `CorrectFinanceControlDomain`
- `RetireFinanceControlDomain`

Reference Events:

- `FinanceControlDomainCreated`
- `FinanceControlDomainValidated`
- `FinanceControlDomainApproved`
- `FinanceControlDomainActivated`
- `FinanceControlDomainSuspended`
- `FinanceControlDomainCorrected`
- `FinanceControlDomainRetired`

Reference queries:

- `GetFinanceControlDomain`
- `ListFinanceControlDomainRecords`
- `GetFinanceControlDomainTimeline`
- `GetFinanceControlDomainEvidence`
- `GetFinanceControlDomainDecisionPackage`
- `GetFinanceControlDomainExceptions`
- `GetFinanceControlDomainAuditTrail`

Reference API surface:

```text
POST /api/v1/financecontroldomain/commands
GET  /api/v1/financecontroldomain/{id}
GET  /api/v1/financecontroldomain/{id}/timeline
GET  /api/v1/financecontroldomain/{id}/evidence
GET  /api/v1/financecontroldomain/{id}/decisions
POST /api/v1/financecontroldomain/{id}/exceptions
POST /api/v1/financecontroldomain/{id}/corrections
```
...
|---|---|
| `financecontroldomain_aggregate` | authoritative subject |
| `financecontroldomain_version` | immutable submitted, approved, issued, or signed versions |
| `financecontroldomain_relationship` | governed relationships |
| `financecontroldomain_decision` | binding Decisions and rationale |
| `financecontroldomain_evidence_link` | provenance and evidence relationships |
| `financecontroldomain_event_outbox` | transactional Event publication |
| `financecontroldomain_idempotency` | duplicate-write protection |
| `financecontroldomain_exception` | exception, claim, dispute, or hold |
| `financecontroldomain_correction` | correction and supersession lineage |
| `financecontroldomain_audit` | privileged action evidence |
