# Finance Control — Portals, AI, Security, SLOs, Tests, and Operations

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-11-04` |
| Subject | `FinanceControlOperatingProfile` |
...
| Accountable owner | Commerce Architecture / Finance Control |
...
This chapter defines how GTOS shall **define Portal journeys, AI participation, trust boundaries, SLOs, runbooks, tests, migration, examples, and conformance for Finance Control**.
...
The accountable owner is **Commerce Architecture / Finance Control**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `FinanceControlOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FinanceControlOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FinanceControlOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FinanceControlOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FinanceControlOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
FinanceControlOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateFinanceControlOperatingProfile`
- `ValidateFinanceControlOperatingProfile`
- `ApproveFinanceControlOperatingProfile`
- `ActivateFinanceControlOperatingProfile`
- `SuspendFinanceControlOperatingProfile`
- `CorrectFinanceControlOperatingProfile`
- `RetireFinanceControlOperatingProfile`

Reference Events:

- `FinanceControlOperatingProfileCreated`
- `FinanceControlOperatingProfileValidated`
- `FinanceControlOperatingProfileApproved`
- `FinanceControlOperatingProfileActivated`
- `FinanceControlOperatingProfileSuspended`
- `FinanceControlOperatingProfileCorrected`
- `FinanceControlOperatingProfileRetired`

Reference queries:

- `GetFinanceControlOperatingProfile`
- `ListFinanceControlOperatingProfileRecords`
- `GetFinanceControlOperatingProfileTimeline`
- `GetFinanceControlOperatingProfileEvidence`
- `GetFinanceControlOperatingProfileDecisionPackage`
- `GetFinanceControlOperatingProfileExceptions`
- `GetFinanceControlOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/financecontroloperatingprofile/commands
GET  /api/v1/financecontroloperatingprofile/{id}
GET  /api/v1/financecontroloperatingprofile/{id}/timeline
GET  /api/v1/financecontroloperatingprofile/{id}/evidence
GET  /api/v1/financecontroloperatingprofile/{id}/decisions
POST /api/v1/financecontroloperatingprofile/{id}/exceptions
POST /api/v1/financecontroloperatingprofile/{id}/corrections
```
...
|---|---|
| `financecontroloperatingprofile_aggregate` | authoritative subject |
| `financecontroloperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `financecontroloperatingprofile_relationship` | governed relationships |
| `financecontroloperatingprofile_decision` | binding Decisions and rationale |
| `financecontroloperatingprofile_evidence_link` | provenance and evidence relationships |
| `financecontroloperatingprofile_event_outbox` | transactional Event publication |
| `financecontroloperatingprofile_idempotency` | duplicate-write protection |
| `financecontroloperatingprofile_exception` | exception, claim, dispute, or hold |
| `financecontroloperatingprofile_correction` | correction and supersession lineage |
| `financecontroloperatingprofile_audit` | privileged action evidence |
...
- Named capability `Finance Control` is part of the target handbook scope.
