# Contract — Orientation and Evidence Baseline

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-06-00` |
| Subject | `ContractDomain` |
...
This chapter defines how GTOS shall **define the scope, non-goals, module and database evidence requirements, ownership boundaries, current status, and target responsibilities of Contract**.
...
|---|---|
| `ContractDomain` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ContractDomainVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ContractDomainDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ContractDomainEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ContractDomainException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveContractDomain`
- `ActivateContractDomain`
- `SuspendContractDomain`
- `CorrectContractDomain`
- `RetireContractDomain`

Reference Events:

- `ContractDomainCreated`
- `ContractDomainValidated`
- `ContractDomainApproved`
- `ContractDomainActivated`
- `ContractDomainSuspended`
- `ContractDomainCorrected`
- `ContractDomainRetired`

Reference queries:

- `GetContractDomain`
- `ListContractDomainRecords`
- `GetContractDomainTimeline`
- `GetContractDomainEvidence`
- `GetContractDomainDecisionPackage`
- `GetContractDomainExceptions`
- `GetContractDomainAuditTrail`

Reference API surface:

```text
POST /api/v1/contractdomain/commands
GET  /api/v1/contractdomain/{id}
GET  /api/v1/contractdomain/{id}/timeline
GET  /api/v1/contractdomain/{id}/evidence
GET  /api/v1/contractdomain/{id}/decisions
POST /api/v1/contractdomain/{id}/exceptions
POST /api/v1/contractdomain/{id}/corrections
```
...
|---|---|
| `contractdomain_aggregate` | authoritative subject |
| `contractdomain_version` | immutable submitted, approved, issued, or signed versions |
| `contractdomain_relationship` | governed relationships |
| `contractdomain_decision` | binding Decisions and rationale |
| `contractdomain_evidence_link` | provenance and evidence relationships |
| `contractdomain_event_outbox` | transactional Event publication |
| `contractdomain_idempotency` | duplicate-write protection |
| `contractdomain_exception` | exception, claim, dispute, or hold |
| `contractdomain_correction` | correction and supersession lineage |
| `contractdomain_audit` | privileged action evidence |
