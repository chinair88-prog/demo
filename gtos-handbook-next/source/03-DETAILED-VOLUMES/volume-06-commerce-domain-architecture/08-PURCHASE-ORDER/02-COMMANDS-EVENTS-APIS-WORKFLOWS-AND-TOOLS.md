# Purchase Order — Commands, Events, APIs, Workflows, and Tools

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-08-02` |
| Subject | `PurchaseOrderContractModel` |
...
This chapter defines how GTOS shall **define typed Commands, completed-fact Events, REST/gRPC/BFF contracts, long-running workflows, compensations, provider adapters, and AI Tools for Purchase Order**.
...
|---|---|
| `PurchaseOrderContractModel` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PurchaseOrderContractModelVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PurchaseOrderContractModelDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PurchaseOrderContractModelEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PurchaseOrderContractModelException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
PurchaseOrderContractModel {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreatePurchaseOrderContractModel`
- `ValidatePurchaseOrderContractModel`
- `ApprovePurchaseOrderContractModel`
- `ActivatePurchaseOrderContractModel`
- `SuspendPurchaseOrderContractModel`
- `CorrectPurchaseOrderContractModel`
- `RetirePurchaseOrderContractModel`

Reference Events:

- `PurchaseOrderContractModelCreated`
- `PurchaseOrderContractModelValidated`
- `PurchaseOrderContractModelApproved`
- `PurchaseOrderContractModelActivated`
- `PurchaseOrderContractModelSuspended`
- `PurchaseOrderContractModelCorrected`
- `PurchaseOrderContractModelRetired`

Reference queries:

- `GetPurchaseOrderContractModel`
- `ListPurchaseOrderContractModelRecords`
- `GetPurchaseOrderContractModelTimeline`
- `GetPurchaseOrderContractModelEvidence`
- `GetPurchaseOrderContractModelDecisionPackage`
- `GetPurchaseOrderContractModelExceptions`
- `GetPurchaseOrderContractModelAuditTrail`

Reference API surface:

```text
POST /api/v1/purchaseordercontractmodel/commands
GET  /api/v1/purchaseordercontractmodel/{id}
GET  /api/v1/purchaseordercontractmodel/{id}/timeline
GET  /api/v1/purchaseordercontractmodel/{id}/evidence
GET  /api/v1/purchaseordercontractmodel/{id}/decisions
POST /api/v1/purchaseordercontractmodel/{id}/exceptions
POST /api/v1/purchaseordercontractmodel/{id}/corrections
```
...
|---|---|
| `purchaseordercontractmodel_aggregate` | authoritative subject |
| `purchaseordercontractmodel_version` | immutable submitted, approved, issued, or signed versions |
| `purchaseordercontractmodel_relationship` | governed relationships |
| `purchaseordercontractmodel_decision` | binding Decisions and rationale |
| `purchaseordercontractmodel_evidence_link` | provenance and evidence relationships |
| `purchaseordercontractmodel_event_outbox` | transactional Event publication |
| `purchaseordercontractmodel_idempotency` | duplicate-write protection |
| `purchaseordercontractmodel_exception` | exception, claim, dispute, or hold |
| `purchaseordercontractmodel_correction` | correction and supersession lineage |
| `purchaseordercontractmodel_audit` | privileged action evidence |
