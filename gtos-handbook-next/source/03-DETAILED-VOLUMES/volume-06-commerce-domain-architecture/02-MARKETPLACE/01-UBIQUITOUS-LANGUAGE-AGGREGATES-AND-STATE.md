# Marketplace — Ubiquitous Language, Aggregates, and State

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-02-01` |
| Subject | `MarketplaceAggregateModel` |
...
This chapter defines how GTOS shall **define entities, value objects, relationships, aggregates, invariants, lifecycle state, effective time, correction, and concurrency for Marketplace**.
...
|---|---|
| `MarketplaceAggregateModel` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MarketplaceAggregateModelVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MarketplaceAggregateModelDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MarketplaceAggregateModelEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MarketplaceAggregateModelException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
MarketplaceAggregateModel {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateMarketplaceAggregateModel`
- `ValidateMarketplaceAggregateModel`
- `ApproveMarketplaceAggregateModel`
- `ActivateMarketplaceAggregateModel`
- `SuspendMarketplaceAggregateModel`
- `CorrectMarketplaceAggregateModel`
- `RetireMarketplaceAggregateModel`

Reference Events:

- `MarketplaceAggregateModelCreated`
- `MarketplaceAggregateModelValidated`
- `MarketplaceAggregateModelApproved`
- `MarketplaceAggregateModelActivated`
- `MarketplaceAggregateModelSuspended`
- `MarketplaceAggregateModelCorrected`
- `MarketplaceAggregateModelRetired`

Reference queries:

- `GetMarketplaceAggregateModel`
- `ListMarketplaceAggregateModelRecords`
- `GetMarketplaceAggregateModelTimeline`
- `GetMarketplaceAggregateModelEvidence`
- `GetMarketplaceAggregateModelDecisionPackage`
- `GetMarketplaceAggregateModelExceptions`
- `GetMarketplaceAggregateModelAuditTrail`

Reference API surface:

```text
POST /api/v1/marketplaceaggregatemodel/commands
GET  /api/v1/marketplaceaggregatemodel/{id}
GET  /api/v1/marketplaceaggregatemodel/{id}/timeline
GET  /api/v1/marketplaceaggregatemodel/{id}/evidence
GET  /api/v1/marketplaceaggregatemodel/{id}/decisions
POST /api/v1/marketplaceaggregatemodel/{id}/exceptions
POST /api/v1/marketplaceaggregatemodel/{id}/corrections
```
...
|---|---|
| `marketplaceaggregatemodel_aggregate` | authoritative subject |
| `marketplaceaggregatemodel_version` | immutable submitted, approved, issued, or signed versions |
| `marketplaceaggregatemodel_relationship` | governed relationships |
| `marketplaceaggregatemodel_decision` | binding Decisions and rationale |
| `marketplaceaggregatemodel_evidence_link` | provenance and evidence relationships |
| `marketplaceaggregatemodel_event_outbox` | transactional Event publication |
| `marketplaceaggregatemodel_idempotency` | duplicate-write protection |
| `marketplaceaggregatemodel_exception` | exception, claim, dispute, or hold |
| `marketplaceaggregatemodel_correction` | correction and supersession lineage |
| `marketplaceaggregatemodel_audit` | privileged action evidence |
