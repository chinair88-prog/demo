# Marketplace — Orientation and Evidence Baseline

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-02-00` |
| Subject | `MarketplaceDomain` |
...
This chapter defines how GTOS shall **define the scope, non-goals, module and database evidence requirements, ownership boundaries, current status, and target responsibilities of Marketplace**.
...
|---|---|
| `MarketplaceDomain` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MarketplaceDomainVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MarketplaceDomainDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MarketplaceDomainEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MarketplaceDomainException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreateMarketplaceDomain`
- `ValidateMarketplaceDomain`
- `ApproveMarketplaceDomain`
- `ActivateMarketplaceDomain`
- `SuspendMarketplaceDomain`
- `CorrectMarketplaceDomain`
- `RetireMarketplaceDomain`

Reference Events:

- `MarketplaceDomainCreated`
- `MarketplaceDomainValidated`
- `MarketplaceDomainApproved`
- `MarketplaceDomainActivated`
- `MarketplaceDomainSuspended`
- `MarketplaceDomainCorrected`
- `MarketplaceDomainRetired`

Reference queries:

- `GetMarketplaceDomain`
- `ListMarketplaceDomainRecords`
- `GetMarketplaceDomainTimeline`
- `GetMarketplaceDomainEvidence`
- `GetMarketplaceDomainDecisionPackage`
- `GetMarketplaceDomainExceptions`
- `GetMarketplaceDomainAuditTrail`

Reference API surface:

```text
POST /api/v1/marketplacedomain/commands
GET  /api/v1/marketplacedomain/{id}
GET  /api/v1/marketplacedomain/{id}/timeline
GET  /api/v1/marketplacedomain/{id}/evidence
GET  /api/v1/marketplacedomain/{id}/decisions
POST /api/v1/marketplacedomain/{id}/exceptions
POST /api/v1/marketplacedomain/{id}/corrections
```
...
|---|---|
| `marketplacedomain_aggregate` | authoritative subject |
| `marketplacedomain_version` | immutable submitted, approved, issued, or signed versions |
| `marketplacedomain_relationship` | governed relationships |
| `marketplacedomain_decision` | binding Decisions and rationale |
| `marketplacedomain_evidence_link` | provenance and evidence relationships |
| `marketplacedomain_event_outbox` | transactional Event publication |
| `marketplacedomain_idempotency` | duplicate-write protection |
| `marketplacedomain_exception` | exception, claim, dispute, or hold |
| `marketplacedomain_correction` | correction and supersession lineage |
| `marketplacedomain_audit` | privileged action evidence |
