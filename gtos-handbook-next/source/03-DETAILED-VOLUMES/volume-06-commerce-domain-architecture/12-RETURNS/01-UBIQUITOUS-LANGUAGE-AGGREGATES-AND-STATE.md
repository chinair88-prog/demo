# Returns — Ubiquitous Language, Aggregates, and State

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-12-01` |
| Subject | `ReturnsAggregateModel` |
...
This chapter defines how GTOS shall **define entities, value objects, relationships, aggregates, invariants, lifecycle state, effective time, correction, and concurrency for Returns**.
...
|---|---|
| `ReturnsAggregateModel` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReturnsAggregateModelVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReturnsAggregateModelDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReturnsAggregateModelEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReturnsAggregateModelException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ReturnsAggregateModel {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateReturnsAggregateModel`
- `ValidateReturnsAggregateModel`
- `ApproveReturnsAggregateModel`
- `ActivateReturnsAggregateModel`
- `SuspendReturnsAggregateModel`
- `CorrectReturnsAggregateModel`
- `RetireReturnsAggregateModel`

Reference Events:

- `ReturnsAggregateModelCreated`
- `ReturnsAggregateModelValidated`
- `ReturnsAggregateModelApproved`
- `ReturnsAggregateModelActivated`
- `ReturnsAggregateModelSuspended`
- `ReturnsAggregateModelCorrected`
- `ReturnsAggregateModelRetired`

Reference queries:

- `GetReturnsAggregateModel`
- `ListReturnsAggregateModelRecords`
- `GetReturnsAggregateModelTimeline`
- `GetReturnsAggregateModelEvidence`
- `GetReturnsAggregateModelDecisionPackage`
- `GetReturnsAggregateModelExceptions`
- `GetReturnsAggregateModelAuditTrail`

Reference API surface:

```text
POST /api/v1/returnsaggregatemodel/commands
GET  /api/v1/returnsaggregatemodel/{id}
GET  /api/v1/returnsaggregatemodel/{id}/timeline
GET  /api/v1/returnsaggregatemodel/{id}/evidence
GET  /api/v1/returnsaggregatemodel/{id}/decisions
POST /api/v1/returnsaggregatemodel/{id}/exceptions
POST /api/v1/returnsaggregatemodel/{id}/corrections
```
...
|---|---|
| `returnsaggregatemodel_aggregate` | authoritative subject |
| `returnsaggregatemodel_version` | immutable submitted, approved, issued, or signed versions |
| `returnsaggregatemodel_relationship` | governed relationships |
| `returnsaggregatemodel_decision` | binding Decisions and rationale |
| `returnsaggregatemodel_evidence_link` | provenance and evidence relationships |
| `returnsaggregatemodel_event_outbox` | transactional Event publication |
| `returnsaggregatemodel_idempotency` | duplicate-write protection |
| `returnsaggregatemodel_exception` | exception, claim, dispute, or hold |
| `returnsaggregatemodel_correction` | correction and supersession lineage |
| `returnsaggregatemodel_audit` | privileged action evidence |
