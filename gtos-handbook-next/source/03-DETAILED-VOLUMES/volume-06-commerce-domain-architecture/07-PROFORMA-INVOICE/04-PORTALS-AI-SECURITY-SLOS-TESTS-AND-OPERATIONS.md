# Proforma Invoice — Portals, AI, Security, SLOs, Tests, and Operations

| Metadata | Value |
|---|---|
| Volume | `6` |
| Chapter ID | `V06-07-04` |
| Subject | `ProformaInvoiceOperatingProfile` |
...
| Accountable owner | Commerce Architecture / Proforma Invoice |
...
This chapter defines how GTOS shall **define Portal journeys, AI participation, trust boundaries, SLOs, runbooks, tests, migration, examples, and conformance for Proforma Invoice**.
...
The accountable owner is **Commerce Architecture / Proforma Invoice**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `ProformaInvoiceOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProformaInvoiceOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProformaInvoiceOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProformaInvoiceOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProformaInvoiceOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ProformaInvoiceOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateProformaInvoiceOperatingProfile`
- `ValidateProformaInvoiceOperatingProfile`
- `ApproveProformaInvoiceOperatingProfile`
- `ActivateProformaInvoiceOperatingProfile`
- `SuspendProformaInvoiceOperatingProfile`
- `CorrectProformaInvoiceOperatingProfile`
- `RetireProformaInvoiceOperatingProfile`

Reference Events:

- `ProformaInvoiceOperatingProfileCreated`
- `ProformaInvoiceOperatingProfileValidated`
- `ProformaInvoiceOperatingProfileApproved`
- `ProformaInvoiceOperatingProfileActivated`
- `ProformaInvoiceOperatingProfileSuspended`
- `ProformaInvoiceOperatingProfileCorrected`
- `ProformaInvoiceOperatingProfileRetired`

Reference queries:

- `GetProformaInvoiceOperatingProfile`
- `ListProformaInvoiceOperatingProfileRecords`
- `GetProformaInvoiceOperatingProfileTimeline`
- `GetProformaInvoiceOperatingProfileEvidence`
- `GetProformaInvoiceOperatingProfileDecisionPackage`
- `GetProformaInvoiceOperatingProfileExceptions`
- `GetProformaInvoiceOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/proformainvoiceoperatingprofile/commands
GET  /api/v1/proformainvoiceoperatingprofile/{id}
GET  /api/v1/proformainvoiceoperatingprofile/{id}/timeline
GET  /api/v1/proformainvoiceoperatingprofile/{id}/evidence
GET  /api/v1/proformainvoiceoperatingprofile/{id}/decisions
POST /api/v1/proformainvoiceoperatingprofile/{id}/exceptions
POST /api/v1/proformainvoiceoperatingprofile/{id}/corrections
```
...
|---|---|
| `proformainvoiceoperatingprofile_aggregate` | authoritative subject |
| `proformainvoiceoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `proformainvoiceoperatingprofile_relationship` | governed relationships |
| `proformainvoiceoperatingprofile_decision` | binding Decisions and rationale |
| `proformainvoiceoperatingprofile_evidence_link` | provenance and evidence relationships |
| `proformainvoiceoperatingprofile_event_outbox` | transactional Event publication |
| `proformainvoiceoperatingprofile_idempotency` | duplicate-write protection |
| `proformainvoiceoperatingprofile_exception` | exception, claim, dispute, or hold |
| `proformainvoiceoperatingprofile_correction` | correction and supersession lineage |
| `proformainvoiceoperatingprofile_audit` | privileged action evidence |
...
- Named capability `Proforma Invoice` is part of the target handbook scope.
