# Provider Portal — Portal Orientation and Information Architecture

| Metadata | Value |
|---|---|
| Volume | `4` |
| Chapter ID | `V04-P06-00` |
| Subject | `ProviderPortalPortal` |
...
This chapter defines how GTOS shall **define personas, navigation, route ownership, domain dependencies and experience boundaries for logistics, customs, carrier and service providers, covering profile, RFQ invitation, transport option, booking, customs, milestones, billing, settlement**.
...
|---|---|
| `ProviderPortalPortal` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProviderPortalPortalVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProviderPortalPortalDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProviderPortalPortalEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProviderPortalPortalException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ProviderPortalPortal {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateProviderPortalPortal`
- `ValidateProviderPortalPortal`
- `ApproveProviderPortalPortal`
- `ActivateProviderPortalPortal`
- `SuspendProviderPortalPortal`
- `CorrectProviderPortalPortal`
- `RetireProviderPortalPortal`

Reference Events:

- `ProviderPortalPortalCreated`
- `ProviderPortalPortalValidated`
- `ProviderPortalPortalApproved`
- `ProviderPortalPortalActivated`
- `ProviderPortalPortalSuspended`
- `ProviderPortalPortalCorrected`
- `ProviderPortalPortalRetired`

Reference queries:

- `GetProviderPortalPortal`
- `ListProviderPortalPortalRecords`
- `GetProviderPortalPortalTimeline`
- `GetProviderPortalPortalEvidence`
- `GetProviderPortalPortalDecisionPackage`
- `GetProviderPortalPortalExceptions`
- `GetProviderPortalPortalAuditTrail`

Reference API surface:

```text
POST /api/v1/providerportalportal/commands
GET  /api/v1/providerportalportal/{id}
GET  /api/v1/providerportalportal/{id}/timeline
GET  /api/v1/providerportalportal/{id}/evidence
GET  /api/v1/providerportalportal/{id}/decisions
POST /api/v1/providerportalportal/{id}/exceptions
POST /api/v1/providerportalportal/{id}/corrections
```
...
|---|---|
| `providerportalportal_aggregate` | authoritative subject |
| `providerportalportal_version` | immutable submitted, approved, issued, or signed versions |
| `providerportalportal_relationship` | governed relationships |
| `providerportalportal_decision` | binding Decisions and rationale |
| `providerportalportal_evidence_link` | provenance and evidence relationships |
| `providerportalportal_event_outbox` | transactional Event publication |
| `providerportalportal_idempotency` | duplicate-write protection |
| `providerportalportal_exception` | exception, claim, dispute, or hold |
| `providerportalportal_correction` | correction and supersession lineage |
| `providerportalportal_audit` | privileged action evidence |
