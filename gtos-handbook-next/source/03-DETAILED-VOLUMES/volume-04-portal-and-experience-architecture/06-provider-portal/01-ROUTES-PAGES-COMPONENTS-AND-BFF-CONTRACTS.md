# Provider Portal — Routes, Pages, Components, and BFF Contracts

| Metadata | Value |
|---|---|
| Volume | `4` |
| Chapter ID | `V04-P06-01` |
| Subject | `ProviderPortalRouteRegistry` |
...
This chapter defines how GTOS shall **inventory routes, layouts, pages, components, server/client boundaries, BFFs, APIs, realtime channels and ownership for logistics, customs, carrier and service providers, covering profile, RFQ invitation, transport option, booking, customs, milestones, billing, settlement**.
...
|---|---|
| `ProviderPortalRouteRegistry` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProviderPortalRouteRegistryVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProviderPortalRouteRegistryDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProviderPortalRouteRegistryEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProviderPortalRouteRegistryException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ProviderPortalRouteRegistry {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateProviderPortalRouteRegistry`
- `ValidateProviderPortalRouteRegistry`
- `ApproveProviderPortalRouteRegistry`
- `ActivateProviderPortalRouteRegistry`
- `SuspendProviderPortalRouteRegistry`
- `CorrectProviderPortalRouteRegistry`
- `RetireProviderPortalRouteRegistry`

Reference Events:

- `ProviderPortalRouteRegistryCreated`
- `ProviderPortalRouteRegistryValidated`
- `ProviderPortalRouteRegistryApproved`
- `ProviderPortalRouteRegistryActivated`
- `ProviderPortalRouteRegistrySuspended`
- `ProviderPortalRouteRegistryCorrected`
- `ProviderPortalRouteRegistryRetired`

Reference queries:

- `GetProviderPortalRouteRegistry`
- `ListProviderPortalRouteRegistryRecords`
- `GetProviderPortalRouteRegistryTimeline`
- `GetProviderPortalRouteRegistryEvidence`
- `GetProviderPortalRouteRegistryDecisionPackage`
- `GetProviderPortalRouteRegistryExceptions`
- `GetProviderPortalRouteRegistryAuditTrail`

Reference API surface:

```text
POST /api/v1/providerportalrouteregistry/commands
GET  /api/v1/providerportalrouteregistry/{id}
GET  /api/v1/providerportalrouteregistry/{id}/timeline
GET  /api/v1/providerportalrouteregistry/{id}/evidence
GET  /api/v1/providerportalrouteregistry/{id}/decisions
POST /api/v1/providerportalrouteregistry/{id}/exceptions
POST /api/v1/providerportalrouteregistry/{id}/corrections
```
...
|---|---|
| `providerportalrouteregistry_aggregate` | authoritative subject |
| `providerportalrouteregistry_version` | immutable submitted, approved, issued, or signed versions |
| `providerportalrouteregistry_relationship` | governed relationships |
| `providerportalrouteregistry_decision` | binding Decisions and rationale |
| `providerportalrouteregistry_evidence_link` | provenance and evidence relationships |
| `providerportalrouteregistry_event_outbox` | transactional Event publication |
| `providerportalrouteregistry_idempotency` | duplicate-write protection |
| `providerportalrouteregistry_exception` | exception, claim, dispute, or hold |
| `providerportalrouteregistry_correction` | correction and supersession lineage |
| `providerportalrouteregistry_audit` | privileged action evidence |
