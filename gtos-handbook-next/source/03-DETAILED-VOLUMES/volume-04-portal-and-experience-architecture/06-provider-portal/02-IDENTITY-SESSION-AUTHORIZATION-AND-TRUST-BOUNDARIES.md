# Provider Portal — Identity, Session, Authorization, and Trust Boundaries

| Metadata | Value |
|---|---|
| Volume | `4` |
| Chapter ID | `V04-P06-02` |
| Subject | `ProviderPortalAccessModel` |
...
This chapter defines how GTOS shall **govern login, session, role, relationship, tenant, facility, channel, delegation, impersonation and privileged action for logistics, customs, carrier and service providers, covering profile, RFQ invitation, transport option, booking, customs, milestones, billing, settlement**.
...
|---|---|
| `ProviderPortalAccessModel` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProviderPortalAccessModelVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProviderPortalAccessModelDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProviderPortalAccessModelEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ProviderPortalAccessModelException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ProviderPortalAccessModel {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateProviderPortalAccessModel`
- `ValidateProviderPortalAccessModel`
- `ApproveProviderPortalAccessModel`
- `ActivateProviderPortalAccessModel`
- `SuspendProviderPortalAccessModel`
- `CorrectProviderPortalAccessModel`
- `RetireProviderPortalAccessModel`

Reference Events:

- `ProviderPortalAccessModelCreated`
- `ProviderPortalAccessModelValidated`
- `ProviderPortalAccessModelApproved`
- `ProviderPortalAccessModelActivated`
- `ProviderPortalAccessModelSuspended`
- `ProviderPortalAccessModelCorrected`
- `ProviderPortalAccessModelRetired`

Reference queries:

- `GetProviderPortalAccessModel`
- `ListProviderPortalAccessModelRecords`
- `GetProviderPortalAccessModelTimeline`
- `GetProviderPortalAccessModelEvidence`
- `GetProviderPortalAccessModelDecisionPackage`
- `GetProviderPortalAccessModelExceptions`
- `GetProviderPortalAccessModelAuditTrail`

Reference API surface:

```text
POST /api/v1/providerportalaccessmodel/commands
GET  /api/v1/providerportalaccessmodel/{id}
GET  /api/v1/providerportalaccessmodel/{id}/timeline
GET  /api/v1/providerportalaccessmodel/{id}/evidence
GET  /api/v1/providerportalaccessmodel/{id}/decisions
POST /api/v1/providerportalaccessmodel/{id}/exceptions
POST /api/v1/providerportalaccessmodel/{id}/corrections
```
...
|---|---|
| `providerportalaccessmodel_aggregate` | authoritative subject |
| `providerportalaccessmodel_version` | immutable submitted, approved, issued, or signed versions |
| `providerportalaccessmodel_relationship` | governed relationships |
| `providerportalaccessmodel_decision` | binding Decisions and rationale |
| `providerportalaccessmodel_evidence_link` | provenance and evidence relationships |
| `providerportalaccessmodel_event_outbox` | transactional Event publication |
| `providerportalaccessmodel_idempotency` | duplicate-write protection |
| `providerportalaccessmodel_exception` | exception, claim, dispute, or hold |
| `providerportalaccessmodel_correction` | correction and supersession lineage |
| `providerportalaccessmodel_audit` | privileged action evidence |
