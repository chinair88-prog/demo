# Web Admin — Portal Orientation and Information Architecture

| Metadata | Value |
|---|---|
| Volume | `4` |
| Chapter ID | `V04-P02-00` |
| Subject | `WebAdminPortal` |
...
This chapter defines how GTOS shall **define personas, navigation, route ownership, domain dependencies and experience boundaries for platform and tenant administrators, covering configuration, identity, catalog, policy, operations, finance, reporting**.
...
|---|---|
| `WebAdminPortal` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebAdminPortalVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebAdminPortalDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebAdminPortalEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebAdminPortalException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreateWebAdminPortal`
- `ValidateWebAdminPortal`
- `ApproveWebAdminPortal`
- `ActivateWebAdminPortal`
- `SuspendWebAdminPortal`
- `CorrectWebAdminPortal`
- `RetireWebAdminPortal`

Reference Events:

- `WebAdminPortalCreated`
- `WebAdminPortalValidated`
- `WebAdminPortalApproved`
- `WebAdminPortalActivated`
- `WebAdminPortalSuspended`
- `WebAdminPortalCorrected`
- `WebAdminPortalRetired`

Reference queries:

- `GetWebAdminPortal`
- `ListWebAdminPortalRecords`
- `GetWebAdminPortalTimeline`
- `GetWebAdminPortalEvidence`
- `GetWebAdminPortalDecisionPackage`
- `GetWebAdminPortalExceptions`
- `GetWebAdminPortalAuditTrail`

Reference API surface:

```text
POST /api/v1/webadminportal/commands
GET  /api/v1/webadminportal/{id}
GET  /api/v1/webadminportal/{id}/timeline
GET  /api/v1/webadminportal/{id}/evidence
GET  /api/v1/webadminportal/{id}/decisions
POST /api/v1/webadminportal/{id}/exceptions
POST /api/v1/webadminportal/{id}/corrections
```
...
|---|---|
| `webadminportal_aggregate` | authoritative subject |
| `webadminportal_version` | immutable submitted, approved, issued, or signed versions |
| `webadminportal_relationship` | governed relationships |
| `webadminportal_decision` | binding Decisions and rationale |
| `webadminportal_evidence_link` | provenance and evidence relationships |
| `webadminportal_event_outbox` | transactional Event publication |
| `webadminportal_idempotency` | duplicate-write protection |
| `webadminportal_exception` | exception, claim, dispute, or hold |
| `webadminportal_correction` | correction and supersession lineage |
| `webadminportal_audit` | privileged action evidence |
