# Web Shop — Portal Orientation and Information Architecture

| Metadata | Value |
|---|---|
| Volume | `4` |
| Chapter ID | `V04-P01-00` |
| Subject | `WebShopPortal` |
...
This chapter defines how GTOS shall **define personas, navigation, route ownership, domain dependencies and experience boundaries for customers and anonymous shoppers, covering catalog, marketplace, pricing, cart, checkout, order, payment, delivery, returns**.
...
|---|---|
| `WebShopPortal` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebShopPortalVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebShopPortalDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebShopPortalEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebShopPortalException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreateWebShopPortal`
- `ValidateWebShopPortal`
- `ApproveWebShopPortal`
- `ActivateWebShopPortal`
- `SuspendWebShopPortal`
- `CorrectWebShopPortal`
- `RetireWebShopPortal`

Reference Events:

- `WebShopPortalCreated`
- `WebShopPortalValidated`
- `WebShopPortalApproved`
- `WebShopPortalActivated`
- `WebShopPortalSuspended`
- `WebShopPortalCorrected`
- `WebShopPortalRetired`

Reference queries:

- `GetWebShopPortal`
- `ListWebShopPortalRecords`
- `GetWebShopPortalTimeline`
- `GetWebShopPortalEvidence`
- `GetWebShopPortalDecisionPackage`
- `GetWebShopPortalExceptions`
- `GetWebShopPortalAuditTrail`

Reference API surface:

```text
POST /api/v1/webshopportal/commands
GET  /api/v1/webshopportal/{id}
GET  /api/v1/webshopportal/{id}/timeline
GET  /api/v1/webshopportal/{id}/evidence
GET  /api/v1/webshopportal/{id}/decisions
POST /api/v1/webshopportal/{id}/exceptions
POST /api/v1/webshopportal/{id}/corrections
```
...
|---|---|
| `webshopportal_aggregate` | authoritative subject |
| `webshopportal_version` | immutable submitted, approved, issued, or signed versions |
| `webshopportal_relationship` | governed relationships |
| `webshopportal_decision` | binding Decisions and rationale |
| `webshopportal_evidence_link` | provenance and evidence relationships |
| `webshopportal_event_outbox` | transactional Event publication |
| `webshopportal_idempotency` | duplicate-write protection |
| `webshopportal_exception` | exception, claim, dispute, or hold |
| `webshopportal_correction` | correction and supersession lineage |
| `webshopportal_audit` | privileged action evidence |
