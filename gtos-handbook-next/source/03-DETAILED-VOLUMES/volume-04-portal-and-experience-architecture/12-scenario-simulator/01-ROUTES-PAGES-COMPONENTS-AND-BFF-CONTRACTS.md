# Scenario Simulator — Routes, Pages, Components, and BFF Contracts

| Metadata | Value |
|---|---|
| Volume | `4` |
| Chapter ID | `V04-P12-01` |
| Subject | `ScenarioSimulatorRouteRegistry` |
...
This chapter defines how GTOS shall **inventory routes, layouts, pages, components, server/client boundaries, BFFs, APIs, realtime channels and ownership for planners, analysts and executives, covering assumptions, scenarios, simulations, sensitivity, comparison, Decision support**.
...
|---|---|
| `ScenarioSimulatorRouteRegistry` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ScenarioSimulatorRouteRegistryVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ScenarioSimulatorRouteRegistryDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ScenarioSimulatorRouteRegistryEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ScenarioSimulatorRouteRegistryException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ScenarioSimulatorRouteRegistry {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateScenarioSimulatorRouteRegistry`
- `ValidateScenarioSimulatorRouteRegistry`
- `ApproveScenarioSimulatorRouteRegistry`
- `ActivateScenarioSimulatorRouteRegistry`
- `SuspendScenarioSimulatorRouteRegistry`
- `CorrectScenarioSimulatorRouteRegistry`
- `RetireScenarioSimulatorRouteRegistry`

Reference Events:

- `ScenarioSimulatorRouteRegistryCreated`
- `ScenarioSimulatorRouteRegistryValidated`
- `ScenarioSimulatorRouteRegistryApproved`
- `ScenarioSimulatorRouteRegistryActivated`
- `ScenarioSimulatorRouteRegistrySuspended`
- `ScenarioSimulatorRouteRegistryCorrected`
- `ScenarioSimulatorRouteRegistryRetired`

Reference queries:

- `GetScenarioSimulatorRouteRegistry`
- `ListScenarioSimulatorRouteRegistryRecords`
- `GetScenarioSimulatorRouteRegistryTimeline`
- `GetScenarioSimulatorRouteRegistryEvidence`
- `GetScenarioSimulatorRouteRegistryDecisionPackage`
- `GetScenarioSimulatorRouteRegistryExceptions`
- `GetScenarioSimulatorRouteRegistryAuditTrail`

Reference API surface:

```text
POST /api/v1/scenariosimulatorrouteregistry/commands
GET  /api/v1/scenariosimulatorrouteregistry/{id}
GET  /api/v1/scenariosimulatorrouteregistry/{id}/timeline
GET  /api/v1/scenariosimulatorrouteregistry/{id}/evidence
GET  /api/v1/scenariosimulatorrouteregistry/{id}/decisions
POST /api/v1/scenariosimulatorrouteregistry/{id}/exceptions
POST /api/v1/scenariosimulatorrouteregistry/{id}/corrections
```
...
|---|---|
| `scenariosimulatorrouteregistry_aggregate` | authoritative subject |
| `scenariosimulatorrouteregistry_version` | immutable submitted, approved, issued, or signed versions |
| `scenariosimulatorrouteregistry_relationship` | governed relationships |
| `scenariosimulatorrouteregistry_decision` | binding Decisions and rationale |
| `scenariosimulatorrouteregistry_evidence_link` | provenance and evidence relationships |
| `scenariosimulatorrouteregistry_event_outbox` | transactional Event publication |
| `scenariosimulatorrouteregistry_idempotency` | duplicate-write protection |
| `scenariosimulatorrouteregistry_exception` | exception, claim, dispute, or hold |
| `scenariosimulatorrouteregistry_correction` | correction and supersession lineage |
| `scenariosimulatorrouteregistry_audit` | privileged action evidence |
