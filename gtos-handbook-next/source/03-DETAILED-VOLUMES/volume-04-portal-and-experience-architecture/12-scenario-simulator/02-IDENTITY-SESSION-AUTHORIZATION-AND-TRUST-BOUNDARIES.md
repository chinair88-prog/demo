# Scenario Simulator — Identity, Session, Authorization, and Trust Boundaries

| Metadata | Value |
|---|---|
| Volume | `4` |
| Chapter ID | `V04-P12-02` |
| Subject | `ScenarioSimulatorAccessModel` |
...
This chapter defines how GTOS shall **govern login, session, role, relationship, tenant, facility, channel, delegation, impersonation and privileged action for planners, analysts and executives, covering assumptions, scenarios, simulations, sensitivity, comparison, Decision support**.
...
|---|---|
| `ScenarioSimulatorAccessModel` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ScenarioSimulatorAccessModelVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ScenarioSimulatorAccessModelDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ScenarioSimulatorAccessModelEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ScenarioSimulatorAccessModelException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ScenarioSimulatorAccessModel {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateScenarioSimulatorAccessModel`
- `ValidateScenarioSimulatorAccessModel`
- `ApproveScenarioSimulatorAccessModel`
- `ActivateScenarioSimulatorAccessModel`
- `SuspendScenarioSimulatorAccessModel`
- `CorrectScenarioSimulatorAccessModel`
- `RetireScenarioSimulatorAccessModel`

Reference Events:

- `ScenarioSimulatorAccessModelCreated`
- `ScenarioSimulatorAccessModelValidated`
- `ScenarioSimulatorAccessModelApproved`
- `ScenarioSimulatorAccessModelActivated`
- `ScenarioSimulatorAccessModelSuspended`
- `ScenarioSimulatorAccessModelCorrected`
- `ScenarioSimulatorAccessModelRetired`

Reference queries:

- `GetScenarioSimulatorAccessModel`
- `ListScenarioSimulatorAccessModelRecords`
- `GetScenarioSimulatorAccessModelTimeline`
- `GetScenarioSimulatorAccessModelEvidence`
- `GetScenarioSimulatorAccessModelDecisionPackage`
- `GetScenarioSimulatorAccessModelExceptions`
- `GetScenarioSimulatorAccessModelAuditTrail`

Reference API surface:

```text
POST /api/v1/scenariosimulatoraccessmodel/commands
GET  /api/v1/scenariosimulatoraccessmodel/{id}
GET  /api/v1/scenariosimulatoraccessmodel/{id}/timeline
GET  /api/v1/scenariosimulatoraccessmodel/{id}/evidence
GET  /api/v1/scenariosimulatoraccessmodel/{id}/decisions
POST /api/v1/scenariosimulatoraccessmodel/{id}/exceptions
POST /api/v1/scenariosimulatoraccessmodel/{id}/corrections
```
...
|---|---|
| `scenariosimulatoraccessmodel_aggregate` | authoritative subject |
| `scenariosimulatoraccessmodel_version` | immutable submitted, approved, issued, or signed versions |
| `scenariosimulatoraccessmodel_relationship` | governed relationships |
| `scenariosimulatoraccessmodel_decision` | binding Decisions and rationale |
| `scenariosimulatoraccessmodel_evidence_link` | provenance and evidence relationships |
| `scenariosimulatoraccessmodel_event_outbox` | transactional Event publication |
| `scenariosimulatoraccessmodel_idempotency` | duplicate-write protection |
| `scenariosimulatoraccessmodel_exception` | exception, claim, dispute, or hold |
| `scenariosimulatoraccessmodel_correction` | correction and supersession lineage |
| `scenariosimulatoraccessmodel_audit` | privileged action evidence |
