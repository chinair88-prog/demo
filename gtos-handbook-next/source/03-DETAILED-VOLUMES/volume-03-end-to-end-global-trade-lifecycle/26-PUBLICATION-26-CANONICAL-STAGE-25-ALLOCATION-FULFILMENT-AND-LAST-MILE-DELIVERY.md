# Publication Chapter 26 — Allocation, Fulfilment, and Last-Mile Delivery

| Metadata | Value |
|---|---|
| Publication Chapter | `26` |
| Canonical Lifecycle Stage | `25` |
...
| Accountable capability | Inventory / Warehouse / Shipping / Order |
...
This stage exists to **allocate accepted demand, execute pick-pack-dispatch, and record delivery attempts and proof while preserving domain ownership**.
...
The accountable capability is **Inventory / Warehouse / Shipping / Order**. Supporting Portals, workflow engines, reports, search indexes, integration adapters, and AI Agents may coordinate or present the work, but they do not silently become owners of the underlying business truth.
...
|---|---|
| `FulfilmentOrder` | independent identified subject or governed subordinate entity; ownership must be declared |
| `Allocation` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PickTask` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PackTask` | independent identified subject or governed subordinate entity; ownership must be declared |
| `Dispatch` | independent identified subject or governed subordinate entity; ownership must be declared |
| `LastMileShipment` | independent identified subject or governed subordinate entity; ownership must be declared |
| `DeliveryAttempt` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| State | Meaning |
|---|---|
| `PENDING_ALLOCATION` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
| `PACKING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `READY_TO_DISPATCH` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `DISPATCHED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `OUT_FOR_DELIVERY` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `DELIVERY_FAILED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `DELIVERED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
|---|---|---|
| `PENDING_ALLOCATION` | `ALLOCATED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `ALLOCATED` | `PICKING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `PICKING` | `PACKING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `PACKING` | `READY_TO_DISPATCH` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `READY_TO_DISPATCH` | `DISPATCHED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `DISPATCHED` | `OUT_FOR_DELIVERY` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `OUT_FOR_DELIVERY` | `DELIVERY_FAILED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `DELIVERY_FAILED` | `DELIVERED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `DELIVERED` | `PARTIALLY_FULFILLED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `PARTIALLY_FULFILLED` | `CANCELLED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `GetFulfilmentOrder`
- `ListFulfilmentOrderRecords`
- `GetFulfilmentOrderTimeline`
- `GetFulfilmentOrderEvidence`
- `GetFulfilmentOrderDecisionPackage`
- `GetFulfilmentOrderExceptions`
- `GetFulfilmentOrderAuditTrail`
- `GetFulfilmentOrderCurrentPerspective`
...
```text
POST /api/v1/allocation_fulfilment_and_last_mile_delivery/commands
GET  /api/v1/allocation_fulfilment_and_last_mile_delivery/{id}
GET  /api/v1/allocation_fulfilment_and_last_mile_delivery/{id}/timeline
GET  /api/v1/allocation_fulfilment_and_last_mile_delivery/{id}/evidence
GET  /api/v1/allocation_fulfilment_and_last_mile_delivery/{id}/decisions
POST /api/v1/allocation_fulfilment_and_last_mile_delivery/{id}/exceptions
POST /api/v1/allocation_fulfilment_and_last_mile_delivery/{id}/corrections
```
...
- Order, inventory, warehouse, and shipping services are registered.
- Cross-domain orchestration and fulfilment aggregate boundaries require direct repository verification.
