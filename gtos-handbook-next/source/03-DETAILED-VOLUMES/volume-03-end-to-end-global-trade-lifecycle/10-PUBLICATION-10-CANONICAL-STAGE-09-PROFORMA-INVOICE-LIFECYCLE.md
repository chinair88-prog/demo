# Publication Chapter 10 — Proforma Invoice Lifecycle

| Metadata | Value |
|---|---|
| Publication Chapter | `10` |
| Canonical Lifecycle Stage | `09` |
...
| Accountable capability | Trade Documents / Contract / Finance Control |
...
This stage exists to **turn accepted commercial terms into a governed, versioned supplier-issued commercial document without confusing it with payment, a commercial invoice, or a purchase order**.
...
The accountable capability is **Trade Documents / Contract / Finance Control**. Supporting Portals, workflow engines, reports, search indexes, integration adapters, and AI Agents may coordinate or present the work, but they do not silently become owners of the underlying business truth.
...
|---|---|
| `ProformaInvoice` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ProformaInvoiceVersion` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ProformaInvoiceLine` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PaymentTermSchedule` | independent identified subject or governed subordinate entity; ownership must be declared |
| `BankingInstruction` | independent identified subject or governed subordinate entity; ownership must be declared |
| `DocumentEvidenceSet` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| State | Meaning |
|---|---|
| `DRAFT` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `VALIDATION_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `APPROVAL_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `APPROVED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `ISSUED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `BUYER_ACCEPTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `PAYMENT_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `PARTIALLY_PAID` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `PAID` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `EXPIRED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `CANCELLED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `SUPERSEDED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    ...
    APPROVED --> ISSUED
    ISSUED --> BUYER_ACCEPTED
    BUYER_ACCEPTED --> PAYMENT_PENDING
...
|---|---|---|
| `DRAFT` | `VALIDATION_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `VALIDATION_PENDING` | `APPROVAL_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `APPROVAL_PENDING` | `APPROVED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `APPROVED` | `ISSUED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `ISSUED` | `BUYER_ACCEPTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `BUYER_ACCEPTED` | `PAYMENT_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `PAYMENT_PENDING` | `PARTIALLY_PAID` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `PARTIALLY_PAID` | `PAID` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `PAID` | `EXPIRED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `EXPIRED` | `CANCELLED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CANCELLED` | `SUPERSEDED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `SUPERSEDED` | `DISPUTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `ProformaInvoiceValidated`
- `ProformaInvoiceApproved`
- `ProformaInvoiceIssued`
- `ProformaInvoiceAccepted`
- `ProformaInvoiceAmended`
- `ProformaInvoiceCancelled`
- `PaymentIntentLinked`
...
- `GetProformaInvoice`
- `ListProformaInvoiceRecords`
- `GetProformaInvoiceTimeline`
- `GetProformaInvoiceEvidence`
- `GetProformaInvoiceDecisionPackage`
- `GetProformaInvoiceExceptions`
- `GetProformaInvoiceAuditTrail`
- `GetProformaInvoiceCurrentPerspective`
...
```text
POST /api/v1/proforma_invoice_lifecycle/commands
GET  /api/v1/proforma_invoice_lifecycle/{id}
GET  /api/v1/proforma_invoice_lifecycle/{id}/timeline
GET  /api/v1/proforma_invoice_lifecycle/{id}/evidence
GET  /api/v1/proforma_invoice_lifecycle/{id}/decisions
POST /api/v1/proforma_invoice_lifecycle/{id}/exceptions
POST /api/v1/proforma_invoice_lifecycle/{id}/corrections
```
...
- extract PI fields and compare them with the accepted quotation
- detect beneficiary or bank-detail changes
- detect arithmetic, currency, tax, and charge anomalies
- draft discrepancy explanations
...
- Registered supporting capabilities include contract, payment, billing, trade, and document-related modules.
- A complete canonical PI aggregate, immutable history, and verified quotation-to-PI backend path have not yet been established.
- This chapter therefore defaults to REFERENCE_ARCHITECTURE and REQUIRES_REPOSITORY_REVIEW.
