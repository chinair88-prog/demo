# Publication Chapter 14 — Production Execution and Milestone Observation

| Metadata | Value |
|---|---|
| Publication Chapter | `14` |
| Canonical Lifecycle Stage | `13` |
...
| Accountable capability | Manufacturing |
...
This stage exists to **record qualified production progress, lots, quantities, delays, exceptions, and milestone evidence while distinguishing observed progress from approved quality release**.
...
The accountable capability is **Manufacturing**. Supporting Portals, workflow engines, reports, search indexes, integration adapters, and AI Agents may coordinate or present the work, but they do not silently become owners of the underlying business truth.
...
|---|---|
| `ProductionExecution` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ProductionBatch` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ProductionLot` | independent identified subject or governed subordinate entity; ownership must be declared |
| `MilestoneObservation` | independent identified subject or governed subordinate entity; ownership must be declared |
| `QuantityProgress` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ProductionException` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ReworkInstruction` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| State | Meaning |
|---|---|
| `NOT_STARTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `STARTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
| `PARTIALLY_COMPLETED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `COMPLETED_PENDING_QC` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `REWORK` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    ...
    IN_PROGRESS --> BLOCKED
    BLOCKED --> DELAYED
    DELAYED --> PARTIALLY_COMPLETED
    PARTIALLY_COMPLETED --> COMPLETED_PENDING_QC
    COMPLETED_PENDING_QC --> REWORK
    REWORK --> CANCELLED
    CANCELLED --> CLOSED
```
...
|---|---|---|
| `NOT_STARTED` | `STARTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `STARTED` | `IN_PROGRESS` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `IN_PROGRESS` | `BLOCKED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `BLOCKED` | `DELAYED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `DELAYED` | `PARTIALLY_COMPLETED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `PARTIALLY_COMPLETED` | `COMPLETED_PENDING_QC` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `COMPLETED_PENDING_QC` | `REWORK` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REWORK` | `CANCELLED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `ReportProductionException`
- `RequestRework`
- `CompleteProduction`
- `CorrectProductionObservation`
...
- `ProductionStarted`
- `ProductionMilestoneObserved`
- `ProductionProgressRecorded`
- `ProductionDelayed`
- `ProductionBlocked`
- `ProductionCompleted`
- `ProductionObservationCorrected`
...
- `GetProductionExecutionTimeline`
- `GetProductionExecutionEvidence`
- `GetProductionExecutionDecisionPackage`
- `GetProductionExecutionExceptions`
- `GetProductionExecutionAuditTrail`
- `GetProductionExecutionCurrentPerspective`
...
```text
POST /api/v1/production_execution_and_milestone_observation/commands
GET  /api/v1/production_execution_and_milestone_observation/{id}
GET  /api/v1/production_execution_and_milestone_observation/{id}/timeline
GET  /api/v1/production_execution_and_milestone_observation/{id}/evidence
GET  /api/v1/production_execution_and_milestone_observation/{id}/decisions
POST /api/v1/production_execution_and_milestone_observation/{id}/exceptions
POST /api/v1/production_execution_and_milestone_observation/{id}/corrections
```
...
- predict delay and completion
- detect inconsistent progress
- summarize production exceptions
- compare observed progress with baseline
...
- Manufacturing and Event Bus capabilities are registered.
- Portal progress views, when present, do not by themselves prove a canonical production-event model.
- This chapter remains REFERENCE_ARCHITECTURE pending repository verification.
