# Publication Chapter 18 — Transport Planning, Booking, and Capacity

| Metadata | Value |
|---|---|
| Publication Chapter | `18` |
| Canonical Lifecycle Stage | `17` |
...
This stage exists to **convert a shipment requirement into a confirmed transport arrangement with route, mode, provider, carrier, capacity, cost, and schedule**.
...
|---|---|
| `TransportRequirement` | independent identified subject or governed subordinate entity; ownership must be declared |
| `RouteOption` | independent identified subject or governed subordinate entity; ownership must be declared |
| `TransportQuote` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CarrierSelectionDecision` | independent identified subject or governed subordinate entity; ownership must be declared |
| `Booking` | independent identified subject or governed subordinate entity; ownership must be declared |
| `BookingVersion` | independent identified subject or governed subordinate entity; ownership must be declared |
| `CapacityConfirmation` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ShipmentPlan` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| State | Meaning |
|---|---|
| `REQUIREMENT_READY` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `OPTIONS_REQUESTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `OPTIONS_RECEIVED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `SELECTION_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `BOOKING_REQUESTED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `CONFIRMATION_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `CONFIRMED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    [*] --> REQUIREMENT_READY
    REQUIREMENT_READY --> OPTIONS_REQUESTED
    OPTIONS_REQUESTED --> OPTIONS_RECEIVED
    OPTIONS_RECEIVED --> SELECTION_PENDING
    SELECTION_PENDING --> BOOKING_REQUESTED
    BOOKING_REQUESTED --> CONFIRMATION_PENDING
    CONFIRMATION_PENDING --> CONFIRMED
    CONFIRMED --> AMENDMENT_PENDING
    AMENDMENT_PENDING --> CANCELLED
    CANCELLED --> FAILED
```
...
|---|---|---|
| `REQUIREMENT_READY` | `OPTIONS_REQUESTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `OPTIONS_REQUESTED` | `OPTIONS_RECEIVED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `OPTIONS_RECEIVED` | `SELECTION_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `SELECTION_PENDING` | `BOOKING_REQUESTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `BOOKING_REQUESTED` | `CONFIRMATION_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CONFIRMATION_PENDING` | `CONFIRMED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CONFIRMED` | `AMENDMENT_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `AMENDMENT_PENDING` | `CANCELLED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CANCELLED` | `FAILED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `TransportRequirementCreated`
- `TransportOptionsReceived`
...
- `ListTransportRequirementRecords`
- `GetTransportRequirementTimeline`
- `GetTransportRequirementEvidence`
- `GetTransportRequirementDecisionPackage`
- `GetTransportRequirementExceptions`
- `GetTransportRequirementAuditTrail`
- `GetTransportRequirementCurrentPerspective`
...
```text
POST /api/v1/transport_planning_booking_and_capacity/commands
GET  /api/v1/transport_planning_booking_and_capacity/{id}
GET  /api/v1/transport_planning_booking_and_capacity/{id}/timeline
GET  /api/v1/transport_planning_booking_and_capacity/{id}/evidence
GET  /api/v1/transport_planning_booking_and_capacity/{id}/decisions
POST /api/v1/transport_planning_booking_and_capacity/{id}/exceptions
POST /api/v1/transport_planning_booking_and_capacity/{id}/corrections
```
...
- compare routes
- predict ETA and delay
- normalize transport cost
- recommend capacity options
...
- Shipping, provider, workflow, and event capabilities are registered.
- Carrier integrations and booking schemas require direct evidence before implementation status can be upgraded.
