# Publication Chapter 24 — Product Publication, Marketplace, and Channel Eligibility

| Metadata | Value |
|---|---|
| Publication Chapter | `24` |
| Canonical Lifecycle Stage | `23` |
...
| Accountable capability | Product Catalog / Pricing / Inventory / Marketplace |
...
This stage exists to **publish governed offers only when identity, content, price, availability, compliance, and channel policies permit**.
...
The accountable capability is **Product Catalog / Pricing / Inventory / Marketplace**. Supporting Portals, workflow engines, reports, search indexes, integration adapters, and AI Agents may coordinate or present the work, but they do not silently become owners of the underlying business truth.
...
|---|---|
| `Product` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ProductVersion` | independent identified subject or governed subordinate entity; ownership must be declared |
| `Offer` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ChannelPublication` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PublicationEligibility` | independent identified subject or governed subordinate entity; ownership must be declared |
| `LocalizedContent` | independent identified subject or governed subordinate entity; ownership must be declared |
| `ModerationCase` | independent identified subject or governed subordinate entity; ownership must be declared |
| `PublicationAudit` | independent identified subject or governed subordinate entity; ownership must be declared |
...
| `CONTENT_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `ELIGIBILITY_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `MODERATION_PENDING` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `APPROVED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `PUBLISHED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `SUSPENDED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
| `UNPUBLISHED` | distinct governed condition; exact entry, exit, timeout, and correction semantics require a Domain specification |
...
stateDiagram-v2
    [*] --> DRAFT
    DRAFT --> CONTENT_PENDING
    CONTENT_PENDING --> ELIGIBILITY_PENDING
    ELIGIBILITY_PENDING --> MODERATION_PENDING
...
|---|---|---|
| `DRAFT` | `CONTENT_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `CONTENT_PENDING` | `ELIGIBILITY_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `ELIGIBILITY_PENDING` | `MODERATION_PENDING` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `MODERATION_PENDING` | `APPROVED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `APPROVED` | `PUBLISHED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `PUBLISHED` | `SUSPENDED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `SUSPENDED` | `UNPUBLISHED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `UNPUBLISHED` | `REJECTED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
| `REJECTED` | `EXPIRED` | authorized Command, expected version, applicable Policy, required Evidence, and idempotency guard |
...
- `GetProductTimeline`
- `GetProductEvidence`
- `GetProductDecisionPackage`
- `GetProductExceptions`
- `GetProductAuditTrail`
- `GetProductCurrentPerspective`
...
```text
POST /api/v1/product_publication_marketplace_and_channel_eligibility/commands
GET  /api/v1/product_publication_marketplace_and_channel_eligibility/{id}
GET  /api/v1/product_publication_marketplace_and_channel_eligibility/{id}/timeline
GET  /api/v1/product_publication_marketplace_and_channel_eligibility/{id}/evidence
GET  /api/v1/product_publication_marketplace_and_channel_eligibility/{id}/decisions
POST /api/v1/product_publication_marketplace_and_channel_eligibility/{id}/exceptions
POST /api/v1/product_publication_marketplace_and_channel_eligibility/{id}/corrections
```
...
- Product Catalog and Marketplace services are registered.
- Exact offer and publication ownership requires repository reconciliation because known catalog overlaps exist.
