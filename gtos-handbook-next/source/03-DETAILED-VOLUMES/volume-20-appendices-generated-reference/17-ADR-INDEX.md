...
This chapter defines how GTOS shall **define the generation source, schema, owner, evidence level, update cadence, validation, publication, retention, and use of the ADR Index**.
...
|---|---|
| `ADRIndex` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ADRIndexVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ADRIndexDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ADRIndexEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ADRIndexException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `GetADRIndexTimeline`
- `GetADRIndexEvidence`
- `GetADRIndexDecisionPackage`
- `GetADRIndexExceptions`
- `GetADRIndexAuditTrail`

Reference API surface:

```text
POST /api/v1/adrindex/commands
GET  /api/v1/adrindex/{id}
GET  /api/v1/adrindex/{id}/timeline
GET  /api/v1/adrindex/{id}/evidence
GET  /api/v1/adrindex/{id}/decisions
POST /api/v1/adrindex/{id}/exceptions
POST /api/v1/adrindex/{id}/corrections
```
...
|---|---|
| `adrindex_aggregate` | authoritative subject |
| `adrindex_version` | immutable submitted, approved, issued, or signed versions |
| `adrindex_relationship` | governed relationships |
| `adrindex_decision` | binding Decisions and rationale |
| `adrindex_evidence_link` | provenance and evidence relationships |
| `adrindex_event_outbox` | transactional Event publication |
| `adrindex_idempotency` | duplicate-write protection |
| `adrindex_exception` | exception, claim, dispute, or hold |
| `adrindex_correction` | correction and supersession lineage |
| `adrindex_audit` | privileged action evidence |
