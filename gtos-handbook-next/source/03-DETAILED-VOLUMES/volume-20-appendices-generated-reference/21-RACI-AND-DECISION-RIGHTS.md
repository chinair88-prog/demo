...
This chapter defines how GTOS shall **define the generation source, schema, owner, evidence level, update cadence, validation, publication, retention, and use of the RACI and Decision Rights**.
...
|---|---|
| `RACIandDecisionRights` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RACIandDecisionRightsVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RACIandDecisionRightsDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RACIandDecisionRightsEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RACIandDecisionRightsException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
RACIandDecisionRights {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateRACIandDecisionRights`
- `ValidateRACIandDecisionRights`
- `ApproveRACIandDecisionRights`
- `ActivateRACIandDecisionRights`
- `SuspendRACIandDecisionRights`
- `CorrectRACIandDecisionRights`
- `RetireRACIandDecisionRights`

Reference Events:

- `RACIandDecisionRightsCreated`
- `RACIandDecisionRightsValidated`
- `RACIandDecisionRightsApproved`
- `RACIandDecisionRightsActivated`
- `RACIandDecisionRightsSuspended`
- `RACIandDecisionRightsCorrected`
- `RACIandDecisionRightsRetired`

Reference queries:

- `GetRACIandDecisionRights`
- `ListRACIandDecisionRightsRecords`
- `GetRACIandDecisionRightsTimeline`
- `GetRACIandDecisionRightsEvidence`
- `GetRACIandDecisionRightsDecisionPackage`
- `GetRACIandDecisionRightsExceptions`
- `GetRACIandDecisionRightsAuditTrail`

Reference API surface:

```text
POST /api/v1/racianddecisionrights/commands
GET  /api/v1/racianddecisionrights/{id}
GET  /api/v1/racianddecisionrights/{id}/timeline
GET  /api/v1/racianddecisionrights/{id}/evidence
GET  /api/v1/racianddecisionrights/{id}/decisions
POST /api/v1/racianddecisionrights/{id}/exceptions
POST /api/v1/racianddecisionrights/{id}/corrections
```
...
|---|---|
| `racianddecisionrights_aggregate` | authoritative subject |
| `racianddecisionrights_version` | immutable submitted, approved, issued, or signed versions |
| `racianddecisionrights_relationship` | governed relationships |
| `racianddecisionrights_decision` | binding Decisions and rationale |
| `racianddecisionrights_evidence_link` | provenance and evidence relationships |
| `racianddecisionrights_event_outbox` | transactional Event publication |
| `racianddecisionrights_idempotency` | duplicate-write protection |
| `racianddecisionrights_exception` | exception, claim, dispute, or hold |
| `racianddecisionrights_correction` | correction and supersession lineage |
| `racianddecisionrights_audit` | privileged action evidence |
