...
This chapter defines how GTOS shall **define the generation source, schema, owner, evidence level, update cadence, validation, publication, retention, and use of the Release Manifest**.
...
|---|---|
| `ReleaseManifest` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReleaseManifestVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReleaseManifestDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReleaseManifestEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReleaseManifestException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `SuspendReleaseManifest`
- `CorrectReleaseManifest`
- `RetireReleaseManifest`

Reference Events:

- `ReleaseManifestCreated`
- `ReleaseManifestValidated`
- `ReleaseManifestApproved`
- `ReleaseManifestActivated`
- `ReleaseManifestSuspended`
- `ReleaseManifestCorrected`
- `ReleaseManifestRetired`

Reference queries:

- `GetReleaseManifest`
- `ListReleaseManifestRecords`
- `GetReleaseManifestTimeline`
- `GetReleaseManifestEvidence`
- `GetReleaseManifestDecisionPackage`
- `GetReleaseManifestExceptions`
- `GetReleaseManifestAuditTrail`

Reference API surface:

```text
POST /api/v1/releasemanifest/commands
GET  /api/v1/releasemanifest/{id}
GET  /api/v1/releasemanifest/{id}/timeline
GET  /api/v1/releasemanifest/{id}/evidence
GET  /api/v1/releasemanifest/{id}/decisions
POST /api/v1/releasemanifest/{id}/exceptions
POST /api/v1/releasemanifest/{id}/corrections
```
...
|---|---|
| `releasemanifest_aggregate` | authoritative subject |
| `releasemanifest_version` | immutable submitted, approved, issued, or signed versions |
| `releasemanifest_relationship` | governed relationships |
| `releasemanifest_decision` | binding Decisions and rationale |
| `releasemanifest_evidence_link` | provenance and evidence relationships |
| `releasemanifest_event_outbox` | transactional Event publication |
| `releasemanifest_idempotency` | duplicate-write protection |
| `releasemanifest_exception` | exception, claim, dispute, or hold |
| `releasemanifest_correction` | correction and supersession lineage |
| `releasemanifest_audit` | privileged action evidence |
