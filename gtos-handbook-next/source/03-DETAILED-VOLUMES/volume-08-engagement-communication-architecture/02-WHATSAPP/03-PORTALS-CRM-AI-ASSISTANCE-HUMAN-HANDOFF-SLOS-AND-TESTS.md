# WhatsApp — Portals, CRM, AI Assistance, Human Handoff, SLOs, and Tests

| Metadata | Value |
|---|---|
| Volume | `8` |
| Chapter ID | `V08-02-03` |
| Subject | `WhatsAppOperatingProfile` |
...
| Accountable owner | Engagement Architecture / WhatsApp |
...
This chapter defines how GTOS shall **define Portal experience, CRM linkage, AI drafting, human handoff, security, SLOs, operations, tests and conformance for WhatsApp**.
...
The accountable owner is **Engagement Architecture / WhatsApp**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `WhatsAppOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WhatsAppOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WhatsAppOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WhatsAppOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WhatsAppOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
WhatsAppOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateWhatsAppOperatingProfile`
- `ValidateWhatsAppOperatingProfile`
- `ApproveWhatsAppOperatingProfile`
- `ActivateWhatsAppOperatingProfile`
- `SuspendWhatsAppOperatingProfile`
- `CorrectWhatsAppOperatingProfile`
- `RetireWhatsAppOperatingProfile`

Reference Events:

- `WhatsAppOperatingProfileCreated`
- `WhatsAppOperatingProfileValidated`
- `WhatsAppOperatingProfileApproved`
- `WhatsAppOperatingProfileActivated`
- `WhatsAppOperatingProfileSuspended`
- `WhatsAppOperatingProfileCorrected`
- `WhatsAppOperatingProfileRetired`

Reference queries:

- `GetWhatsAppOperatingProfile`
- `ListWhatsAppOperatingProfileRecords`
- `GetWhatsAppOperatingProfileTimeline`
- `GetWhatsAppOperatingProfileEvidence`
- `GetWhatsAppOperatingProfileDecisionPackage`
- `GetWhatsAppOperatingProfileExceptions`
- `GetWhatsAppOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/whatsappoperatingprofile/commands
GET  /api/v1/whatsappoperatingprofile/{id}
GET  /api/v1/whatsappoperatingprofile/{id}/timeline
GET  /api/v1/whatsappoperatingprofile/{id}/evidence
GET  /api/v1/whatsappoperatingprofile/{id}/decisions
POST /api/v1/whatsappoperatingprofile/{id}/exceptions
POST /api/v1/whatsappoperatingprofile/{id}/corrections
```
...
|---|---|
| `whatsappoperatingprofile_aggregate` | authoritative subject |
| `whatsappoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `whatsappoperatingprofile_relationship` | governed relationships |
| `whatsappoperatingprofile_decision` | binding Decisions and rationale |
| `whatsappoperatingprofile_evidence_link` | provenance and evidence relationships |
| `whatsappoperatingprofile_event_outbox` | transactional Event publication |
| `whatsappoperatingprofile_idempotency` | duplicate-write protection |
| `whatsappoperatingprofile_exception` | exception, claim, dispute, or hold |
| `whatsappoperatingprofile_correction` | correction and supersession lineage |
| `whatsappoperatingprofile_audit` | privileged action evidence |
