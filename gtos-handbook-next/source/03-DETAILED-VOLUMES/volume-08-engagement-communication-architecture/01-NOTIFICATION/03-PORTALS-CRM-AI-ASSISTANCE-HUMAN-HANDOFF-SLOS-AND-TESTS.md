# Notification — Portals, CRM, AI Assistance, Human Handoff, SLOs, and Tests

| Metadata | Value |
|---|---|
| Volume | `8` |
| Chapter ID | `V08-01-03` |
| Subject | `NotificationOperatingProfile` |
...
| Accountable owner | Engagement Architecture / Notification |
...
This chapter defines how GTOS shall **define Portal experience, CRM linkage, AI drafting, human handoff, security, SLOs, operations, tests and conformance for Notification**.
...
The accountable owner is **Engagement Architecture / Notification**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `NotificationOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `NotificationOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `NotificationOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `NotificationOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `NotificationOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
NotificationOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateNotificationOperatingProfile`
- `ValidateNotificationOperatingProfile`
- `ApproveNotificationOperatingProfile`
- `ActivateNotificationOperatingProfile`
- `SuspendNotificationOperatingProfile`
- `CorrectNotificationOperatingProfile`
- `RetireNotificationOperatingProfile`

Reference Events:

- `NotificationOperatingProfileCreated`
- `NotificationOperatingProfileValidated`
- `NotificationOperatingProfileApproved`
- `NotificationOperatingProfileActivated`
- `NotificationOperatingProfileSuspended`
- `NotificationOperatingProfileCorrected`
- `NotificationOperatingProfileRetired`

Reference queries:

- `GetNotificationOperatingProfile`
- `ListNotificationOperatingProfileRecords`
- `GetNotificationOperatingProfileTimeline`
- `GetNotificationOperatingProfileEvidence`
- `GetNotificationOperatingProfileDecisionPackage`
- `GetNotificationOperatingProfileExceptions`
- `GetNotificationOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/notificationoperatingprofile/commands
GET  /api/v1/notificationoperatingprofile/{id}
GET  /api/v1/notificationoperatingprofile/{id}/timeline
GET  /api/v1/notificationoperatingprofile/{id}/evidence
GET  /api/v1/notificationoperatingprofile/{id}/decisions
POST /api/v1/notificationoperatingprofile/{id}/exceptions
POST /api/v1/notificationoperatingprofile/{id}/corrections
```
...
|---|---|
| `notificationoperatingprofile_aggregate` | authoritative subject |
| `notificationoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `notificationoperatingprofile_relationship` | governed relationships |
| `notificationoperatingprofile_decision` | binding Decisions and rationale |
| `notificationoperatingprofile_evidence_link` | provenance and evidence relationships |
| `notificationoperatingprofile_event_outbox` | transactional Event publication |
| `notificationoperatingprofile_idempotency` | duplicate-write protection |
| `notificationoperatingprofile_exception` | exception, claim, dispute, or hold |
| `notificationoperatingprofile_correction` | correction and supersession lineage |
| `notificationoperatingprofile_audit` | privileged action evidence |
