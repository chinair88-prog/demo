# WhatsApp Connector — Portals, CRM, AI Assistance, Human Handoff, SLOs, and Tests

| Metadata | Value |
|---|---|
| Volume | `8` |
| Chapter ID | `V08-03-03` |
| Subject | `WhatsAppConnectorOperatingProfile` |
...
| Accountable owner | Engagement Architecture / WhatsApp Connector |
...
This chapter defines how GTOS shall **define Portal experience, CRM linkage, AI drafting, human handoff, security, SLOs, operations, tests and conformance for WhatsApp Connector**.
...
The accountable owner is **Engagement Architecture / WhatsApp Connector**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `WhatsAppConnectorOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WhatsAppConnectorOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WhatsAppConnectorOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WhatsAppConnectorOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WhatsAppConnectorOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
WhatsAppConnectorOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateWhatsAppConnectorOperatingProfile`
- `ValidateWhatsAppConnectorOperatingProfile`
- `ApproveWhatsAppConnectorOperatingProfile`
- `ActivateWhatsAppConnectorOperatingProfile`
- `SuspendWhatsAppConnectorOperatingProfile`
- `CorrectWhatsAppConnectorOperatingProfile`
- `RetireWhatsAppConnectorOperatingProfile`

Reference Events:

- `WhatsAppConnectorOperatingProfileCreated`
- `WhatsAppConnectorOperatingProfileValidated`
- `WhatsAppConnectorOperatingProfileApproved`
- `WhatsAppConnectorOperatingProfileActivated`
- `WhatsAppConnectorOperatingProfileSuspended`
- `WhatsAppConnectorOperatingProfileCorrected`
- `WhatsAppConnectorOperatingProfileRetired`

Reference queries:

- `GetWhatsAppConnectorOperatingProfile`
- `ListWhatsAppConnectorOperatingProfileRecords`
- `GetWhatsAppConnectorOperatingProfileTimeline`
- `GetWhatsAppConnectorOperatingProfileEvidence`
- `GetWhatsAppConnectorOperatingProfileDecisionPackage`
- `GetWhatsAppConnectorOperatingProfileExceptions`
- `GetWhatsAppConnectorOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/whatsappconnectoroperatingprofile/commands
GET  /api/v1/whatsappconnectoroperatingprofile/{id}
GET  /api/v1/whatsappconnectoroperatingprofile/{id}/timeline
GET  /api/v1/whatsappconnectoroperatingprofile/{id}/evidence
GET  /api/v1/whatsappconnectoroperatingprofile/{id}/decisions
POST /api/v1/whatsappconnectoroperatingprofile/{id}/exceptions
POST /api/v1/whatsappconnectoroperatingprofile/{id}/corrections
```
...
|---|---|
| `whatsappconnectoroperatingprofile_aggregate` | authoritative subject |
| `whatsappconnectoroperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `whatsappconnectoroperatingprofile_relationship` | governed relationships |
| `whatsappconnectoroperatingprofile_decision` | binding Decisions and rationale |
| `whatsappconnectoroperatingprofile_evidence_link` | provenance and evidence relationships |
| `whatsappconnectoroperatingprofile_event_outbox` | transactional Event publication |
| `whatsappconnectoroperatingprofile_idempotency` | duplicate-write protection |
| `whatsappconnectoroperatingprofile_exception` | exception, claim, dispute, or hold |
| `whatsappconnectoroperatingprofile_correction` | correction and supersession lineage |
| `whatsappconnectoroperatingprofile_audit` | privileged action evidence |
