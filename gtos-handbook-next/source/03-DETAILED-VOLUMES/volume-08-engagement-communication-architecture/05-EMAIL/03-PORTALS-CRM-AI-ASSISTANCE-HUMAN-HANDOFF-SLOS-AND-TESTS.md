# Email — Portals, CRM, AI Assistance, Human Handoff, SLOs, and Tests

| Metadata | Value |
|---|---|
| Volume | `8` |
| Chapter ID | `V08-05-03` |
| Subject | `EmailOperatingProfile` |
...
| Accountable owner | Engagement Architecture / Email |
...
This chapter defines how GTOS shall **define Portal experience, CRM linkage, AI drafting, human handoff, security, SLOs, operations, tests and conformance for Email**.
...
The accountable owner is **Engagement Architecture / Email**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `EmailOperatingProfile` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `EmailOperatingProfileVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `EmailOperatingProfileDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `EmailOperatingProfileEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `EmailOperatingProfileException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
EmailOperatingProfile {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateEmailOperatingProfile`
- `ValidateEmailOperatingProfile`
- `ApproveEmailOperatingProfile`
- `ActivateEmailOperatingProfile`
- `SuspendEmailOperatingProfile`
- `CorrectEmailOperatingProfile`
- `RetireEmailOperatingProfile`

Reference Events:

- `EmailOperatingProfileCreated`
- `EmailOperatingProfileValidated`
- `EmailOperatingProfileApproved`
- `EmailOperatingProfileActivated`
- `EmailOperatingProfileSuspended`
- `EmailOperatingProfileCorrected`
- `EmailOperatingProfileRetired`

Reference queries:

- `GetEmailOperatingProfile`
- `ListEmailOperatingProfileRecords`
- `GetEmailOperatingProfileTimeline`
- `GetEmailOperatingProfileEvidence`
- `GetEmailOperatingProfileDecisionPackage`
- `GetEmailOperatingProfileExceptions`
- `GetEmailOperatingProfileAuditTrail`

Reference API surface:

```text
POST /api/v1/emailoperatingprofile/commands
GET  /api/v1/emailoperatingprofile/{id}
GET  /api/v1/emailoperatingprofile/{id}/timeline
GET  /api/v1/emailoperatingprofile/{id}/evidence
GET  /api/v1/emailoperatingprofile/{id}/decisions
POST /api/v1/emailoperatingprofile/{id}/exceptions
POST /api/v1/emailoperatingprofile/{id}/corrections
```
...
|---|---|
| `emailoperatingprofile_aggregate` | authoritative subject |
| `emailoperatingprofile_version` | immutable submitted, approved, issued, or signed versions |
| `emailoperatingprofile_relationship` | governed relationships |
| `emailoperatingprofile_decision` | binding Decisions and rationale |
| `emailoperatingprofile_evidence_link` | provenance and evidence relationships |
| `emailoperatingprofile_event_outbox` | transactional Event publication |
| `emailoperatingprofile_idempotency` | duplicate-write protection |
| `emailoperatingprofile_exception` | exception, claim, dispute, or hold |
| `emailoperatingprofile_correction` | correction and supersession lineage |
| `emailoperatingprofile_audit` | privileged action evidence |
