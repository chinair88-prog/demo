...
This chapter defines how GTOS shall **apply MFA and step-up based on action risk, financial threshold, legal effect, device, and anomaly signals**.
...
|---|---|
| `MFAControl` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MFAControlVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MFAControlDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MFAControlEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MFAControlException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `GetMFAControl`
- `ListMFAControlRecords`
- `GetMFAControlTimeline`
- `GetMFAControlEvidence`
- `GetMFAControlDecisionPackage`
- `GetMFAControlExceptions`
- `GetMFAControlAuditTrail`

Reference API surface:

```text
POST /api/v1/mfacontrol/commands
GET  /api/v1/mfacontrol/{id}
GET  /api/v1/mfacontrol/{id}/timeline
GET  /api/v1/mfacontrol/{id}/evidence
GET  /api/v1/mfacontrol/{id}/decisions
POST /api/v1/mfacontrol/{id}/exceptions
POST /api/v1/mfacontrol/{id}/corrections
```
...
|---|---|
| `mfacontrol_aggregate` | authoritative subject |
| `mfacontrol_version` | immutable submitted, approved, issued, or signed versions |
| `mfacontrol_relationship` | governed relationships |
| `mfacontrol_decision` | binding Decisions and rationale |
| `mfacontrol_evidence_link` | provenance and evidence relationships |
| `mfacontrol_event_outbox` | transactional Event publication |
| `mfacontrol_idempotency` | duplicate-write protection |
| `mfacontrol_exception` | exception, claim, dispute, or hold |
| `mfacontrol_correction` | correction and supersession lineage |
| `mfacontrol_audit` | privileged action evidence |
