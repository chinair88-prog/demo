# Human Identity and Person Profile

| Metadata | Value |
|---|---|
| Volume | `5` |
| Chapter ID | `V05-01` |
| Subject | `HumanIdentity` |
...
| Accountable owner | Identity Domain |
...
This chapter defines how GTOS shall **represent a natural person independently from accounts, roles, memberships, credentials, and employment**.
...
The accountable owner is **Identity Domain**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `HumanIdentity` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `HumanIdentityVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `HumanIdentityDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `HumanIdentityEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `HumanIdentityException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveHumanIdentity`
- `ActivateHumanIdentity`
- `SuspendHumanIdentity`
- `CorrectHumanIdentity`
- `RetireHumanIdentity`
...
- `HumanIdentityApproved`
- `HumanIdentityActivated`
- `HumanIdentitySuspended`
- `HumanIdentityCorrected`
- `HumanIdentityRetired`

Reference queries:

- `GetHumanIdentity`
- `ListHumanIdentityRecords`
- `GetHumanIdentityTimeline`
- `GetHumanIdentityEvidence`
- `GetHumanIdentityDecisionPackage`
- `GetHumanIdentityExceptions`
- `GetHumanIdentityAuditTrail`

Reference API surface:

```text
POST /api/v1/humanidentity/commands
GET  /api/v1/humanidentity/{id}
GET  /api/v1/humanidentity/{id}/timeline
GET  /api/v1/humanidentity/{id}/evidence
GET  /api/v1/humanidentity/{id}/decisions
POST /api/v1/humanidentity/{id}/exceptions
POST /api/v1/humanidentity/{id}/corrections
```
...
|---|---|
| `humanidentity_aggregate` | authoritative subject |
| `humanidentity_version` | immutable submitted, approved, issued, or signed versions |
| `humanidentity_relationship` | governed relationships |
| `humanidentity_decision` | binding Decisions and rationale |
| `humanidentity_evidence_link` | provenance and evidence relationships |
| `humanidentity_event_outbox` | transactional Event publication |
| `humanidentity_idempotency` | duplicate-write protection |
| `humanidentity_exception` | exception, claim, dispute, or hold |
| `humanidentity_correction` | correction and supersession lineage |
| `humanidentity_audit` | privileged action evidence |
