# Organization and Legal-Entity Identity

| Metadata | Value |
|---|---|
| Volume | `5` |
| Chapter ID | `V05-02` |
| Subject | `OrganizationIdentity` |
...
| Accountable owner | Identity / Customer / Provider |
...
This chapter defines how GTOS shall **represent companies, legal entities, branches, trade names, registrations, jurisdictions, and organizational relationships**.
...
The accountable owner is **Identity / Customer / Provider**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `OrganizationIdentity` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `OrganizationIdentityVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `OrganizationIdentityDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `OrganizationIdentityEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `OrganizationIdentityException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveOrganizationIdentity`
- `ActivateOrganizationIdentity`
- `SuspendOrganizationIdentity`
- `CorrectOrganizationIdentity`
- `RetireOrganizationIdentity`

Reference Events:

- `OrganizationIdentityCreated`
- `OrganizationIdentityValidated`
- `OrganizationIdentityApproved`
- `OrganizationIdentityActivated`
- `OrganizationIdentitySuspended`
- `OrganizationIdentityCorrected`
- `OrganizationIdentityRetired`

Reference queries:

- `GetOrganizationIdentity`
- `ListOrganizationIdentityRecords`
- `GetOrganizationIdentityTimeline`
- `GetOrganizationIdentityEvidence`
- `GetOrganizationIdentityDecisionPackage`
- `GetOrganizationIdentityExceptions`
- `GetOrganizationIdentityAuditTrail`

Reference API surface:

```text
POST /api/v1/organizationidentity/commands
GET  /api/v1/organizationidentity/{id}
GET  /api/v1/organizationidentity/{id}/timeline
GET  /api/v1/organizationidentity/{id}/evidence
GET  /api/v1/organizationidentity/{id}/decisions
POST /api/v1/organizationidentity/{id}/exceptions
POST /api/v1/organizationidentity/{id}/corrections
```
...
|---|---|
| `organizationidentity_aggregate` | authoritative subject |
| `organizationidentity_version` | immutable submitted, approved, issued, or signed versions |
| `organizationidentity_relationship` | governed relationships |
| `organizationidentity_decision` | binding Decisions and rationale |
| `organizationidentity_evidence_link` | provenance and evidence relationships |
| `organizationidentity_event_outbox` | transactional Event publication |
| `organizationidentity_idempotency` | duplicate-write protection |
| `organizationidentity_exception` | exception, claim, dispute, or hold |
| `organizationidentity_correction` | correction and supersession lineage |
| `organizationidentity_audit` | privileged action evidence |
