# Delegation, Mandate, Power of Attorney, and Representation

| Metadata | Value |
|---|---|
| Volume | `5` |
| Chapter ID | `V05-13` |
| Subject | `DelegationMandate` |
...
| Accountable owner | Security / Contract |
...
This chapter defines how GTOS shall **record who may represent whom, for which actions, limits, time, jurisdiction, revocation, and evidence**.
...
The accountable owner is **Security / Contract**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `DelegationMandate` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DelegationMandateVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DelegationMandateDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DelegationMandateEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DelegationMandateException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ValidateDelegationMandate`
- `ApproveDelegationMandate`
- `ActivateDelegationMandate`
- `SuspendDelegationMandate`
- `CorrectDelegationMandate`
- `RetireDelegationMandate`

Reference Events:

- `DelegationMandateCreated`
- `DelegationMandateValidated`
- `DelegationMandateApproved`
- `DelegationMandateActivated`
- `DelegationMandateSuspended`
- `DelegationMandateCorrected`
- `DelegationMandateRetired`

Reference queries:

- `GetDelegationMandate`
- `ListDelegationMandateRecords`
- `GetDelegationMandateTimeline`
- `GetDelegationMandateEvidence`
- `GetDelegationMandateDecisionPackage`
- `GetDelegationMandateExceptions`
- `GetDelegationMandateAuditTrail`

Reference API surface:

```text
POST /api/v1/delegationmandate/commands
GET  /api/v1/delegationmandate/{id}
GET  /api/v1/delegationmandate/{id}/timeline
GET  /api/v1/delegationmandate/{id}/evidence
GET  /api/v1/delegationmandate/{id}/decisions
POST /api/v1/delegationmandate/{id}/exceptions
POST /api/v1/delegationmandate/{id}/corrections
```
...
|---|---|
| `delegationmandate_aggregate` | authoritative subject |
| `delegationmandate_version` | immutable submitted, approved, issued, or signed versions |
| `delegationmandate_relationship` | governed relationships |
| `delegationmandate_decision` | binding Decisions and rationale |
| `delegationmandate_evidence_link` | provenance and evidence relationships |
| `delegationmandate_event_outbox` | transactional Event publication |
| `delegationmandate_idempotency` | duplicate-write protection |
| `delegationmandate_exception` | exception, claim, dispute, or hold |
| `delegationmandate_correction` | correction and supersession lineage |
| `delegationmandate_audit` | privileged action evidence |
