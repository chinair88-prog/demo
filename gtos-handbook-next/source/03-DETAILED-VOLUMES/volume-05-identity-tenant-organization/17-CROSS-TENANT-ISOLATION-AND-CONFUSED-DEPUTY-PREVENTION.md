# Cross-Tenant Isolation and Confused-Deputy Prevention

| Metadata | Value |
|---|---|
| Volume | `5` |
| Chapter ID | `V05-17` |
| Subject | `TenantIsolation` |
...
This chapter defines how GTOS shall **prove that humans, services, Portals, Jobs, Events, caches, AI Tools, and operators cannot cross authority boundaries**.
...
The accountable owner is **Security Architecture**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `TenantIsolation` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `TenantIsolationVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `TenantIsolationDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `TenantIsolationEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `TenantIsolationException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveTenantIsolation`
- `ActivateTenantIsolation`
- `SuspendTenantIsolation`
- `CorrectTenantIsolation`
- `RetireTenantIsolation`

Reference Events:

- `TenantIsolationCreated`
- `TenantIsolationValidated`
- `TenantIsolationApproved`
- `TenantIsolationActivated`
- `TenantIsolationSuspended`
- `TenantIsolationCorrected`
- `TenantIsolationRetired`

Reference queries:

- `GetTenantIsolation`
- `ListTenantIsolationRecords`
- `GetTenantIsolationTimeline`
- `GetTenantIsolationEvidence`
- `GetTenantIsolationDecisionPackage`
- `GetTenantIsolationExceptions`
- `GetTenantIsolationAuditTrail`

Reference API surface:

```text
POST /api/v1/tenantisolation/commands
GET  /api/v1/tenantisolation/{id}
GET  /api/v1/tenantisolation/{id}/timeline
GET  /api/v1/tenantisolation/{id}/evidence
GET  /api/v1/tenantisolation/{id}/decisions
POST /api/v1/tenantisolation/{id}/exceptions
POST /api/v1/tenantisolation/{id}/corrections
```
...
|---|---|
| `tenantisolation_aggregate` | authoritative subject |
| `tenantisolation_version` | immutable submitted, approved, issued, or signed versions |
| `tenantisolation_relationship` | governed relationships |
| `tenantisolation_decision` | binding Decisions and rationale |
| `tenantisolation_evidence_link` | provenance and evidence relationships |
| `tenantisolation_event_outbox` | transactional Event publication |
| `tenantisolation_idempotency` | duplicate-write protection |
| `tenantisolation_exception` | exception, claim, dispute, or hold |
| `tenantisolation_correction` | correction and supersession lineage |
| `tenantisolation_audit` | privileged action evidence |
