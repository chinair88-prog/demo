# Tenant, Company, Workspace, Facility, and Channel Scope
...
| Accountable owner | Identity / Platform |
...
This chapter defines how GTOS shall **separate tenant, legal entity, company, workspace, facility, warehouse, channel, project, and jurisdiction context**.
...
The accountable owner is **Identity / Platform**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `TenantScope` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `TenantScopeVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `TenantScopeDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `TenantScopeEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `TenantScopeException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `TenantScopeCreated`
- `TenantScopeValidated`
- `TenantScopeApproved`
- `TenantScopeActivated`
- `TenantScopeSuspended`
- `TenantScopeCorrected`
- `TenantScopeRetired`

Reference queries:

- `GetTenantScope`
- `ListTenantScopeRecords`
- `GetTenantScopeTimeline`
- `GetTenantScopeEvidence`
- `GetTenantScopeDecisionPackage`
- `GetTenantScopeExceptions`
- `GetTenantScopeAuditTrail`

Reference API surface:

```text
POST /api/v1/tenantscope/commands
GET  /api/v1/tenantscope/{id}
GET  /api/v1/tenantscope/{id}/timeline
GET  /api/v1/tenantscope/{id}/evidence
GET  /api/v1/tenantscope/{id}/decisions
POST /api/v1/tenantscope/{id}/exceptions
POST /api/v1/tenantscope/{id}/corrections
```
...
|---|---|
| `tenantscope_aggregate` | authoritative subject |
| `tenantscope_version` | immutable submitted, approved, issued, or signed versions |
| `tenantscope_relationship` | governed relationships |
| `tenantscope_decision` | binding Decisions and rationale |
| `tenantscope_evidence_link` | provenance and evidence relationships |
| `tenantscope_event_outbox` | transactional Event publication |
| `tenantscope_idempotency` | duplicate-write protection |
| `tenantscope_exception` | exception, claim, dispute, or hold |
| `tenantscope_correction` | correction and supersession lineage |
| `tenantscope_audit` | privileged action evidence |
