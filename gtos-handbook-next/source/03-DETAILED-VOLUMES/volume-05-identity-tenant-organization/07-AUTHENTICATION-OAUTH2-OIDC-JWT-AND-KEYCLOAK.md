# Authentication, OAuth2, OIDC, JWT, and Keycloak

| Metadata | Value |
|---|---|
| Volume | `5` |
| Chapter ID | `V05-07` |
| Subject | `AuthenticationProtocol` |
...
| Accountable owner | Auth / Security |
...
This chapter defines how GTOS shall **define token issuance, claims, audiences, scopes, session binding, rotation, logout, revocation, and Keycloak integration**.
...
The accountable owner is **Auth / Security**. Supporting applications, shared infrastructure, integration adapters, analytics, and AI coordinate or present the capability but shall not silently own its authoritative propositions.
...
|---|---|
| `AuthenticationProtocol` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AuthenticationProtocolVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AuthenticationProtocolDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AuthenticationProtocolEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AuthenticationProtocolException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ValidateAuthenticationProtocol`
- `ApproveAuthenticationProtocol`
- `ActivateAuthenticationProtocol`
- `SuspendAuthenticationProtocol`
- `CorrectAuthenticationProtocol`
- `RetireAuthenticationProtocol`

Reference Events:

- `AuthenticationProtocolCreated`
- `AuthenticationProtocolValidated`
- `AuthenticationProtocolApproved`
- `AuthenticationProtocolActivated`
- `AuthenticationProtocolSuspended`
- `AuthenticationProtocolCorrected`
- `AuthenticationProtocolRetired`

Reference queries:

- `GetAuthenticationProtocol`
- `ListAuthenticationProtocolRecords`
- `GetAuthenticationProtocolTimeline`
- `GetAuthenticationProtocolEvidence`
- `GetAuthenticationProtocolDecisionPackage`
- `GetAuthenticationProtocolExceptions`
- `GetAuthenticationProtocolAuditTrail`

Reference API surface:

```text
POST /api/v1/authenticationprotocol/commands
GET  /api/v1/authenticationprotocol/{id}
GET  /api/v1/authenticationprotocol/{id}/timeline
GET  /api/v1/authenticationprotocol/{id}/evidence
GET  /api/v1/authenticationprotocol/{id}/decisions
POST /api/v1/authenticationprotocol/{id}/exceptions
POST /api/v1/authenticationprotocol/{id}/corrections
```
...
|---|---|
| `authenticationprotocol_aggregate` | authoritative subject |
| `authenticationprotocol_version` | immutable submitted, approved, issued, or signed versions |
| `authenticationprotocol_relationship` | governed relationships |
| `authenticationprotocol_decision` | binding Decisions and rationale |
| `authenticationprotocol_evidence_link` | provenance and evidence relationships |
| `authenticationprotocol_event_outbox` | transactional Event publication |
| `authenticationprotocol_idempotency` | duplicate-write protection |
| `authenticationprotocol_exception` | exception, claim, dispute, or hold |
| `authenticationprotocol_correction` | correction and supersession lineage |
| `authenticationprotocol_audit` | privileged action evidence |
