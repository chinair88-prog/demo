...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for SLI SLO and Error Budgets**.
...
|---|---|
| `SLISLOandErrorBudgets` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SLISLOandErrorBudgetsVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SLISLOandErrorBudgetsDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SLISLOandErrorBudgetsEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SLISLOandErrorBudgetsException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
SLISLOandErrorBudgets {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateSLISLOandErrorBudgets`
- `ValidateSLISLOandErrorBudgets`
- `ApproveSLISLOandErrorBudgets`
- `ActivateSLISLOandErrorBudgets`
- `SuspendSLISLOandErrorBudgets`
- `CorrectSLISLOandErrorBudgets`
- `RetireSLISLOandErrorBudgets`

Reference Events:

- `SLISLOandErrorBudgetsCreated`
- `SLISLOandErrorBudgetsValidated`
- `SLISLOandErrorBudgetsApproved`
- `SLISLOandErrorBudgetsActivated`
- `SLISLOandErrorBudgetsSuspended`
- `SLISLOandErrorBudgetsCorrected`
- `SLISLOandErrorBudgetsRetired`

Reference queries:

- `GetSLISLOandErrorBudgets`
- `ListSLISLOandErrorBudgetsRecords`
- `GetSLISLOandErrorBudgetsTimeline`
- `GetSLISLOandErrorBudgetsEvidence`
- `GetSLISLOandErrorBudgetsDecisionPackage`
- `GetSLISLOandErrorBudgetsExceptions`
- `GetSLISLOandErrorBudgetsAuditTrail`

Reference API surface:

```text
POST /api/v1/slisloanderrorbudgets/commands
GET  /api/v1/slisloanderrorbudgets/{id}
GET  /api/v1/slisloanderrorbudgets/{id}/timeline
GET  /api/v1/slisloanderrorbudgets/{id}/evidence
GET  /api/v1/slisloanderrorbudgets/{id}/decisions
POST /api/v1/slisloanderrorbudgets/{id}/exceptions
POST /api/v1/slisloanderrorbudgets/{id}/corrections
```
...
|---|---|
| `slisloanderrorbudgets_aggregate` | authoritative subject |
| `slisloanderrorbudgets_version` | immutable submitted, approved, issued, or signed versions |
| `slisloanderrorbudgets_relationship` | governed relationships |
| `slisloanderrorbudgets_decision` | binding Decisions and rationale |
| `slisloanderrorbudgets_evidence_link` | provenance and evidence relationships |
| `slisloanderrorbudgets_event_outbox` | transactional Event publication |
| `slisloanderrorbudgets_idempotency` | duplicate-write protection |
| `slisloanderrorbudgets_exception` | exception, claim, dispute, or hold |
| `slisloanderrorbudgets_correction` | correction and supersession lineage |
| `slisloanderrorbudgets_audit` | privileged action evidence |
...
- `SLI SLO and Error Budgets` is part of the target GTOS handbook scope.
