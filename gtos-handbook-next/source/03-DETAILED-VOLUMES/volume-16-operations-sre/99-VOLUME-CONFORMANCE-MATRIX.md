# Volume 16 Conformance and Evidence Matrix

| Metadata | Value |
|---|---|
| Volume | `16` |
| Chapter ID | `V16-SYNTH` |
| Subject | `Volume16ConformanceMatrix` |
...
This chapter defines how GTOS shall **trace every requirement in Volume 16 to owner, repository source, configuration, deployment, runtime evidence, tests, runbooks, exceptions and review status**.
...
|---|---|
| `Volume16ConformanceMatrix` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `Volume16ConformanceMatrixVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `Volume16ConformanceMatrixDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `Volume16ConformanceMatrixEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `Volume16ConformanceMatrixException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
Volume16ConformanceMatrix {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateVolume16ConformanceMatrix`
- `ValidateVolume16ConformanceMatrix`
- `ApproveVolume16ConformanceMatrix`
- `ActivateVolume16ConformanceMatrix`
- `SuspendVolume16ConformanceMatrix`
- `CorrectVolume16ConformanceMatrix`
- `RetireVolume16ConformanceMatrix`

Reference Events:

- `Volume16ConformanceMatrixCreated`
- `Volume16ConformanceMatrixValidated`
- `Volume16ConformanceMatrixApproved`
- `Volume16ConformanceMatrixActivated`
- `Volume16ConformanceMatrixSuspended`
- `Volume16ConformanceMatrixCorrected`
- `Volume16ConformanceMatrixRetired`

Reference queries:

- `GetVolume16ConformanceMatrix`
- `ListVolume16ConformanceMatrixRecords`
- `GetVolume16ConformanceMatrixTimeline`
- `GetVolume16ConformanceMatrixEvidence`
- `GetVolume16ConformanceMatrixDecisionPackage`
- `GetVolume16ConformanceMatrixExceptions`
- `GetVolume16ConformanceMatrixAuditTrail`

Reference API surface:

```text
POST /api/v1/volume16conformancematrix/commands
GET  /api/v1/volume16conformancematrix/{id}
GET  /api/v1/volume16conformancematrix/{id}/timeline
GET  /api/v1/volume16conformancematrix/{id}/evidence
GET  /api/v1/volume16conformancematrix/{id}/decisions
POST /api/v1/volume16conformancematrix/{id}/exceptions
POST /api/v1/volume16conformancematrix/{id}/corrections
```
...
|---|---|
| `volume16conformancematrix_aggregate` | authoritative subject |
| `volume16conformancematrix_version` | immutable submitted, approved, issued, or signed versions |
| `volume16conformancematrix_relationship` | governed relationships |
| `volume16conformancematrix_decision` | binding Decisions and rationale |
| `volume16conformancematrix_evidence_link` | provenance and evidence relationships |
| `volume16conformancematrix_event_outbox` | transactional Event publication |
| `volume16conformancematrix_idempotency` | duplicate-write protection |
| `volume16conformancematrix_exception` | exception, claim, dispute, or hold |
| `volume16conformancematrix_correction` | correction and supersession lineage |
| `volume16conformancematrix_audit` | privileged action evidence |
