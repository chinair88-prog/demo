...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Release and Rollback**.
...
|---|---|
| `ReleaseandRollback` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReleaseandRollbackVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReleaseandRollbackDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReleaseandRollbackEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReleaseandRollbackException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreateReleaseandRollback`
- `ValidateReleaseandRollback`
- `ApproveReleaseandRollback`
- `ActivateReleaseandRollback`
- `SuspendReleaseandRollback`
- `CorrectReleaseandRollback`
- `RetireReleaseandRollback`

Reference Events:

- `ReleaseandRollbackCreated`
- `ReleaseandRollbackValidated`
- `ReleaseandRollbackApproved`
- `ReleaseandRollbackActivated`
- `ReleaseandRollbackSuspended`
- `ReleaseandRollbackCorrected`
- `ReleaseandRollbackRetired`

Reference queries:

- `GetReleaseandRollback`
- `ListReleaseandRollbackRecords`
- `GetReleaseandRollbackTimeline`
- `GetReleaseandRollbackEvidence`
- `GetReleaseandRollbackDecisionPackage`
- `GetReleaseandRollbackExceptions`
- `GetReleaseandRollbackAuditTrail`

Reference API surface:

```text
POST /api/v1/releaseandrollback/commands
GET  /api/v1/releaseandrollback/{id}
GET  /api/v1/releaseandrollback/{id}/timeline
GET  /api/v1/releaseandrollback/{id}/evidence
GET  /api/v1/releaseandrollback/{id}/decisions
POST /api/v1/releaseandrollback/{id}/exceptions
POST /api/v1/releaseandrollback/{id}/corrections
```
...
|---|---|
| `releaseandrollback_aggregate` | authoritative subject |
| `releaseandrollback_version` | immutable submitted, approved, issued, or signed versions |
| `releaseandrollback_relationship` | governed relationships |
| `releaseandrollback_decision` | binding Decisions and rationale |
| `releaseandrollback_evidence_link` | provenance and evidence relationships |
| `releaseandrollback_event_outbox` | transactional Event publication |
| `releaseandrollback_idempotency` | duplicate-write protection |
| `releaseandrollback_exception` | exception, claim, dispute, or hold |
| `releaseandrollback_correction` | correction and supersession lineage |
| `releaseandrollback_audit` | privileged action evidence |
...
- `Release and Rollback` is part of the target GTOS handbook scope.
