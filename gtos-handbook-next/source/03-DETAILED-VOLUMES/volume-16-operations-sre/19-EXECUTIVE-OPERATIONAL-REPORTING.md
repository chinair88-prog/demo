...
| Volume | `16` |
| Chapter ID | `V16-19` |
| Subject | `ExecutiveOperationalReporting` |
...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Executive Operational Reporting**.
...
|---|---|
| `ExecutiveOperationalReporting` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ExecutiveOperationalReportingVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ExecutiveOperationalReportingDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ExecutiveOperationalReportingEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ExecutiveOperationalReportingException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ExecutiveOperationalReporting {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateExecutiveOperationalReporting`
- `ValidateExecutiveOperationalReporting`
- `ApproveExecutiveOperationalReporting`
- `ActivateExecutiveOperationalReporting`
- `SuspendExecutiveOperationalReporting`
- `CorrectExecutiveOperationalReporting`
- `RetireExecutiveOperationalReporting`

Reference Events:

- `ExecutiveOperationalReportingCreated`
- `ExecutiveOperationalReportingValidated`
- `ExecutiveOperationalReportingApproved`
- `ExecutiveOperationalReportingActivated`
- `ExecutiveOperationalReportingSuspended`
- `ExecutiveOperationalReportingCorrected`
- `ExecutiveOperationalReportingRetired`

Reference queries:

- `GetExecutiveOperationalReporting`
- `ListExecutiveOperationalReportingRecords`
- `GetExecutiveOperationalReportingTimeline`
- `GetExecutiveOperationalReportingEvidence`
- `GetExecutiveOperationalReportingDecisionPackage`
- `GetExecutiveOperationalReportingExceptions`
- `GetExecutiveOperationalReportingAuditTrail`

Reference API surface:

```text
POST /api/v1/executiveoperationalreporting/commands
GET  /api/v1/executiveoperationalreporting/{id}
GET  /api/v1/executiveoperationalreporting/{id}/timeline
GET  /api/v1/executiveoperationalreporting/{id}/evidence
GET  /api/v1/executiveoperationalreporting/{id}/decisions
POST /api/v1/executiveoperationalreporting/{id}/exceptions
POST /api/v1/executiveoperationalreporting/{id}/corrections
```
...
|---|---|
| `executiveoperationalreporting_aggregate` | authoritative subject |
| `executiveoperationalreporting_version` | immutable submitted, approved, issued, or signed versions |
| `executiveoperationalreporting_relationship` | governed relationships |
| `executiveoperationalreporting_decision` | binding Decisions and rationale |
| `executiveoperationalreporting_evidence_link` | provenance and evidence relationships |
| `executiveoperationalreporting_event_outbox` | transactional Event publication |
| `executiveoperationalreporting_idempotency` | duplicate-write protection |
| `executiveoperationalreporting_exception` | exception, claim, dispute, or hold |
| `executiveoperationalreporting_correction` | correction and supersession lineage |
| `executiveoperationalreporting_audit` | privileged action evidence |
...
- `Executive Operational Reporting` is part of the target GTOS handbook scope.
