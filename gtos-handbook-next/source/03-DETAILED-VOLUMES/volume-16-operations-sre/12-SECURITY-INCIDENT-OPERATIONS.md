...
| Volume | `16` |
| Chapter ID | `V16-12` |
| Subject | `SecurityIncidentOperations` |
...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Security Incident Operations**.
...
|---|---|
| `SecurityIncidentOperations` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SecurityIncidentOperationsVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SecurityIncidentOperationsDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SecurityIncidentOperationsEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `SecurityIncidentOperationsException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
SecurityIncidentOperations {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateSecurityIncidentOperations`
- `ValidateSecurityIncidentOperations`
- `ApproveSecurityIncidentOperations`
- `ActivateSecurityIncidentOperations`
- `SuspendSecurityIncidentOperations`
- `CorrectSecurityIncidentOperations`
- `RetireSecurityIncidentOperations`

Reference Events:

- `SecurityIncidentOperationsCreated`
- `SecurityIncidentOperationsValidated`
- `SecurityIncidentOperationsApproved`
- `SecurityIncidentOperationsActivated`
- `SecurityIncidentOperationsSuspended`
- `SecurityIncidentOperationsCorrected`
- `SecurityIncidentOperationsRetired`

Reference queries:

- `GetSecurityIncidentOperations`
- `ListSecurityIncidentOperationsRecords`
- `GetSecurityIncidentOperationsTimeline`
- `GetSecurityIncidentOperationsEvidence`
- `GetSecurityIncidentOperationsDecisionPackage`
- `GetSecurityIncidentOperationsExceptions`
- `GetSecurityIncidentOperationsAuditTrail`

Reference API surface:

```text
POST /api/v1/securityincidentoperations/commands
GET  /api/v1/securityincidentoperations/{id}
GET  /api/v1/securityincidentoperations/{id}/timeline
GET  /api/v1/securityincidentoperations/{id}/evidence
GET  /api/v1/securityincidentoperations/{id}/decisions
POST /api/v1/securityincidentoperations/{id}/exceptions
POST /api/v1/securityincidentoperations/{id}/corrections
```
...
|---|---|
| `securityincidentoperations_aggregate` | authoritative subject |
| `securityincidentoperations_version` | immutable submitted, approved, issued, or signed versions |
| `securityincidentoperations_relationship` | governed relationships |
| `securityincidentoperations_decision` | binding Decisions and rationale |
| `securityincidentoperations_evidence_link` | provenance and evidence relationships |
| `securityincidentoperations_event_outbox` | transactional Event publication |
| `securityincidentoperations_idempotency` | duplicate-write protection |
| `securityincidentoperations_exception` | exception, claim, dispute, or hold |
| `securityincidentoperations_correction` | correction and supersession lineage |
| `securityincidentoperations_audit` | privileged action evidence |
...
- `Security Incident Operations` is part of the target GTOS handbook scope.
