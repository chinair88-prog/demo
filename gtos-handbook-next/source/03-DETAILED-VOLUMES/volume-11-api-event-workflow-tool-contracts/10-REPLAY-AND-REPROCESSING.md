...
This chapter defines how GTOS shall **define interoperable semantics, schemas, compatibility, security, failure, testing, and lifecycle governance for Replay and Reprocessing**.
...
|---|---|
| `ReplayandReprocessing` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReplayandReprocessingVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReplayandReprocessingDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReplayandReprocessingEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReplayandReprocessingException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
ReplayandReprocessing {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateReplayandReprocessing`
- `ValidateReplayandReprocessing`
- `ApproveReplayandReprocessing`
- `ActivateReplayandReprocessing`
- `SuspendReplayandReprocessing`
- `CorrectReplayandReprocessing`
- `RetireReplayandReprocessing`

Reference Events:

- `ReplayandReprocessingCreated`
- `ReplayandReprocessingValidated`
- `ReplayandReprocessingApproved`
- `ReplayandReprocessingActivated`
- `ReplayandReprocessingSuspended`
- `ReplayandReprocessingCorrected`
- `ReplayandReprocessingRetired`

Reference queries:

- `GetReplayandReprocessing`
- `ListReplayandReprocessingRecords`
- `GetReplayandReprocessingTimeline`
- `GetReplayandReprocessingEvidence`
- `GetReplayandReprocessingDecisionPackage`
- `GetReplayandReprocessingExceptions`
- `GetReplayandReprocessingAuditTrail`

Reference API surface:

```text
POST /api/v1/replayandreprocessing/commands
GET  /api/v1/replayandreprocessing/{id}
GET  /api/v1/replayandreprocessing/{id}/timeline
GET  /api/v1/replayandreprocessing/{id}/evidence
GET  /api/v1/replayandreprocessing/{id}/decisions
POST /api/v1/replayandreprocessing/{id}/exceptions
POST /api/v1/replayandreprocessing/{id}/corrections
```
...
|---|---|
| `replayandreprocessing_aggregate` | authoritative subject |
| `replayandreprocessing_version` | immutable submitted, approved, issued, or signed versions |
| `replayandreprocessing_relationship` | governed relationships |
| `replayandreprocessing_decision` | binding Decisions and rationale |
| `replayandreprocessing_evidence_link` | provenance and evidence relationships |
| `replayandreprocessing_event_outbox` | transactional Event publication |
| `replayandreprocessing_idempotency` | duplicate-write protection |
| `replayandreprocessing_exception` | exception, claim, dispute, or hold |
| `replayandreprocessing_correction` | correction and supersession lineage |
| `replayandreprocessing_audit` | privileged action evidence |
