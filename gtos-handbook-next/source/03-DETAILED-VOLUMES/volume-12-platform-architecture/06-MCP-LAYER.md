...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for MCP Layer**.
...
|---|---|
| `MCPLayer` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MCPLayerVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MCPLayerDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MCPLayerEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `MCPLayerException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `GetMCPLayerTimeline`
- `GetMCPLayerEvidence`
- `GetMCPLayerDecisionPackage`
- `GetMCPLayerExceptions`
- `GetMCPLayerAuditTrail`

Reference API surface:

```text
POST /api/v1/mcplayer/commands
GET  /api/v1/mcplayer/{id}
GET  /api/v1/mcplayer/{id}/timeline
GET  /api/v1/mcplayer/{id}/evidence
GET  /api/v1/mcplayer/{id}/decisions
POST /api/v1/mcplayer/{id}/exceptions
POST /api/v1/mcplayer/{id}/corrections
```
...
|---|---|
| `mcplayer_aggregate` | authoritative subject |
| `mcplayer_version` | immutable submitted, approved, issued, or signed versions |
| `mcplayer_relationship` | governed relationships |
| `mcplayer_decision` | binding Decisions and rationale |
| `mcplayer_evidence_link` | provenance and evidence relationships |
| `mcplayer_event_outbox` | transactional Event publication |
| `mcplayer_idempotency` | duplicate-write protection |
| `mcplayer_exception` | exception, claim, dispute, or hold |
| `mcplayer_correction` | correction and supersession lineage |
| `mcplayer_audit` | privileged action evidence |
...
- `MCP Layer` is part of the target GTOS handbook scope.
