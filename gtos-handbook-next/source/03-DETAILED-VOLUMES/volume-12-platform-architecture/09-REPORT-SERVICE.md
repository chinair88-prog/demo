...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Report Service**.
...
|---|---|
| `ReportService` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReportServiceVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReportServiceDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReportServiceEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ReportServiceException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreateReportService`
- `ValidateReportService`
- `ApproveReportService`
- `ActivateReportService`
- `SuspendReportService`
- `CorrectReportService`
- `RetireReportService`

Reference Events:

- `ReportServiceCreated`
- `ReportServiceValidated`
- `ReportServiceApproved`
- `ReportServiceActivated`
- `ReportServiceSuspended`
- `ReportServiceCorrected`
- `ReportServiceRetired`

Reference queries:

- `GetReportService`
- `ListReportServiceRecords`
- `GetReportServiceTimeline`
- `GetReportServiceEvidence`
- `GetReportServiceDecisionPackage`
- `GetReportServiceExceptions`
- `GetReportServiceAuditTrail`

Reference API surface:

```text
POST /api/v1/reportservice/commands
GET  /api/v1/reportservice/{id}
GET  /api/v1/reportservice/{id}/timeline
GET  /api/v1/reportservice/{id}/evidence
GET  /api/v1/reportservice/{id}/decisions
POST /api/v1/reportservice/{id}/exceptions
POST /api/v1/reportservice/{id}/corrections
```
...
|---|---|
| `reportservice_aggregate` | authoritative subject |
| `reportservice_version` | immutable submitted, approved, issued, or signed versions |
| `reportservice_relationship` | governed relationships |
| `reportservice_decision` | binding Decisions and rationale |
| `reportservice_evidence_link` | provenance and evidence relationships |
| `reportservice_event_outbox` | transactional Event publication |
| `reportservice_idempotency` | duplicate-write protection |
| `reportservice_exception` | exception, claim, dispute, or hold |
| `reportservice_correction` | correction and supersession lineage |
| `reportservice_audit` | privileged action evidence |
...
- `Report Service` is part of the target GTOS handbook scope.
