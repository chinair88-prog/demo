...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Service Discovery**.
...
|---|---|
| `ServiceDiscovery` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ServiceDiscoveryVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ServiceDiscoveryDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ServiceDiscoveryEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ServiceDiscoveryException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveServiceDiscovery`
- `ActivateServiceDiscovery`
- `SuspendServiceDiscovery`
- `CorrectServiceDiscovery`
- `RetireServiceDiscovery`

Reference Events:

- `ServiceDiscoveryCreated`
- `ServiceDiscoveryValidated`
- `ServiceDiscoveryApproved`
- `ServiceDiscoveryActivated`
- `ServiceDiscoverySuspended`
- `ServiceDiscoveryCorrected`
- `ServiceDiscoveryRetired`

Reference queries:

- `GetServiceDiscovery`
- `ListServiceDiscoveryRecords`
- `GetServiceDiscoveryTimeline`
- `GetServiceDiscoveryEvidence`
- `GetServiceDiscoveryDecisionPackage`
- `GetServiceDiscoveryExceptions`
- `GetServiceDiscoveryAuditTrail`

Reference API surface:

```text
POST /api/v1/servicediscovery/commands
GET  /api/v1/servicediscovery/{id}
GET  /api/v1/servicediscovery/{id}/timeline
GET  /api/v1/servicediscovery/{id}/evidence
GET  /api/v1/servicediscovery/{id}/decisions
POST /api/v1/servicediscovery/{id}/exceptions
POST /api/v1/servicediscovery/{id}/corrections
```
...
|---|---|
| `servicediscovery_aggregate` | authoritative subject |
| `servicediscovery_version` | immutable submitted, approved, issued, or signed versions |
| `servicediscovery_relationship` | governed relationships |
| `servicediscovery_decision` | binding Decisions and rationale |
| `servicediscovery_evidence_link` | provenance and evidence relationships |
| `servicediscovery_event_outbox` | transactional Event publication |
| `servicediscovery_idempotency` | duplicate-write protection |
| `servicediscovery_exception` | exception, claim, dispute, or hold |
| `servicediscovery_correction` | correction and supersession lineage |
| `servicediscovery_audit` | privileged action evidence |
...
- `Service Discovery` is part of the target GTOS handbook scope.
