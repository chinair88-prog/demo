...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for WebSocket Gateway**.
...
|---|---|
| `WebSocketGateway` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebSocketGatewayVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebSocketGatewayDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebSocketGatewayEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `WebSocketGatewayException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ValidateWebSocketGateway`
- `ApproveWebSocketGateway`
- `ActivateWebSocketGateway`
- `SuspendWebSocketGateway`
- `CorrectWebSocketGateway`
- `RetireWebSocketGateway`

Reference Events:

- `WebSocketGatewayCreated`
- `WebSocketGatewayValidated`
- `WebSocketGatewayApproved`
- `WebSocketGatewayActivated`
- `WebSocketGatewaySuspended`
- `WebSocketGatewayCorrected`
- `WebSocketGatewayRetired`

Reference queries:

- `GetWebSocketGateway`
- `ListWebSocketGatewayRecords`
- `GetWebSocketGatewayTimeline`
- `GetWebSocketGatewayEvidence`
- `GetWebSocketGatewayDecisionPackage`
- `GetWebSocketGatewayExceptions`
- `GetWebSocketGatewayAuditTrail`

Reference API surface:

```text
POST /api/v1/websocketgateway/commands
GET  /api/v1/websocketgateway/{id}
GET  /api/v1/websocketgateway/{id}/timeline
GET  /api/v1/websocketgateway/{id}/evidence
GET  /api/v1/websocketgateway/{id}/decisions
POST /api/v1/websocketgateway/{id}/exceptions
POST /api/v1/websocketgateway/{id}/corrections
```
...
|---|---|
| `websocketgateway_aggregate` | authoritative subject |
| `websocketgateway_version` | immutable submitted, approved, issued, or signed versions |
| `websocketgateway_relationship` | governed relationships |
| `websocketgateway_decision` | binding Decisions and rationale |
| `websocketgateway_evidence_link` | provenance and evidence relationships |
| `websocketgateway_event_outbox` | transactional Event publication |
| `websocketgateway_idempotency` | duplicate-write protection |
| `websocketgateway_exception` | exception, claim, dispute, or hold |
| `websocketgateway_correction` | correction and supersession lineage |
| `websocketgateway_audit` | privileged action evidence |
...
- `WebSocket Gateway` is part of the target GTOS handbook scope.
