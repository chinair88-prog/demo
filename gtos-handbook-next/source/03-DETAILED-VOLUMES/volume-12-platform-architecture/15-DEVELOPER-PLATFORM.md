...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Developer Platform**.
...
|---|---|
| `DeveloperPlatform` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DeveloperPlatformVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DeveloperPlatformDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DeveloperPlatformEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `DeveloperPlatformException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApproveDeveloperPlatform`
- `ActivateDeveloperPlatform`
- `SuspendDeveloperPlatform`
- `CorrectDeveloperPlatform`
- `RetireDeveloperPlatform`

Reference Events:

- `DeveloperPlatformCreated`
- `DeveloperPlatformValidated`
- `DeveloperPlatformApproved`
- `DeveloperPlatformActivated`
- `DeveloperPlatformSuspended`
- `DeveloperPlatformCorrected`
- `DeveloperPlatformRetired`

Reference queries:

- `GetDeveloperPlatform`
- `ListDeveloperPlatformRecords`
- `GetDeveloperPlatformTimeline`
- `GetDeveloperPlatformEvidence`
- `GetDeveloperPlatformDecisionPackage`
- `GetDeveloperPlatformExceptions`
- `GetDeveloperPlatformAuditTrail`

Reference API surface:

```text
POST /api/v1/developerplatform/commands
GET  /api/v1/developerplatform/{id}
GET  /api/v1/developerplatform/{id}/timeline
GET  /api/v1/developerplatform/{id}/evidence
GET  /api/v1/developerplatform/{id}/decisions
POST /api/v1/developerplatform/{id}/exceptions
POST /api/v1/developerplatform/{id}/corrections
```
...
|---|---|
| `developerplatform_aggregate` | authoritative subject |
| `developerplatform_version` | immutable submitted, approved, issued, or signed versions |
| `developerplatform_relationship` | governed relationships |
| `developerplatform_decision` | binding Decisions and rationale |
| `developerplatform_evidence_link` | provenance and evidence relationships |
| `developerplatform_event_outbox` | transactional Event publication |
| `developerplatform_idempotency` | duplicate-write protection |
| `developerplatform_exception` | exception, claim, dispute, or hold |
| `developerplatform_correction` | correction and supersession lineage |
| `developerplatform_audit` | privileged action evidence |
...
- `Developer Platform` is part of the target GTOS handbook scope.
