...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for API Gateway**.
...
|---|---|
| `APIGateway` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `APIGatewayVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `APIGatewayDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `APIGatewayEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `APIGatewayException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `GetAPIGateway`
- `ListAPIGatewayRecords`
- `GetAPIGatewayTimeline`
- `GetAPIGatewayEvidence`
- `GetAPIGatewayDecisionPackage`
- `GetAPIGatewayExceptions`
- `GetAPIGatewayAuditTrail`

Reference API surface:

```text
POST /api/v1/apigateway/commands
GET  /api/v1/apigateway/{id}
GET  /api/v1/apigateway/{id}/timeline
GET  /api/v1/apigateway/{id}/evidence
GET  /api/v1/apigateway/{id}/decisions
POST /api/v1/apigateway/{id}/exceptions
POST /api/v1/apigateway/{id}/corrections
```
...
|---|---|
| `apigateway_aggregate` | authoritative subject |
| `apigateway_version` | immutable submitted, approved, issued, or signed versions |
| `apigateway_relationship` | governed relationships |
| `apigateway_decision` | binding Decisions and rationale |
| `apigateway_evidence_link` | provenance and evidence relationships |
| `apigateway_event_outbox` | transactional Event publication |
| `apigateway_idempotency` | duplicate-write protection |
| `apigateway_exception` | exception, claim, dispute, or hold |
| `apigateway_correction` | correction and supersession lineage |
| `apigateway_audit` | privileged action evidence |
...
- `API Gateway` is part of the target GTOS handbook scope.
