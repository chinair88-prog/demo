...
This chapter defines how GTOS shall **define enterprise and Domain data ownership, semantics, persistence, quality, lineage, correction, protection, operations, and verification for Vector Stores**.
...
|---|---|
| `VectorStores` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `VectorStoresVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `VectorStoresDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `VectorStoresEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `VectorStoresException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `VectorStoresCreated`
- `VectorStoresValidated`
- `VectorStoresApproved`
- `VectorStoresActivated`
- `VectorStoresSuspended`
- `VectorStoresCorrected`
- `VectorStoresRetired`

Reference queries:

- `GetVectorStores`
- `ListVectorStoresRecords`
- `GetVectorStoresTimeline`
- `GetVectorStoresEvidence`
- `GetVectorStoresDecisionPackage`
- `GetVectorStoresExceptions`
- `GetVectorStoresAuditTrail`

Reference API surface:

```text
POST /api/v1/vectorstores/commands
GET  /api/v1/vectorstores/{id}
GET  /api/v1/vectorstores/{id}/timeline
GET  /api/v1/vectorstores/{id}/evidence
GET  /api/v1/vectorstores/{id}/decisions
POST /api/v1/vectorstores/{id}/exceptions
POST /api/v1/vectorstores/{id}/corrections
```
...
|---|---|
| `vectorstores_aggregate` | authoritative subject |
| `vectorstores_version` | immutable submitted, approved, issued, or signed versions |
| `vectorstores_relationship` | governed relationships |
| `vectorstores_decision` | binding Decisions and rationale |
| `vectorstores_evidence_link` | provenance and evidence relationships |
| `vectorstores_event_outbox` | transactional Event publication |
| `vectorstores_idempotency` | duplicate-write protection |
| `vectorstores_exception` | exception, claim, dispute, or hold |
| `vectorstores_correction` | correction and supersession lineage |
| `vectorstores_audit` | privileged action evidence |
