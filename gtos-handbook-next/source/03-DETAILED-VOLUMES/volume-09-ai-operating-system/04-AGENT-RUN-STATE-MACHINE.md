...
This chapter defines how GTOS shall **define the ownership, lifecycle, policy, evidence, Tools, security, evaluation, failure, recovery, and repository verification requirements for Agent Run State Machine**.
...
|---|---|
| `AgentRunStateMachine` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AgentRunStateMachineVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AgentRunStateMachineDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AgentRunStateMachineEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AgentRunStateMachineException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ValidateAgentRunStateMachine`
- `ApproveAgentRunStateMachine`
- `ActivateAgentRunStateMachine`
- `SuspendAgentRunStateMachine`
- `CorrectAgentRunStateMachine`
- `RetireAgentRunStateMachine`

Reference Events:

- `AgentRunStateMachineCreated`
- `AgentRunStateMachineValidated`
- `AgentRunStateMachineApproved`
- `AgentRunStateMachineActivated`
- `AgentRunStateMachineSuspended`
- `AgentRunStateMachineCorrected`
- `AgentRunStateMachineRetired`

Reference queries:

- `GetAgentRunStateMachine`
- `ListAgentRunStateMachineRecords`
- `GetAgentRunStateMachineTimeline`
- `GetAgentRunStateMachineEvidence`
- `GetAgentRunStateMachineDecisionPackage`
- `GetAgentRunStateMachineExceptions`
- `GetAgentRunStateMachineAuditTrail`

Reference API surface:

```text
POST /api/v1/agentrunstatemachine/commands
GET  /api/v1/agentrunstatemachine/{id}
GET  /api/v1/agentrunstatemachine/{id}/timeline
GET  /api/v1/agentrunstatemachine/{id}/evidence
GET  /api/v1/agentrunstatemachine/{id}/decisions
POST /api/v1/agentrunstatemachine/{id}/exceptions
POST /api/v1/agentrunstatemachine/{id}/corrections
```
...
|---|---|
| `agentrunstatemachine_aggregate` | authoritative subject |
| `agentrunstatemachine_version` | immutable submitted, approved, issued, or signed versions |
| `agentrunstatemachine_relationship` | governed relationships |
| `agentrunstatemachine_decision` | binding Decisions and rationale |
| `agentrunstatemachine_evidence_link` | provenance and evidence relationships |
| `agentrunstatemachine_event_outbox` | transactional Event publication |
| `agentrunstatemachine_idempotency` | duplicate-write protection |
| `agentrunstatemachine_exception` | exception, claim, dispute, or hold |
| `agentrunstatemachine_correction` | correction and supersession lineage |
| `agentrunstatemachine_audit` | privileged action evidence |
