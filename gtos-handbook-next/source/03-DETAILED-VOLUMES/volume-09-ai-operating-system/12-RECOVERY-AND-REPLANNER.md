# Recovery and Replanner

| Metadata | Value |
|---|---|
| Volume | `9` |
| Chapter ID | `V09-12` |
| Subject | `RecoveryandReplanner` |
...
This chapter defines how GTOS shall **define the ownership, lifecycle, policy, evidence, Tools, security, evaluation, failure, recovery, and repository verification requirements for Recovery and Replanner**.
...
|---|---|
| `RecoveryandReplanner` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RecoveryandReplannerVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RecoveryandReplannerDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RecoveryandReplannerEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `RecoveryandReplannerException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |

Reference aggregate:

```text
RecoveryandReplanner {
  subjectId
  tenantId
  organizationContext
  sourceIdentityRefs[]
...
- `CreateRecoveryandReplanner`
- `ValidateRecoveryandReplanner`
- `ApproveRecoveryandReplanner`
- `ActivateRecoveryandReplanner`
- `SuspendRecoveryandReplanner`
- `CorrectRecoveryandReplanner`
- `RetireRecoveryandReplanner`

Reference Events:

- `RecoveryandReplannerCreated`
- `RecoveryandReplannerValidated`
- `RecoveryandReplannerApproved`
- `RecoveryandReplannerActivated`
- `RecoveryandReplannerSuspended`
- `RecoveryandReplannerCorrected`
- `RecoveryandReplannerRetired`

Reference queries:

- `GetRecoveryandReplanner`
- `ListRecoveryandReplannerRecords`
- `GetRecoveryandReplannerTimeline`
- `GetRecoveryandReplannerEvidence`
- `GetRecoveryandReplannerDecisionPackage`
- `GetRecoveryandReplannerExceptions`
- `GetRecoveryandReplannerAuditTrail`

Reference API surface:

```text
POST /api/v1/recoveryandreplanner/commands
GET  /api/v1/recoveryandreplanner/{id}
GET  /api/v1/recoveryandreplanner/{id}/timeline
GET  /api/v1/recoveryandreplanner/{id}/evidence
GET  /api/v1/recoveryandreplanner/{id}/decisions
POST /api/v1/recoveryandreplanner/{id}/exceptions
POST /api/v1/recoveryandreplanner/{id}/corrections
```
...
|---|---|
| `recoveryandreplanner_aggregate` | authoritative subject |
| `recoveryandreplanner_version` | immutable submitted, approved, issued, or signed versions |
| `recoveryandreplanner_relationship` | governed relationships |
| `recoveryandreplanner_decision` | binding Decisions and rationale |
| `recoveryandreplanner_evidence_link` | provenance and evidence relationships |
| `recoveryandreplanner_event_outbox` | transactional Event publication |
| `recoveryandreplanner_idempotency` | duplicate-write protection |
| `recoveryandreplanner_exception` | exception, claim, dispute, or hold |
| `recoveryandreplanner_correction` | correction and supersession lineage |
| `recoveryandreplanner_audit` | privileged action evidence |
