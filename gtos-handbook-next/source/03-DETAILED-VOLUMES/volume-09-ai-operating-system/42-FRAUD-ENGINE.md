...
This chapter defines how GTOS shall **define the ownership, lifecycle, policy, evidence, Tools, security, evaluation, failure, recovery, and repository verification requirements for Fraud Engine**.
...
|---|---|
| `FraudEngine` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FraudEngineVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FraudEngineDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FraudEngineEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FraudEngineException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `FraudEngineCreated`
- `FraudEngineValidated`
- `FraudEngineApproved`
- `FraudEngineActivated`
- `FraudEngineSuspended`
- `FraudEngineCorrected`
- `FraudEngineRetired`

Reference queries:

- `GetFraudEngine`
- `ListFraudEngineRecords`
- `GetFraudEngineTimeline`
- `GetFraudEngineEvidence`
- `GetFraudEngineDecisionPackage`
- `GetFraudEngineExceptions`
- `GetFraudEngineAuditTrail`

Reference API surface:

```text
POST /api/v1/fraudengine/commands
GET  /api/v1/fraudengine/{id}
GET  /api/v1/fraudengine/{id}/timeline
GET  /api/v1/fraudengine/{id}/evidence
GET  /api/v1/fraudengine/{id}/decisions
POST /api/v1/fraudengine/{id}/exceptions
POST /api/v1/fraudengine/{id}/corrections
```
...
|---|---|
| `fraudengine_aggregate` | authoritative subject |
| `fraudengine_version` | immutable submitted, approved, issued, or signed versions |
| `fraudengine_relationship` | governed relationships |
| `fraudengine_decision` | binding Decisions and rationale |
| `fraudengine_evidence_link` | provenance and evidence relationships |
| `fraudengine_event_outbox` | transactional Event publication |
| `fraudengine_idempotency` | duplicate-write protection |
| `fraudengine_exception` | exception, claim, dispute, or hold |
| `fraudengine_correction` | correction and supersession lineage |
| `fraudengine_audit` | privileged action evidence |
