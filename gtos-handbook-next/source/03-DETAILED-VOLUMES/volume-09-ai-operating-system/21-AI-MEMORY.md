...
This chapter defines how GTOS shall **define the ownership, lifecycle, policy, evidence, Tools, security, evaluation, failure, recovery, and repository verification requirements for AI Memory**.
...
|---|---|
| `AIMemory` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIMemoryVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIMemoryDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIMemoryEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AIMemoryException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `GetAIMemoryTimeline`
- `GetAIMemoryEvidence`
- `GetAIMemoryDecisionPackage`
- `GetAIMemoryExceptions`
- `GetAIMemoryAuditTrail`

Reference API surface:

```text
POST /api/v1/aimemory/commands
GET  /api/v1/aimemory/{id}
GET  /api/v1/aimemory/{id}/timeline
GET  /api/v1/aimemory/{id}/evidence
GET  /api/v1/aimemory/{id}/decisions
POST /api/v1/aimemory/{id}/exceptions
POST /api/v1/aimemory/{id}/corrections
```
...
|---|---|
| `aimemory_aggregate` | authoritative subject |
| `aimemory_version` | immutable submitted, approved, issued, or signed versions |
| `aimemory_relationship` | governed relationships |
| `aimemory_decision` | binding Decisions and rationale |
| `aimemory_evidence_link` | provenance and evidence relationships |
| `aimemory_event_outbox` | transactional Event publication |
| `aimemory_idempotency` | duplicate-write protection |
| `aimemory_exception` | exception, claim, dispute, or hold |
| `aimemory_correction` | correction and supersession lineage |
| `aimemory_audit` | privileged action evidence |
