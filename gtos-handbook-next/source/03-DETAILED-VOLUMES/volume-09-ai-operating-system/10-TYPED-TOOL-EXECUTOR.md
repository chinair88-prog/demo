...
This chapter defines how GTOS shall **define the ownership, lifecycle, policy, evidence, Tools, security, evaluation, failure, recovery, and repository verification requirements for Typed Tool Executor**.
...
|---|---|
| `TypedToolExecutor` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `TypedToolExecutorVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `TypedToolExecutorDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `TypedToolExecutorEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `TypedToolExecutorException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreateTypedToolExecutor`
- `ValidateTypedToolExecutor`
- `ApproveTypedToolExecutor`
- `ActivateTypedToolExecutor`
- `SuspendTypedToolExecutor`
- `CorrectTypedToolExecutor`
- `RetireTypedToolExecutor`

Reference Events:

- `TypedToolExecutorCreated`
- `TypedToolExecutorValidated`
- `TypedToolExecutorApproved`
- `TypedToolExecutorActivated`
- `TypedToolExecutorSuspended`
- `TypedToolExecutorCorrected`
- `TypedToolExecutorRetired`

Reference queries:

- `GetTypedToolExecutor`
- `ListTypedToolExecutorRecords`
- `GetTypedToolExecutorTimeline`
- `GetTypedToolExecutorEvidence`
- `GetTypedToolExecutorDecisionPackage`
- `GetTypedToolExecutorExceptions`
- `GetTypedToolExecutorAuditTrail`

Reference API surface:

```text
POST /api/v1/typedtoolexecutor/commands
GET  /api/v1/typedtoolexecutor/{id}
GET  /api/v1/typedtoolexecutor/{id}/timeline
GET  /api/v1/typedtoolexecutor/{id}/evidence
GET  /api/v1/typedtoolexecutor/{id}/decisions
POST /api/v1/typedtoolexecutor/{id}/exceptions
POST /api/v1/typedtoolexecutor/{id}/corrections
```
...
|---|---|
| `typedtoolexecutor_aggregate` | authoritative subject |
| `typedtoolexecutor_version` | immutable submitted, approved, issued, or signed versions |
| `typedtoolexecutor_relationship` | governed relationships |
| `typedtoolexecutor_decision` | binding Decisions and rationale |
| `typedtoolexecutor_evidence_link` | provenance and evidence relationships |
| `typedtoolexecutor_event_outbox` | transactional Event publication |
| `typedtoolexecutor_idempotency` | duplicate-write protection |
| `typedtoolexecutor_exception` | exception, claim, dispute, or hold |
| `typedtoolexecutor_correction` | correction and supersession lineage |
| `typedtoolexecutor_audit` | privileged action evidence |
