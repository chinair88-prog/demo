...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Unit Testing**.
...
|---|---|
| `UnitTesting` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `UnitTestingVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `UnitTestingDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `UnitTestingEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `UnitTestingException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `UnitTestingCreated`
- `UnitTestingValidated`
- `UnitTestingApproved`
- `UnitTestingActivated`
- `UnitTestingSuspended`
- `UnitTestingCorrected`
- `UnitTestingRetired`

Reference queries:

- `GetUnitTesting`
- `ListUnitTestingRecords`
- `GetUnitTestingTimeline`
- `GetUnitTestingEvidence`
- `GetUnitTestingDecisionPackage`
- `GetUnitTestingExceptions`
- `GetUnitTestingAuditTrail`

Reference API surface:

```text
POST /api/v1/unittesting/commands
GET  /api/v1/unittesting/{id}
GET  /api/v1/unittesting/{id}/timeline
GET  /api/v1/unittesting/{id}/evidence
GET  /api/v1/unittesting/{id}/decisions
POST /api/v1/unittesting/{id}/exceptions
POST /api/v1/unittesting/{id}/corrections
```
...
|---|---|
| `unittesting_aggregate` | authoritative subject |
| `unittesting_version` | immutable submitted, approved, issued, or signed versions |
| `unittesting_relationship` | governed relationships |
| `unittesting_decision` | binding Decisions and rationale |
| `unittesting_evidence_link` | provenance and evidence relationships |
| `unittesting_event_outbox` | transactional Event publication |
| `unittesting_idempotency` | duplicate-write protection |
| `unittesting_exception` | exception, claim, dispute, or hold |
| `unittesting_correction` | correction and supersession lineage |
| `unittesting_audit` | privileged action evidence |
...
- `Unit Testing` is part of the target GTOS handbook scope.
