...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Property Testing**.
...
|---|---|
| `PropertyTesting` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PropertyTestingVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PropertyTestingDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PropertyTestingEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PropertyTestingException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreatePropertyTesting`
- `ValidatePropertyTesting`
- `ApprovePropertyTesting`
- `ActivatePropertyTesting`
- `SuspendPropertyTesting`
- `CorrectPropertyTesting`
- `RetirePropertyTesting`

Reference Events:

- `PropertyTestingCreated`
- `PropertyTestingValidated`
- `PropertyTestingApproved`
- `PropertyTestingActivated`
- `PropertyTestingSuspended`
- `PropertyTestingCorrected`
- `PropertyTestingRetired`

Reference queries:

- `GetPropertyTesting`
- `ListPropertyTestingRecords`
- `GetPropertyTestingTimeline`
- `GetPropertyTestingEvidence`
- `GetPropertyTestingDecisionPackage`
- `GetPropertyTestingExceptions`
- `GetPropertyTestingAuditTrail`

Reference API surface:

```text
POST /api/v1/propertytesting/commands
GET  /api/v1/propertytesting/{id}
GET  /api/v1/propertytesting/{id}/timeline
GET  /api/v1/propertytesting/{id}/evidence
GET  /api/v1/propertytesting/{id}/decisions
POST /api/v1/propertytesting/{id}/exceptions
POST /api/v1/propertytesting/{id}/corrections
```
...
|---|---|
| `propertytesting_aggregate` | authoritative subject |
| `propertytesting_version` | immutable submitted, approved, issued, or signed versions |
| `propertytesting_relationship` | governed relationships |
| `propertytesting_decision` | binding Decisions and rationale |
| `propertytesting_evidence_link` | provenance and evidence relationships |
| `propertytesting_event_outbox` | transactional Event publication |
| `propertytesting_idempotency` | duplicate-write protection |
| `propertytesting_exception` | exception, claim, dispute, or hold |
| `propertytesting_correction` | correction and supersession lineage |
| `propertytesting_audit` | privileged action evidence |
...
- `Property Testing` is part of the target GTOS handbook scope.
