...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Threat Modeling**.
...
|---|---|
| `ThreatModeling` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ThreatModelingVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ThreatModelingDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ThreatModelingEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `ThreatModelingException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreateThreatModeling`
- `ValidateThreatModeling`
- `ApproveThreatModeling`
- `ActivateThreatModeling`
- `SuspendThreatModeling`
- `CorrectThreatModeling`
- `RetireThreatModeling`

Reference Events:

- `ThreatModelingCreated`
- `ThreatModelingValidated`
- `ThreatModelingApproved`
- `ThreatModelingActivated`
- `ThreatModelingSuspended`
- `ThreatModelingCorrected`
- `ThreatModelingRetired`

Reference queries:

- `GetThreatModeling`
- `ListThreatModelingRecords`
- `GetThreatModelingTimeline`
- `GetThreatModelingEvidence`
- `GetThreatModelingDecisionPackage`
- `GetThreatModelingExceptions`
- `GetThreatModelingAuditTrail`

Reference API surface:

```text
POST /api/v1/threatmodeling/commands
GET  /api/v1/threatmodeling/{id}
GET  /api/v1/threatmodeling/{id}/timeline
GET  /api/v1/threatmodeling/{id}/evidence
GET  /api/v1/threatmodeling/{id}/decisions
POST /api/v1/threatmodeling/{id}/exceptions
POST /api/v1/threatmodeling/{id}/corrections
```
...
|---|---|
| `threatmodeling_aggregate` | authoritative subject |
| `threatmodeling_version` | immutable submitted, approved, issued, or signed versions |
| `threatmodeling_relationship` | governed relationships |
| `threatmodeling_decision` | binding Decisions and rationale |
| `threatmodeling_evidence_link` | provenance and evidence relationships |
| `threatmodeling_event_outbox` | transactional Event publication |
| `threatmodeling_idempotency` | duplicate-write protection |
| `threatmodeling_exception` | exception, claim, dispute, or hold |
| `threatmodeling_correction` | correction and supersession lineage |
| `threatmodeling_audit` | privileged action evidence |
...
- `Threat Modeling` is part of the target GTOS handbook scope.
