...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Fraud and Abuse**.
...
|---|---|
| `FraudandAbuse` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FraudandAbuseVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FraudandAbuseDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FraudandAbuseEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `FraudandAbuseException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `CreateFraudandAbuse`
- `ValidateFraudandAbuse`
- `ApproveFraudandAbuse`
- `ActivateFraudandAbuse`
- `SuspendFraudandAbuse`
- `CorrectFraudandAbuse`
- `RetireFraudandAbuse`

Reference Events:

- `FraudandAbuseCreated`
- `FraudandAbuseValidated`
- `FraudandAbuseApproved`
- `FraudandAbuseActivated`
- `FraudandAbuseSuspended`
- `FraudandAbuseCorrected`
- `FraudandAbuseRetired`

Reference queries:

- `GetFraudandAbuse`
- `ListFraudandAbuseRecords`
- `GetFraudandAbuseTimeline`
- `GetFraudandAbuseEvidence`
- `GetFraudandAbuseDecisionPackage`
- `GetFraudandAbuseExceptions`
- `GetFraudandAbuseAuditTrail`

Reference API surface:

```text
POST /api/v1/fraudandabuse/commands
GET  /api/v1/fraudandabuse/{id}
GET  /api/v1/fraudandabuse/{id}/timeline
GET  /api/v1/fraudandabuse/{id}/evidence
GET  /api/v1/fraudandabuse/{id}/decisions
POST /api/v1/fraudandabuse/{id}/exceptions
POST /api/v1/fraudandabuse/{id}/corrections
```
...
|---|---|
| `fraudandabuse_aggregate` | authoritative subject |
| `fraudandabuse_version` | immutable submitted, approved, issued, or signed versions |
| `fraudandabuse_relationship` | governed relationships |
| `fraudandabuse_decision` | binding Decisions and rationale |
| `fraudandabuse_evidence_link` | provenance and evidence relationships |
| `fraudandabuse_event_outbox` | transactional Event publication |
| `fraudandabuse_idempotency` | duplicate-write protection |
| `fraudandabuse_exception` | exception, claim, dispute, or hold |
| `fraudandabuse_correction` | correction and supersession lineage |
| `fraudandabuse_audit` | privileged action evidence |
...
- `Fraud and Abuse` is part of the target GTOS handbook scope.
