...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for AI Security**.
...
|---|---|
| `AISecurity` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AISecurityVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AISecurityDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AISecurityEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AISecurityException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `GetAISecurity`
- `ListAISecurityRecords`
- `GetAISecurityTimeline`
- `GetAISecurityEvidence`
- `GetAISecurityDecisionPackage`
- `GetAISecurityExceptions`
- `GetAISecurityAuditTrail`

Reference API surface:

```text
POST /api/v1/aisecurity/commands
GET  /api/v1/aisecurity/{id}
GET  /api/v1/aisecurity/{id}/timeline
GET  /api/v1/aisecurity/{id}/evidence
GET  /api/v1/aisecurity/{id}/decisions
POST /api/v1/aisecurity/{id}/exceptions
POST /api/v1/aisecurity/{id}/corrections
```
...
|---|---|
| `aisecurity_aggregate` | authoritative subject |
| `aisecurity_version` | immutable submitted, approved, issued, or signed versions |
| `aisecurity_relationship` | governed relationships |
| `aisecurity_decision` | binding Decisions and rationale |
| `aisecurity_evidence_link` | provenance and evidence relationships |
| `aisecurity_event_outbox` | transactional Event publication |
| `aisecurity_idempotency` | duplicate-write protection |
| `aisecurity_exception` | exception, claim, dispute, or hold |
| `aisecurity_correction` | correction and supersession lineage |
| `aisecurity_audit` | privileged action evidence |
...
- `AI Security` is part of the target GTOS handbook scope.
