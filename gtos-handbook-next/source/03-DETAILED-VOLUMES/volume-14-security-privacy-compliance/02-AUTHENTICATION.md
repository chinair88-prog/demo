...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Authentication**.
...
|---|---|
| `Authentication` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AuthenticationVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AuthenticationDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AuthenticationEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `AuthenticationException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `AuthenticationCorrected`
- `AuthenticationRetired`

Reference queries:

- `GetAuthentication`
- `ListAuthenticationRecords`
- `GetAuthenticationTimeline`
- `GetAuthenticationEvidence`
- `GetAuthenticationDecisionPackage`
...
```text
POST /api/v1/authentication/commands
GET  /api/v1/authentication/{id}
GET  /api/v1/authentication/{id}/timeline
GET  /api/v1/authentication/{id}/evidence
GET  /api/v1/authentication/{id}/decisions
POST /api/v1/authentication/{id}/exceptions
POST /api/v1/authentication/{id}/corrections
```
...
|---|---|
| `authentication_aggregate` | authoritative subject |
| `authentication_version` | immutable submitted, approved, issued, or signed versions |
| `authentication_relationship` | governed relationships |
| `authentication_decision` | binding Decisions and rationale |
| `authentication_evidence_link` | provenance and evidence relationships |
| `authentication_event_outbox` | transactional Event publication |
| `authentication_idempotency` | duplicate-write protection |
| `authentication_exception` | exception, claim, dispute, or hold |
| `authentication_correction` | correction and supersession lineage |
| `authentication_audit` | privileged action evidence |
...
- `Authentication` is part of the target GTOS handbook scope.
