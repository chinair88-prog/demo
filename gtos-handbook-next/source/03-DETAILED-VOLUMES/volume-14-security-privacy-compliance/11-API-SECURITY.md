...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for API Security**.
...
|---|---|
| `APISecurity` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `APISecurityVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `APISecurityDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `APISecurityEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `APISecurityException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `APISecurityCreated`
- `APISecurityValidated`
- `APISecurityApproved`
- `APISecurityActivated`
- `APISecuritySuspended`
- `APISecurityCorrected`
- `APISecurityRetired`

Reference queries:

- `GetAPISecurity`
- `ListAPISecurityRecords`
- `GetAPISecurityTimeline`
- `GetAPISecurityEvidence`
- `GetAPISecurityDecisionPackage`
- `GetAPISecurityExceptions`
- `GetAPISecurityAuditTrail`

Reference API surface:

```text
POST /api/v1/apisecurity/commands
GET  /api/v1/apisecurity/{id}
GET  /api/v1/apisecurity/{id}/timeline
GET  /api/v1/apisecurity/{id}/evidence
GET  /api/v1/apisecurity/{id}/decisions
POST /api/v1/apisecurity/{id}/exceptions
POST /api/v1/apisecurity/{id}/corrections
```
...
|---|---|
| `apisecurity_aggregate` | authoritative subject |
| `apisecurity_version` | immutable submitted, approved, issued, or signed versions |
| `apisecurity_relationship` | governed relationships |
| `apisecurity_decision` | binding Decisions and rationale |
| `apisecurity_evidence_link` | provenance and evidence relationships |
| `apisecurity_event_outbox` | transactional Event publication |
| `apisecurity_idempotency` | duplicate-write protection |
| `apisecurity_exception` | exception, claim, dispute, or hold |
| `apisecurity_correction` | correction and supersession lineage |
| `apisecurity_audit` | privileged action evidence |
...
- `API Security` is part of the target GTOS handbook scope.
