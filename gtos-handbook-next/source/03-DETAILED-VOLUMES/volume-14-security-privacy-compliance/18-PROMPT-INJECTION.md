...
This chapter defines how GTOS shall **define the target architecture, implementation standard, evidence, security, reliability, operations, tests, migration and conformance requirements for Prompt Injection**.
...
|---|---|
| `PromptInjection` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PromptInjectionVersion` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PromptInjectionDecision` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PromptInjectionEvidenceSet` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
| `PromptInjectionException` | Independent identified subject, governed subordinate entity, or qualified perspective; ownership must be explicit |
...
- `ApprovePromptInjection`
- `ActivatePromptInjection`
- `SuspendPromptInjection`
- `CorrectPromptInjection`
- `RetirePromptInjection`

Reference Events:

- `PromptInjectionCreated`
- `PromptInjectionValidated`
- `PromptInjectionApproved`
- `PromptInjectionActivated`
- `PromptInjectionSuspended`
- `PromptInjectionCorrected`
- `PromptInjectionRetired`

Reference queries:

- `GetPromptInjection`
- `ListPromptInjectionRecords`
- `GetPromptInjectionTimeline`
- `GetPromptInjectionEvidence`
- `GetPromptInjectionDecisionPackage`
- `GetPromptInjectionExceptions`
- `GetPromptInjectionAuditTrail`

Reference API surface:

```text
POST /api/v1/promptinjection/commands
GET  /api/v1/promptinjection/{id}
GET  /api/v1/promptinjection/{id}/timeline
GET  /api/v1/promptinjection/{id}/evidence
GET  /api/v1/promptinjection/{id}/decisions
POST /api/v1/promptinjection/{id}/exceptions
POST /api/v1/promptinjection/{id}/corrections
```
...
|---|---|
| `promptinjection_aggregate` | authoritative subject |
| `promptinjection_version` | immutable submitted, approved, issued, or signed versions |
| `promptinjection_relationship` | governed relationships |
| `promptinjection_decision` | binding Decisions and rationale |
| `promptinjection_evidence_link` | provenance and evidence relationships |
| `promptinjection_event_outbox` | transactional Event publication |
| `promptinjection_idempotency` | duplicate-write protection |
| `promptinjection_exception` | exception, claim, dispute, or hold |
| `promptinjection_correction` | correction and supersession lineage |
| `promptinjection_audit` | privileged action evidence |
...
- `Prompt Injection` is part of the target GTOS handbook scope.
